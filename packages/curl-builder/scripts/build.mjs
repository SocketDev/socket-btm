...
export async function downloadCurl(options = {}) {
  ...
  // Normalize win32- to win- for directory path to match Makefile expectations.
  // Makefiles use 'win-arm64' and 'win-x64', not 'win32-arm64' and 'win32-x64'.
  const normalizedPlatformArch = resolvedPlatformArch.replace(/^win32-/, 'win-')
  const targetDir = path.join(
    packageRoot,
    'build',
    'downloaded',
    'curl',
    normalizedPlatformArch,
  )
...
export async function ensureCurl(options = {}) {
  ...
  // Normalize win32- to win- for directory path to match Makefile expectations.
  const normalizedPlatformArch = resolvedPlatformArch.replace(/^win32-/, 'win-')
  // With tool: 'curl', downloadSocketBtmRelease creates curl/assets/ subdirectory
...
if (isMainModule) {
  main().catch(err => {
    console.error('Error building curl:', err)
    process.exitCode = 1
  })
}
