...
// Guard every trampoline against a missing active target. The TLS slot
// defaults to nullptr; if user JS invokes a fast path without the
// preceding SetActiveTarget() call (cached reference crossing a microtask,
// V8 inlining pattern skipping the setter, or a CFunction extracted and
// called directly), dereferencing nullptr would SIGSEGV the process.
//
// V8 ≥ 12.x removed FastApiCallbackOptions::fallback. The modern idiom
// for signaling a fast-path failure is `options.isolate->ThrowError(...)`
// followed by returning a placeholder value — V8 observes the pending
// exception, discards the fast result, and replays the call through the
// slow callback (where a userland error is rethrown into JS). The
// placeholder value (0 / nullptr / void) never reaches JS on this path.
#define FFI_FAST_GUARD_VOID()                                               \
  do {                                                                      \
    if (active_fn_ptr_ == nullptr) {                                        \
      options.isolate->ThrowError("smol_ffi: active target not set");       \
      return;                                                               \
    }                                                                       \
  } while (0)
#define FFI_FAST_GUARD_RET(zero)                                            \
  do {                                                                      \
    if (active_fn_ptr_ == nullptr) {                                        \
      options.isolate->ThrowError("smol_ffi: active target not set");       \
      return (zero);                                                        \
