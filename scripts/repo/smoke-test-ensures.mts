 ...
 *     1 on any failure. Usage: node scripts/repo/smoke-test-ensures.mts node
 *     scripts/repo/smoke-test-ensures.mts --json
 */
...
export async function smokeTest(spec: EnsureSpec): Promise<CheckResult> {
  ...
  try {
    ...
    return {
      errors: [`module import failed: ${errorMessage(e)}`],
      ok: false,
      spec,
    }
  }
  ...
  try {
    ...
    errors.push(`runtime error: ${errorMessage(e)}`)
  }
