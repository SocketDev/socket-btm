...
sea_config_t* parse_sea_config(const char *config_path) {
    ...
    // Block absolute paths on Unix (config should be relative to project).
    // Note: Windows absolute paths (C:\) are allowed for compatibility.
    if (config_path[0] == '/') {
        fprintf(stderr, "Error: Absolute paths not allowed in config path\n");
        return NULL;
    }

    // Verify file exists and is regular file (not symlink).
    // Use lstat() to detect symlinks - prevents symlink attacks where attacker
    // creates symlink pointing to sensitive files (e.g., /etc/passwd).
    struct stat st;
