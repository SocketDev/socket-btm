...
int binpress_segment_embed(
    ...
    // Detect platform metadata using shared helper.
    uint8_t platform_byte;
    uint8_t arch_byte;
    uint8_t libc_byte;
    uint8_t compression_algorithm;

    smol_detect_platform_metadata(&platform_byte, &arch_byte, &libc_byte, &compression_algorithm);

    // Override architecture by parsing Mach-O stub binary.
    ...
    if (smol_build_section_data(compressed_data, compressed_size, uncompressed_size,
                                 platform_byte, arch_byte, libc_byte, compression_algorithm, &section) != 0) {
        free(compressed_data);
