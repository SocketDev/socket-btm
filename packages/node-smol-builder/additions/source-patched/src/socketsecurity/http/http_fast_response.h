// ============================================================================
// http_fast_response.h — Direct-to-socket HTTP response writer
// ============================================================================
//
// WHAT THIS FILE DOES
// Declares the FastResponse class, which writes complete HTTP responses
// directly to the OS network socket, bypassing Node.js's entire JavaScript
// HTTP response pipeline (Writable stream, cork/uncork, Buffer allocation).
//
// WHY IT EXISTS
// A normal Node.js `response.end(data)` triggers ~10 JS function calls and
// 2-3 memory allocations per response. FastResponse replaces all of that
// with a single C++ function that builds headers on the stack and calls
// uv_try_write() — one synchronous system call, zero heap allocations.
//
// HOW IT WORKS (the key insight)
// Every net.Socket in Node.js has a `_handle` property pointing to a C++
// LibuvStreamWrap object. We reach through that to get the raw uv_stream_t*
// (libuv's OS socket handle), then call uv_try_write() to send data
// directly to the kernel's socket buffer. If the kernel buffer is full
// (rare), we return false and let JS handle it the slow way.
// ============================================================================
