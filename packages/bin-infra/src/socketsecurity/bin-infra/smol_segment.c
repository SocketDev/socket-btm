...
#if defined(SMOL_USE_CODESIGN_INFRA)
#include "socketsecurity/codesign/codesign.h"

/*
 * DISABLED SEAM (off by default). The in-process, cross-host ad-hoc signer from
 * codesign-infra — no /usr/bin/codesign dependency, so a Mach-O can be signed on a
 * Linux/Windows build host too. Reads the binary, ad-hoc signs in memory, writes
 * the signed image back. Enable with -DSMOL_USE_CODESIGN_INFRA and link
 * codesign-infra + BoringSSL (libcrypto). See docs/ports/codesign-infra-lockstep.md
 * (phase 4: the binject seam). Replaces the fork-to-codesign default below.
 */
static int smol_codesign_infra(const char *binary_path) {
    FILE *f = fopen(binary_path, "rb");
    if (!f) {
        fprintf(stderr, "Error: codesign-infra: cannot open %s\n", binary_path);
        return -1;
    }
    fseek(f, 0, SEEK_END);
    long n = ftell(f);
    ...
    fclose(f);
    /* Identifier = basename, matching codesign's default ad-hoc identifier. */
    const char *base = strrchr(binary_path, '/');
    base = base ? base + 1 : binary_path;
    unsigned char *out = NULL;
    size_t out_len = 0;
    int rc = codesign_macho_adhoc(buf, (size_t)n, base, &out, &out_len);
    free(buf);
    if (rc != 0) {
        fprintf(stderr, "Error: codesign-infra: %s\n", codesign_last_error());
        return -1;
    }
    FILE *w = fopen(binary_path, "wb");
    int ok = w && fwrite(out, 1, out_len, w) == out_len;
    if (w) {
        fclose(w);
    }
    codesign_free(out);
...
#endif /* SMOL_USE_CODESIGN_INFRA */

/**
 * Ad-hoc code sign a binary (macOS, or any host via the codesign-infra seam).
 */
...
#if defined(SMOL_USE_CODESIGN_INFRA)
    return smol_codesign_infra(binary_path);
