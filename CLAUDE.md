...
🚨 **Builder publish dispatch order (after a registry/source SHA cascade):** curl + lief in PARALLEL → stubs (after curl + lief green) → binsuite (after stubs green) → node-smol (after binsuite green). Bump `cache-versions.json` BEFORE re-dispatching or the cache key doesn't change. Gated by `scripts/check-publish-prereq.mts`.

🚨 **Node.js Additions (`additions/`) constraints:** built-ins only (NEVER `@socketsecurity/*`); `require('fs')` not `require('node:fs')` (no `node:` at bootstrap); flat `.js` files; `'use strict';` start; primordials for Map/Set/Array. **C++: NEVER use exceptions** — `-fno-exceptions` makes `std::bad_alloc` abort the isolate. Allocate with `new (std::nothrow) T(...)` + null-check + `isolate->ThrowException(...)`.

🚨 **Source patches: 1 patch, 1 file. 1 file, 1 patch.** Bidirectional. Standard unified diff (`--- a/`, `+++ b/`), NEVER `git format-patch`. Required first-line headers: `# @<project>-versions: vX.Y.Z` + `# @description: ...`.

🚨 **SEA entry:** Node 25.7+ replaces ambient `require` inside a CJS SEA with embedder hooks resolving built-ins only. Always use `Module.createRequire(scriptPath)` — `createVFSRequire()` already does this; `await import(pathToFileURL(...))` has the same limitation.

Full Additions / C++ / SEA / source-patches / check-gates / build-conventions / glossary in [`docs/claude.md/repo/architecture.md`](docs/claude.md/repo/architecture.md) and the existing `docs/references/btm-*.md` references.
