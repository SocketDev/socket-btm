 ...
 *
 * Usage:
 *   node scripts/cover.mts [package-name]
 *
 * Examples:
 *   node scripts/cover.mts binject
 *   node scripts/cover.mts binpress
 *   node scripts/cover.mts binflate
 *   node scripts/cover.mts all
 */
...
async function runCoverageForPackage(packageName) {
  ...
  // For node-smol-builder, check test/coverage/ directory first.
  let makefilePath = path.join(packagePath, 'test', 'coverage', makefileName)
  if (!existsSync(makefilePath)) {
    // Fall back to package root for other packages.
