...
async function staticVerifyBinary(binaryPath, options = {}) {
  ...
  try {
    ...
    printError('Static verification failed', error)
    return false
  }
}
...
async function testBinaryWithDocker(binaryPath, options = {}) {
  ...
  try {
    ...
    printError('Docker test failed', error)
    return false
  }
}
...
export async function smokeTestBinary(binaryPath, options = {}) {
  ...
  try {
    ...
    printError(`Binary smoke test failed: ${binaryPath}`, error)
    return false
  }
}
