...
describe('cache-key', () => {
  describe('generateCacheKey', () => {
    it('should generate cache key with basic options', () => {
      ...
      expect(key).toMatch(
        new RegExp(
          `^v${escapeRegExp(nodeVersionRaw)}-darwin-arm64-[a-f0-9]{8}-2\\.215$`,
        ),
      )
    })
    ...
    it('should use default platform and arch if not provided', () => {
      ...
      expect(key).toMatch(
        new RegExp(
          `^v${escapeRegExp(nodeVersionRaw)}-\\w+-\\w+-[a-f0-9]{8}-2\\.215$`,
        ),
      )
    ...
    it('should handle different package versions', () => {
      ...
      expect(key1).not.toBe(key2)
      expect(key1).toContain('-2.100')
      expect(key2).toContain('-2.200')
    })
  ...
  describe('parseCacheKey', () => {
    it('should parse valid cache key', () => {
      const key = `v${nodeVersionRaw}-darwin-arm64-b71671ba-2.215`
    ...
    it('should return undefined for invalid cache key format', () => {
      ...
      // Missing 'v'
      expect(
        parseCacheKey(`${nodeVersionRaw}-darwin-arm64-b71671ba-2.215`),
      ).toBeUndefined()
    })

    it('should handle different node versions', () => {
      const key1 = 'v20.0.0-darwin-arm64-abc12345-2.100'
      const key2 = `v${nodeVersionRaw}-darwin-arm64-abc12345-2.100`
    ...
    it('should handle different platforms and architectures', () => {
      const darwinKey = `v${nodeVersionRaw}-darwin-arm64-abc12345-2.100`
      const linuxKey = `v${nodeVersionRaw}-linux-x64-abc12345-2.100`
      const win32Key = `v${nodeVersionRaw}-win32-x64-abc12345-2.100`
    ...
    it('should restore package version with dots', () => {
      const key = `v${nodeVersionRaw}-darwin-arm64-abc12345-2.215`
      const parsed = parseCacheKey(key)

      // Version restoration works for any version due to dot count encoding
