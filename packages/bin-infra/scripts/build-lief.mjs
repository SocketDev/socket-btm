  ...
  try {
    ...
    const listResult = await new Promise((resolve, reject) => {
      ...
      proc.on('close', code => {
        if (code === 0) {
          ...
          reject(new Error(`gh release list failed with exit code ${code}`))
        }
      })
    ...
    await runCommand(
      ...
      [
        ...
        assetName,
        '--output',
        path.join(liefBuildDir, assetName),
      ],
      packageRoot,
    )
...
async function main() {
  try {
    ...
    if (downloaded) {
      ...
      const liefLibUnixNew = path.join(buildDir, 'lief', 'libLIEF.a')
      const liefLibMSVCNew = path.join(buildDir, 'lief', 'LIEF.lib')
