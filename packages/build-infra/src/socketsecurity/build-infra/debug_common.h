 ...
 *   DEBUG=1                 -> enables all (backward compat)
...
static inline int _debug_is_enabled(const char *ns) {
    ...
    // Backward compatibility: treat "1", "true", "yes" as enable-all
...
/**
 * Legacy compatibility: DEBUG_INIT_SIMPLE() for code that doesn't use namespaces
 * Enables debug if DEBUG is truthy (1, true, yes, or any non-zero value)
 */
#define DEBUG_INIT_SIMPLE() do { \
    const char *debug_env = getenv("DEBUG"); \
    if (debug_env && debug_env[0] && \
        debug_env[0] != '0' && debug_env[0] != 'f' && debug_env[0] != 'F') { \
        _debug_enabled = 1; \
        snprintf(_debug_namespace, sizeof(_debug_namespace), "DEBUG"); \
    } \
} while(0)
