// ============================================================================
// macho_inject_lief.cpp — Inject data into Mach-O binaries (macOS executables)
// ============================================================================
//
// WHAT THIS FILE DOES
// Injects named data segments into Mach-O binaries using LIEF. Handles
// segment creation, ad-hoc code signing, fat/universal binary support,
// and atomic file writes. Also removes existing code signatures before
// injection (they're invalid after modification anyway).
//
// WHY IT EXISTS
// macOS executables use the Mach-O format which has segments and sections.
// LIEF provides the C++ API for manipulating these, but needs careful
// handling for content injection (uses a hybrid LIEF + manual I/O approach).
// ============================================================================
/**
...
static bool sign_binary_adhoc(const char* binary_path) {
    ...
    return false;
#else
    // Non-macOS platforms don't need code signing; silence unused-parameter.
    (void)binary_path;
    return true;
#endif
}
  ...
  if (is_sip_protected_path(executable)) {
    ...
    if (strcmp(section_name, MACHO_SECTION_NODE_SEA_BLOB) == 0) {
      ...
      for (LIEF::MachO::Section& section : binary->sections()) {
        ...
        // Search in span first (avoid copy if no match)
        for (size_t i = 0; i + fuse_length <= content_span.size(); i++) {
          if (memcmp(content_span.data() + i, fuse_string, fuse_length) == 0) {
            // Only copy when match found
...
extern "C" int binject_macho_lief_batch(
    ...
    if (sea_data && sea_size > 0 && !segment_exists) {
      ...
      for (LIEF::MachO::Section& section : binary->sections()) {
        ...
        // Search in span first (avoid copy if no match)
        for (size_t i = 0; i + fuse_length <= content_span.size(); i++) {
          if (memcmp(content_span.data() + i, fuse_unflipped, fuse_length) == 0) {
            // Only copy when match found
            std::vector<uint8_t> content(content_span.begin(), content_span.end());
    ...
    // Add VFS config section if provided (VFS_CFG_SIZE bytes SVFG format)
    // This section stores the VFS config (108 bytes) with mode and prefix fields.
    if (vfs_config_data) {
      printf("Creating VFS config section %s with %d bytes...\n", MACHO_SECTION_SMOL_VFS_CONFIG, VFS_CFG_SIZE);

      LIEF::MachO::Section vfs_config_section(MACHO_SECTION_SMOL_VFS_CONFIG);
      std::vector<uint8_t> vfs_config_content(vfs_config_data, vfs_config_data + VFS_CFG_SIZE);
 ...
 * @param extracted_source_path Path to extracted binary with NODE_SEA sections (NULL if none).
 * @return BINJECT_OK on success, error code otherwise.
 */
extern "C" int binject_macho_repack_smol_lief(
    ...
    int result = smol_repack_lief(stub_path, section_data, section_size, output_path, extracted_source_path);
    if (result != 0) {
        return BINJECT_ERROR;
    }
    return BINJECT_OK;
