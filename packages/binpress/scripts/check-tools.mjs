...
if (IS_MACOS) {
  ...
  // XZ Utils for liblzma (though macOS uses LZFSE for compression)
  autoInstallableTools.push('xz')
} else if (IS_LINUX) {
  // On Linux, gcc is standard
  autoInstallableTools.push('gcc')
  // XZ Utils for liblzma (LZMA compression on Linux)
...
async function main() {
  ...
  console.log('Checking required build tools for binpress...\n')
