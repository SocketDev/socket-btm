...
// node:ilp - High-performance ILP (InfluxDB Line Protocol) client
// Used for time-series data ingestion to QuestDB, InfluxDB, and compatible databases.
...
const { validateObject, validateString, validateNumber } = require('internal/validators');
...
// Native binding for ILP operations.
...
class Sender extends EventEmitter {
  ...
  constructor(options) {
    ...
    validateNumber(config.maxBufferSize, 'options.maxBufferSize', config.bufferSize);
  ...
  intColumn(name, value) {
    ...
    if (type === 'number' && !NumberIsFinite(value)) {
      throw new ERR_INVALID_ARG_VALUE('value', value, 'must be a finite number or bigint');
    }
  ...
  timestampColumn(name, value, unit = TimeUnit.Nanoseconds) {
    ...
    if (unit < 0 || unit > 3) {
      throw new ERR_INVALID_ARG_VALUE('unit', unit, 'must be 0-3 (TimeUnit.Nanoseconds to TimeUnit.Seconds)');
    }
  ...
  field(name, value) {
    ...
    throw new ERR_INVALID_ARG_TYPE('value', ['string', 'boolean', 'number', 'bigint'], value);
  }
  ...
  at(timestamp, unit = TimeUnit.Nanoseconds) {
    ...
    if (typeof timestamp !== 'number' && typeof timestamp !== 'bigint') {
      throw new ERR_INVALID_ARG_TYPE('timestamp', ['number', 'bigint'], timestamp);
    }
    if (unit < 0 || unit > 3) {
      throw new ERR_INVALID_ARG_VALUE('unit', unit, 'must be 0-3 (TimeUnit.Nanoseconds to TimeUnit.Seconds)');
    }
    ...
    if (StringPrototypeIncludes(connectionString, '://')) {
      ...
      try {
        ...
        URLSearchParamsPrototypeForEach(url.searchParams, (value, key) => {
          if (key === 'bufferSize' || key === 'maxBufferSize' ||
              key === 'connectTimeout' || key === 'sendTimeout' ||
              key === 'autoFlushRows' || key === 'autoFlushInterval') {
      ...
      if (colonIndex > 0) {
        ...
        if (!NumberIsFinite(parsedPort) || parsedPort < 1 || parsedPort > 65535) {
          throw new ILPError(
  ...
  #checkBufferPressure() {
    ...
    if (ratio >= BUFFER_PRESSURE_CRITICAL && this[kPressureEmitted] < 2) {
      ...
      this.emit('bufferCritical', this.#createBufferPressureInfo(used, max, ratio));
    } else if (ratio >= BUFFER_PRESSURE_HIGH && this[kPressureEmitted] < 1) {
      this[kPressureEmitted] = 1;
      this.emit('bufferPressure', this.#createBufferPressureInfo(used, max, ratio));
...
class BulkRowBuilder {
  ...
  constructor(sender, table, options) {
    ...
    if (!NumberIsInteger(this.#batchSize) || this.#batchSize < 1) {
      throw new ERR_INVALID_ARG_VALUE('options.batchSize', this.#batchSize, 'must be a positive integer');
    }
  }
