 ...
 *
 * Returns the most recent release tag matching the tool prefix by checking
 * recent releases. Since we publish tools with tags like `{tool}-{date}-{commit}`,
 * we search for the first matching tag.
 *
...
export async function getLatestRelease(tool, { quiet = false } = {}) {
  return await pRetry(
    async () => {
      // Fetch recent releases (10 should be more than enough).
      ...
      if (!quiet) {
        logger.info(`  No ${tool} release found in recent releases`)
      }
      return null
    },
    {
      ...
      onRetry: (attempt, error) => {
        if (!quiet) {
          logger.info(
            `  Retry attempt ${attempt + 1}/${RETRY_CONFIG.retries + 1} for ${tool} release...`,
          )
          logger.warn(
