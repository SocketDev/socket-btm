// ============================================================================
// stub_selector.c — Stub selection and temp file management implementation
// ============================================================================
//
// WHAT THIS FILE DOES
// Detects the input binary's format and architecture by reading magic bytes
// and headers, then matches it to one of the 8 embedded decompressor stubs.
// Also handles writing the selected stub to a temp file and cleaning it up.
//
// WHY IT EXISTS
// This is the implementation of the stub selection logic. It reads the
// first few bytes of an executable to determine if it is Mach-O, ELF, or
// PE, and whether it is x64 or arm64. Then it returns the matching
// pre-compiled stub so binpress can embed compressed data into it.
// ============================================================================
...
/**
 * Detect architecture from Mach-O binary using pre-read buffer.
 * Buffer must contain at least 12 bytes.
 */
static const char *detect_macho_architecture_from_buffer(const uint8_t *buffer, size_t size) {
  if (size < 12) {
    return NULL;
  }
  ...
  if (magic == 0xcafebabe || magic == 0xbebafeca) {
    // Fat binary - cputype is at offset 8 (after magic + nfat_arch)
    uint32_t cputype = buffer[8] | (buffer[9] << 8) | (buffer[10] << 16) | (buffer[11] << 24);
  ...
  if (magic == 0xfeedfacf || magic == 0xcffaedfe) {
    ...
    uint32_t cputype = buffer[4] | (buffer[5] << 8) | (buffer[6] << 16) | (buffer[7] << 24);
...
/**
 * Detect architecture from ELF binary using pre-read buffer.
 * Buffer must contain at least 20 bytes.
 */
static const char *detect_elf_architecture_from_buffer(const uint8_t *buffer, size_t size) {
  if (size < 20) {
    return NULL;
  }

  // e_machine is at offset 18 (uint16_t)
  uint16_t e_machine = buffer[18] | (buffer[19] << 8);
...
/**
 * Detect if ELF binary uses musl libc using pre-read header.
 * Requires elf_header (64 bytes) already read, and file handle positioned after header.
 * This saves one file open compared to the standalone version.
 *
 * @param input_path Path to binary (for seeking to program headers)
 * @param elf_header Pre-read 64-byte ELF header
 * @return 1 if musl, 0 if glibc or unknown
 */
static int is_musl_elf_from_header(const char *input_path, const uint8_t *elf_header) {
  // Check if 64-bit ELF
  ...
  // Need to open file for seeking to program headers and interpreter string
...
/**
 * Detect architecture from PE binary using pre-read buffer.
 * Buffer should contain at least 512 bytes for most PE files.
 * Falls back to file seek if PE header is beyond buffer.
 */
static const char *detect_pe_architecture_from_buffer(const uint8_t *buffer, size_t size,
                                                       const char *input_path) {
  // PE offset is at 0x3C (60)
  if (size < 64) {
    return NULL;
  }

  uint32_t pe_offset = buffer[0x3C] | (buffer[0x3D] << 8) |
                       (buffer[0x3E] << 16) | (buffer[0x3F] << 24);

  // Machine field is at pe_offset + 4 (skip PE signature)
  size_t machine_offset = pe_offset + 4;

  // Check if machine field is within buffer
  if (machine_offset + 2 <= size) {
    // Can read from buffer - no file I/O needed
    uint16_t machine = buffer[machine_offset] | (buffer[machine_offset + 1] << 8);
  ...
  // PE header beyond buffer, need to seek (rare case)
  FILE *f = fopen(input_path, "rb");
  if (!f) {
    return NULL;
  }

  if (fseek(f, machine_offset, SEEK_SET) != 0) {
    fclose(f);
    return NULL;
  }
...
const embedded_stub_t *select_stub_for_binary(const char *input_path) {
  // Read 512 bytes for format + architecture detection in a single read.
  // This covers: magic (4), Mach-O header (12), ELF header (64), PE header (varies).
  // Previously this function opened the file 2-3 times; now it's 1 open for
  // Mach-O/PE, and 2 opens for ELF (musl detection needs to seek to interpreter).
  FILE *f = fopen(input_path, "rb");
  if (!f) {
  ...
  if (bytes_read < 4) {
    fprintf(stderr, "Error: Binary too small or cannot read: %s\n", input_path);
    return NULL;
  }

  // Detect binary format from first 4 bytes
  binary_format_t format = detect_binary_format(buffer);

  switch (format) {
  case BINARY_FORMAT_MACHO: {
    // Detect macOS architecture from buffer (no additional file open)
    const char *arch = detect_macho_architecture_from_buffer(buffer, bytes_read);
    if (!arch) {
  ...
  case BINARY_FORMAT_ELF: {
    // Detect Linux architecture from buffer (no additional file open)
    const char *arch = detect_elf_architecture_from_buffer(buffer, bytes_read);
    if (!arch) {
      fprintf(stderr, "Error: Cannot detect ELF binary architecture\n");
      return NULL;
    }

    // Detect musl vs glibc using pre-read ELF header (saves 1 file open)
    // Still needs 1 file open to seek to interpreter string
    int use_musl = is_musl_elf_from_header(input_path, buffer);
    DEBUG_LOG("is_musl_elf_from_header() returned: %d for %s\n", use_musl, input_path);
  ...
  case BINARY_FORMAT_PE: {
    // Detect Windows architecture from buffer (usually no additional file open)
    const char *arch = detect_pe_architecture_from_buffer(buffer, bytes_read, input_path);
    if (!arch) {
...
const embedded_stub_t *select_stub_with_target(
  ...
  if (!final_platform || !final_arch) {
    // Need to detect format/arch from binary - read 512-byte buffer once
    FILE *f = fopen(input_path, "rb");
    if (!f) {
      fprintf(stderr, "Error: Cannot open binary: %s (%s)\n", input_path, strerror(errno));
      return NULL;
    ...
    if (!final_arch) {
      switch (format) {
        case BINARY_FORMAT_MACHO:
          final_arch = detect_macho_architecture_from_buffer(buffer, bytes_read);
          break;
        case BINARY_FORMAT_ELF:
          final_arch = detect_elf_architecture_from_buffer(buffer, bytes_read);
          break;
        case BINARY_FORMAT_PE:
          final_arch = detect_pe_architecture_from_buffer(buffer, bytes_read, input_path);
          break;
        default: break;
      }
      if (!final_arch) {
