/**
 * node:smol-vfs - High-Performance Virtual Filesystem for SEA
 *
 * A read-only embedded filesystem for Single Executable Applications (SEA).
 * Files are stored in a TAR archive embedded in the binary.
 *
 * Performance features:
 * - **Memory-mapped access**: Direct buffer access without copying
 * - **Lazy extraction**: Files extracted on-demand, not at startup
 * - **Symlink resolution**: Cached symlink targets (max depth: 32)
 * - **fs-compatible API**: Drop-in replacement for common fs operations
 * - **Native addon support**: Seamless .node file loading from VFS
 *
 * Extraction modes:
 * - `MODE_IN_MEMORY`: Keep extracted files in memory (fastest, uses RAM)
 * - `MODE_ON_DISK`: Extract to temp dir (persistent across reads)
 * - `MODE_COMPAT`: On-demand extraction (balanced)
 *
 * @example
 * ```ts
 * import * as vfs from 'node:smol-vfs';
 *
 * if (vfs.hasVFS()) {
 *   // Read embedded file (zero-copy if possible)
 *   const config = vfs.readFileAsJSON('/snapshot/config.json');
 *
 *   // Stream large files
 *   const stream = vfs.createReadStream('/snapshot/data.csv');
 * }
 * ```
 *
 * @module
