 ...
 *   2. Decompresses inline using liblzma
...
// Debug mode: set SOCKET_SMOL_DEBUG=1 to enable verbose output
static int debug_enabled = 0;
#define DEBUG_LOG(...) do { if (debug_enabled) { fprintf(stderr, "[smol] " __VA_ARGS__); fflush(stderr); } } while(0)
...
static int decompress_lzma(const unsigned char *compressed_data, size_t compressed_size,
    ...
    // Initialize decoder for .xz format (matches lzma_easy_encoder)
    ...
    if (strm.total_out != uncompressed_size) {
        fprintf(stderr, "Error: Decompressed size mismatch (got %zu, expected %zu)\n",
                strm.total_out, uncompressed_size);
        return -1;
    }
...
static int extract_and_execute(int self_fd, const char *exe_path, int argc, char *argv[], char *envp[]) {
    ...
    DEBUG_LOG("Found marker at offset %ld\n", data_offset);
    ...
    // Decompress
    DEBUG_LOG("Starting LZMA decompression...\n");
    if (decompress_lzma(compressed_data, compressed_size, decompressed_data, uncompressed_size) != 0) {
...
int main(int argc, char *argv[], char *envp[]) {
    ...
    // Enable debug mode if SOCKET_SMOL_DEBUG environment variable is set
    const char *debug_env = getenv("SOCKET_SMOL_DEBUG");
    debug_enabled = (debug_env != NULL && debug_env[0] != '\0' && debug_env[0] != '0');
