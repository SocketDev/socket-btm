...
async function extractVFS() {
  ...
  // Import helpers for VFS entry access
  const { getContent, VFS_DIRECTORY_MARKER } = require('internal/socketsecurity/vfs/tar_parser')

  // Extract all files
  let extracted = 0
  for (const [filepath, entry] of vfs) {
    const content = getContent(entry)

    // Skip directories
    if (content === VFS_DIRECTORY_MARKER) continue
...
if (process.smol && process.smol.mount) {
  try {
    ...
    const dir = process.smol.mount('/snapshot/bin')
    console.log('DIR=' + dir)
