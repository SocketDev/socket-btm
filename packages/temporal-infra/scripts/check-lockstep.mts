...
const STUB_PATTERNS = [
  ...
  // \brequires\b: word boundary; (?:a |an )?: optional article (non-capturing) before "calendar"
...
export function extractV8Calls(): Array<{
  ...
  // temporal_rs::([A-Z][A-Za-z0-9]*): Rust type path with PascalCase class capture;
  // ::([a-z_][a-z_0-9]*)\s*\(: snake_case method name followed by open paren
  const callPattern =
