...
static char* generate_sea_blob_from_config(const char *config_path, const char *executable) {
    ...
    /* Parse SEA config once and cache results for reuse below */
    ...
    if (target_version && system_version) {
    ...
    } else if (!target_version && system_version) {
        ...
        if (config_opts.has_code_cache || config_opts.has_snapshot) {
            fprintf(stderr, "\n");
            fprintf(stderr, "⚠️  Cannot generate code cache for cross-platform target\n");
            fprintf(stderr, "   Host Node.js: %s (%s-%s)\n", system_version, HOST_PLATFORM, HOST_ARCH);
            fprintf(stderr, "   Target binary: %s (cannot execute on %s)\n", target_platform, HOST_OS_NAME);
            fprintf(stderr, "\n");
            fprintf(stderr, "   V8 bytecode/snapshots are platform-specific.\n");
            fprintf(stderr, "   Code cache generated on %s won't work on the target platform.\n", HOST_OS_NAME);
            fprintf(stderr, "\n");
            fprintf(stderr, "   Generating plain JS blob (no code cache)...\n");
            fprintf(stderr, "\n");
            // The blob will be generated without code cache - plain JS is portable
        } else {
            // Safe: plain JS blob is platform-agnostic
...
static void print_usage(const char *program) {
    ...
    printf("  %s node-version <executable>\n", program);
    printf("  %s --help\n", program);
    printf("  %s --version\n\n", program);
    printf("Commands:\n");
    ...
    printf("  node-version      Extract Node.js version from binary (stub, node-smol, or plain Node.js)\n\n");
    printf("Options:\n");
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(command, "node-version") == 0) {
        if (argc < 3) {
            fprintf(stderr, "Error: node-version requires an executable path\n");
            return BINJECT_ERROR_INVALID_ARGS;
        }

        const char *executable = argv[2];

        char *version = smol_extract_node_version(executable);
        if (version) {
            printf("%s\n", version);
            free(version);
            return BINJECT_OK;
        } else {
            fprintf(stderr, "Error: Could not extract Node.js version from %s\n", executable);
            return BINJECT_ERROR;
        }
    }
