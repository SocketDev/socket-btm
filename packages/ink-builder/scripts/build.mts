...
async function main() {
  ...
  for (const file of YOGA_IMPORT_FILES) {
    ...
    // Files are in build/, yoga-sync will be in build/yoga-sync.mts.
    content = content.replace(
      /import Yoga from ['"]yoga-layout['"];?/g,
      "import Yoga from './yoga-sync.mts';",
    )
  ...
  if (!existsSync(yogaSyncSource)) {
    throw new Error(
      `yoga-sync.mts not found. Run yoga-layout-builder build first.`,
    )
  }
  logger.log(`Using yoga-sync from: ${yogaSyncSource}`)

  await fs.copyFile(yogaSyncSource, yogaSyncDest)
  logger.success('Bundled yoga-sync.mts')
  ...
  logger.log('Includes: bundled yoga-sync.mts (no yoga-layout dependency)')
}
