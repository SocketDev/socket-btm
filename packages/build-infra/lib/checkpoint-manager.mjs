      ...
      if (isDir) {
        try {
          ...
          if (readErr.code === 'EACCES' || readErr.code === 'EPERM') {
            throw new Error(
              `Cannot read directory: ${artifactPath} (${readErr.code}). ` +
    ...
    try {
      ...
      while (extractAttempts < MAX_EXTRACT_ATTEMPTS) {
        try {
          ...
          // This prevents TOCTOU race where checkpoint is replaced after mtime check
          // eslint-disable-next-line no-await-in-loop
          const extractedHash = await computeSourceHash([targetPath])
          if (
            checkpointData.artifactHash &&
            extractedHash !== checkpointData.artifactHash
          ) {
            throw new Error(
