...
int path_join(char *result, const char *base_path, const char *component) {
    ...
    /* Check for overflow with safe comparison to avoid integer overflow */
    if (base_len > PATH_MAX - 2 || comp_len > PATH_MAX - base_len - 2) {
        return -1;
    }

    /* Copy base path using memcpy for explicit length control */
    ...
    size_t comp_start_len = strlen(comp_start);

    /* Join with separator using memcpy for explicit length control */
    if (base_len > 0 && comp_start[0] != '\0') {
        result[base_len] = sep;
        memcpy(result + base_len + 1, comp_start, comp_start_len + 1);
    } else if (comp_start[0] != '\0') {
        memcpy(result + base_len, comp_start, comp_start_len + 1);
    }
