...
function nativeWriteBinary(socket, statusCode, buffer, type) {
  return smolHttp().writeBinaryResponse(socket, statusCode, buffer, type)
}
function nativeWriteNotModified(socket) {
  return smolHttp().writeNotModifiedResponse(socket)
}

// Lazy: json_cache does work at module scope that we don't want during bootstrap.
let _jsonCache
function getJsonCache() {
  return (
    _jsonCache ||
    (_jsonCache = require('internal/socketsecurity/http/json_cache'))
  )
}
function createCacheKey(...args) {
  return getJsonCache().createCacheKey(...args)
}
function stringifyWithCache(...args) {
  return getJsonCache().stringifyWithCache(...args)
}
...
function fastJsonResponse(response, statusCode, data, cacheKey) {
  ...
  const json =
    typeof data === 'string' ? data : stringifyWithCache(data, cacheKey)
  ...
  if (!success) {
    ...
    response.setHeader('Content-Length', BufferByteLength(json))
    response.end(json)
  }
...
function fastErrorResponse(response, statusCode, message) {
  const error = message
    ? { __proto__: null, error: getErrorName(statusCode), message }
    : { __proto__: null, error: getErrorName(statusCode) }
