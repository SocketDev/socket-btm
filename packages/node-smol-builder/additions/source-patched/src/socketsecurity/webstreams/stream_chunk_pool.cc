...
Local<Object> StreamChunkPool::AcquireChunk() {
  ...
  // EscapableHandleScope — a plain HandleScope would reclaim the
  // returned Local's handle slot when this function returns, leaving
  // the caller with a dangling Local. Per V8 contract, any Local
  // crossing a HandleScope boundary must be passed through Escape.
  ...
  if (SMOL_LIKELY(!pool_.empty())) {
    ...
    pool_.pop_back();
    return scope.Escape(global_chunk.Get(isolate));
  }

  // Pool empty: create new ordinary object (prototype = %Object.prototype%).
  // WebIDL dictionary conversion of ReadableStreamReadResult yields an
  // ordinary object — WPT's assert_iter_result checks
  // Object.getPrototypeOf(result) === Object.prototype, and a null-prototype
  // object also drops V8 into dictionary-mode properties.
