...
function createServer(opts) {
  ...
  const uwsServer = binding.createUwsServer(maxBodySize)

  // Register route handlers. A bare-function entry (RouteHandler) catches
  // every method for its pattern ('ANY'); a MethodHandlers object registers
  // each of its keys ('*' included) under that specific method — a route
  // that only wants GET/QUERY, say, no longer silently gets 'GET' for both.
  let nextHandlerId = 0
  if (routeHandlers) {
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (typeof value === 'function') {
        ...
        binding.uwsServerAddRoute(uwsServer, 'ANY', pattern, id, value)
        continue
      }
      const methodEntries = ObjectEntries(value)
      for (let j = 0; j < methodEntries.length; j++) {
        const [method, handler] = methodEntries[j]
        const id = nextHandlerId++
        binding.uwsServerAddRoute(
          uwsServer,
          method === '*' ? 'ANY' : method,
          pattern,
          id,
...
function serve(options) {
  ...
  // `process.argv[1]` is empty in SEA and `node -e` contexts — without it
  // we cannot fork the user's script. Fall back to single-process so the
  // caller gets a working server instead of a crash.
  if (!process.argv[1]) {
    return createServer(opts)
  }
  ...
  return {
    ...
    stop() {
      ...
      return new Promise(resolve => {
        ...
        // Track pending children in a Set so `done()` for a given child
        // fires AT MOST ONCE even if both the `exit` event AND the
        // already-exited post-check race to resolve it. The R34 shape
        // (shared counter + bare decrement) would double-count fast-
        // exiting workers and resolve stop() while peers are still
        // alive.
        const pending = new Set(workerProcesses)
        const done = child => {
          if (pending.delete(child) && pending.size === 0) {
            resolve()
          }
        }
        for (let i = 0; i < workerProcesses.length; i++) {
          const child = workerProcesses[i]
          child.once('exit', () => done(child))
