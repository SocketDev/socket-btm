...
globalThis.primordials = {
  ...
  // `Error.isError` is ES2025 (Node 22.18+); the real primordials expose it on
  // a Node 26 smol build. Fall back to an instanceof approximation when the
  // test host predates it, so smol code reading `primordials.ErrorIsError`
  // resolves either way.
  ErrorIsError:
