...
`.node` binaries for downstream consumption. Downstream Go-backed
builders add `napi-go-infra` as a workspace dependency, import the Go
package and the C shim, and produce their own per-platform-arch `.node`
artifacts.
