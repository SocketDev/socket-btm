...
import { MAX_NODE_BINARY_SIZE } from 'build-infra/lib/constants'
  ...
  () => {
    describe('directory structure', () => {
      ...
      it('should have dev/int8 build directory', () => {
        ...
        expect(existsSync(devInt8Dir)).toBeTruthy()
      })
    ...
    describe('int8 quantization output (dev)', () => {
      ...
      it('should generate encoder.onnx model file', () => {
        expect(existsSync(encoderPath)).toBeTruthy()
      })

      it('should generate decoder.onnx model file', () => {
        expect(existsSync(decoderPath)).toBeTruthy()
      })

      it('should generate tokenizer.json', () => {
        expect(existsSync(tokenizerPath)).toBeTruthy()
      })
      ...
      it('encoder should be reasonably sized', async () => {
        ...
        // > 20MB
        expect(stats.size).toBeLessThan(MAX_NODE_BINARY_SIZE)
        // Must fit within binject's Node.js binary size limit
      })
      ...
      it('tokenizer.json should be valid JSON', async () => {
        ...
        const content = JSON.parse(await fs.readFile(tokenizerPath, 'utf8'))
    ...
    describe('int4 quantization output (prod)', () => {
      ...
      it('should generate encoder for int4', () => {
        ...
        expect(existsSync(int4EncoderPath)).toBeTruthy()
      })
      ...
      it('should generate decoder for int4', () => {
        ...
        expect(existsSync(int4DecoderPath)).toBeTruthy()
      })
    ...
    describe('package configuration', () => {
      ...
      it('should have external-tools.json for Python dependencies', async () => {
        ...
        expect(existsSync(externalToolsPath)).toBeTruthy()
    ...
    describe('shared models cache', () => {
      it('should have models directory for shared cache', () => {
        ...
        expect(existsSync(modelsDir)).toBeTruthy()
      })
      ...
      it('models cache should have tokenizer.json', async () => {
        ...
        expect(existsSync(tokenizerFile)).toBeTruthy()
        const content = JSON.parse(await fs.readFile(tokenizerFile, 'utf8'))
        expect(content).toHaveProperty('model')
      })
