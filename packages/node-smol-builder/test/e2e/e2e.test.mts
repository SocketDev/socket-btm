...
// Check if VFS is available via process.smol
const hasVFS = !!(process.smol && process.smol.hasVFS);
console.log('VFS available:', hasVFS);

if (hasVFS) {
  const fs = require('fs');
  const entries = fs.readdirSync('/snapshot');
  console.log('VFS files:', entries.length);

  // Try to read package.json from VFS
  try {
    const pkg = JSON.parse(fs.readFileSync('/snapshot/package.json', 'utf8'));
  ...
  describe('sEA application build', () => {
    it(
      ...
      async () => {
        ...
        // VFS functionality is tested separately in vfs.test.mts
        const appJs = path.join(testDir, 'app.js')
        await fs.writeFile(
          appJs,
          `
...
`,
        ...
        logger.log('SEA blob will be generated by binject…')
        ...
        const vfsTarGz = path.join(testDir, 'vfs.tar.gz')
        logger.log('Creating VFS archive…')
        const tarResult = await spawn(
          'tar',
          ['czf', vfsTarGz, '-C', vfsDir, '.'],
          { cwd: testDir },
        ...
        expect(tarResult.code).toBe(0)
        expect(existsSync(vfsTarGz)).toBeTruthy()
        ...
        logger.log('Executing application…')
        const execResult = await spawn(appBinary, [], {
          cwd: testDir,
          stdio: ['ignore', 'pipe', 'pipe'],
          timeout: 10_000,
        ...
        expect(execResult.code).toBe(0)
        expect(execResult.stdout).toContain('=== SEA Application ===')
        expect(execResult.stdout).toContain('Running as SEA: true')
        expect(execResult.stdout).toContain('SEA execution successful')
      },
    )
  })
  ...
  describe('binary compression integration', () => {
    it('should work with compressed binary if available', async () => {
      ...
      if (!existsSync(compressedBinary)) {
        logger.log('Skipping: No compressed binary found')
        return
      }
