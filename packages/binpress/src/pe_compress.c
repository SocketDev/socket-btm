...
static int parse_args(int argc, char *argv[], compress_config *config) {
    if (argc >= 2) {
        ...
        if (strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0) {
            printf("Usage: %s <input> <output> [--quality=lzms|xpress|xpress_huff]\n", argv[0]);
            printf("\nCompress PE binaries using Windows Compression API.\n");
            printf("\nArguments:\n");
            printf("  input   Path to input binary\n");
            printf("  output  Path to output compressed binary\n");
            printf("\nOptions:\n");
            printf("  --quality=ALGO  Compression algorithm (lzms, xpress, xpress_huff)\n");
            printf("                  Default: lzms\n");
