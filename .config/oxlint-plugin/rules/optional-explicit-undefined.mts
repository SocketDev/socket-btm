/**
 * @file Enforce `foo?: T | undefined` over `foo?: T` on interface /
 *   type-literal properties. Pairs with `exactOptionalPropertyTypes: true` (set
 *   in tsconfig.base.json) so the value `undefined` is a separately-modeled
 *   state from "property omitted." With both, you can write either form at the
 *   call site; in mixed-codebase code, both happen, so we require both to be
 *   allowed. Applies to `.ts`, `.cts`, `.mts` files. JS (`.js`, `.cjs`, `.mjs`)
 *   has no type annotations to enforce. Triggers on:
 *
 ...
 *     true`, a call site that writes `{ foo: undefined }` is rejected when the
