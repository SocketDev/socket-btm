...
Extract pristine Node.js source from git submodule.

## Flow

```
submodule/ → build/shared/source/ → build/shared/checkpoints/source-cloned.tar
                    ↓
         ┌──────────┴──────────┐
         ↓                     ↓
...
```javascript
{ nodeVersion, nodeSHA, submodulePath }
```
