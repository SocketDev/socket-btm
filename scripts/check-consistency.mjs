...
async function discoverPackages() {
  const entries = await fs.promises.readdir(PACKAGES_DIR, {
    withFileTypes: true,
  })
  const packages = []
...
async function checkRequiredFiles(packages) {
  ...
  for (const pkg of packages) {
    ...
    if (fs.existsSync(path.join(pkg.path, 'README.md'))) {
      const readme = await fs.promises.readFile(
        path.join(pkg.path, 'README.md'),
        'utf-8',
      )
...
async function checkVitestConfig(packages) {
  ...
  const vitestPackages = packages.filter(pkg =>
    fs.existsSync(path.join(pkg.path, 'vitest.config.mts')),
  )
...
`
          await fs.promises.writeFile(configPath, standardConfig, 'utf-8')
...
async function checkExternalTools(packages) {
  ...
  for (const pkgName of C_PACKAGES) {
    ...
    if (!fs.existsSync(externalToolsPath)) {
      reportIssue(
        'warning',
        'external-tools',
    ...
    try {
      const tools = JSON.parse(
        await fs.promises.readFile(externalToolsPath, 'utf-8'),
      )
...
async function checkBuildOutputStructure(packages) {
  ...
  for (const pkg of packages) {
    ...
    if (fs.existsSync(path.join(buildDir, 'dev'))) {
      if (!fs.existsSync(devFinalDir)) {
        reportIssue(
          'warning',
          'build-output',
    ...
    if (fs.existsSync(path.join(buildDir, 'prod'))) {
      if (!fs.existsSync(prodFinalDir)) {
        reportIssue(
          'warning',
          'build-output',
    ...
    if (!scripts || !scripts.clean) {
      reportIssue(
        ...
        async () => {
          ...
          const hasNodeGyp = fs.existsSync(path.join(pkg.path, 'binding.gyp'))
