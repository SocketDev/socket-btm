...
```bash
PATCH_NAME="001-common-gypi-lto.patch"
...
3. **Manual validation:**
   ```bash
   cd packages/node-smol-builder/upstream/node
   patch --verbose --dry-run < ../../patches/source-patched/001-common-gypi-lto.patch
   ```
...
2. **Store in temporary file:**
   ...
   ```bash
   # First bash call
   echo "001-common-gypi-lto.patch" > /tmp/current-patch.txt
...
3. **Use inline variables:**
   ...
   for PATCH_NAME in 001-common-gypi-lto.patch 002-polyfills.patch; do
