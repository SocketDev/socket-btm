// ============================================================================
// json_parser.h — sea-config.json parser API
// ============================================================================
//
// WHAT THIS FILE DOES
// Declares functions to parse sea-config.json (the Node.js SEA build
// configuration file) and extract fields like output path, main script,
// VFS settings, and self-update config.
//
// WHY IT EXISTS
// The binject CLI reads sea-config.json to know what resources to inject
// and where. This parser uses the cJSON library (a lightweight C JSON
// parser) to safely extract the needed fields with proper error handling.
// ============================================================================
...
/**
 * Parsed VFS configuration from smol.vfs section (build-time).
 */
typedef struct {
...
typedef struct {
    ...
    vfs_build_config_t *vfs;      // parsed VFS config (optional, owned).
} sea_config_t;
 ...
 * String fields are strdup'd; caller must call smol_config_free() when done.
 *
