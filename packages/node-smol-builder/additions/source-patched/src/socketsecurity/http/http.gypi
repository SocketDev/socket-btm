...
#
# simdutf (vendored at deps/v8/third_party/simdutf) is intentionally NOT used here:
#
# 1. uWebSockets has a UWS_USE_SIMDUTF gate in WebSocketProtocol.h but its
#    custom Kuhn/Hultman fallback is ~40% faster on ASCII-dominant WebSocket
#    traffic — the 16-byte uint64 fast-path (bits & 0x8080...) exits before
#    simdutf's SIMD setup overhead pays off.
#
# 2. All bulk UTF-16→UTF-8 transcoding in the additions goes through V8's
#    WriteUtf8V2 / NewFromUtf8 APIs, which already use simdutf internally
#    since Node 18.15. There is no bypass path that would benefit from a
#    direct simdutf call.
#
# If a future addition does bulk UTF transcoding outside V8 (e.g. a file
# reader or protocol codec that materializes UTF-8 directly), add an
# include_dir for deps/v8/third_party/simdutf and call
# simdutf::convert_utf16le_to_utf8 / simdutf::validate_utf8 there.
