...
int binject_is_compressed_stub(const char *executable) {
    ...
    if (found) {
        ...
        size_t cache_key_offset = marker_offset + MAGIC_MARKER_LEN + 8 + 8;
        size_t required_size = cache_key_offset + 16;
...
int binject_extract_stub_to_cache(const char *compressed_stub, const char *extracted_path) {
    ...
    /* Read metadata: [marker][8-byte: compressed_size][8-byte: uncompressed_size][16-byte: cache_key]. */
    size_t metadata_offset = marker_offset + MAGIC_MARKER_LEN;
    if (metadata_offset + 8 + 8 > read_size) {
    ...
    memcpy(&compressed_size, buffer + metadata_offset, 8);
    memcpy(&uncompressed_size, buffer + metadata_offset + 8, 8);

    /* Compressed data starts after: marker + compressed_size + uncompressed_size + cache_key. */
    size_t data_offset = marker_offset + MAGIC_MARKER_LEN + 8 + 8 + 16;
    ...
    /* Decompress using cross-platform decompressor. */
    ...
    char *last_slash = strrchr(extracted_path, '/');
    if (!last_slash) {
#ifdef _WIN32
        last_slash = strrchr(extracted_path, '\\');
#endif
    }

    if (last_slash) {
        char cache_dir[PATH_MAX];
        size_t dir_len = last_slash - extracted_path;
        if (dir_len >= sizeof(cache_dir)) {
            fprintf(stderr, "Error: Cache directory path too long\n");
            goto cleanup;
        }
        memcpy(cache_dir, extracted_path, dir_len);
        cache_dir[dir_len] = '\0';
...
#ifdef _WIN32
            if (_mkdir(cache_dir) != 0 && errno != EEXIST) {
#else
