...
int macho_compress_lief(const char* input_path,
  ...
  if (!input_path || !output_path) {
    ...
    std::unique_ptr<LIEF::MachO::FatBinary> input_fat = LIEF::MachO::Parser::parse(input_path);
    if (input_fat && input_fat->size() > 0) {
      LIEF::MachO::Binary* input_binary = input_fat->at(0);
      if (input_binary && input_binary->has_segment(MACHO_SEGMENT_SMOL)) {
        is_repack = true;
        printf("  Detected existing SMOL segment - automatic repacking mode\n");
    ...
    if (compress_lief_common(compress_input, algorithm, &context, target, target_platform, target_arch, target_libc) != 0) {
      if (is_repack) {
