...
import { MONOREPO_ROOT, PACKAGE_ROOT } from '../../scripts/paths.mts'
...
// Node.js source uses node_version.h, not a plain NODE_VERSION file; the monorepo
// root .node-version pin is the authoritative version that was built.
const MONOREPO_NODE_VERSION_FILE = path.join(MONOREPO_ROOT, '.node-version')
export function getNodeVersionRaw() {
  if (existsSync(MONOREPO_NODE_VERSION_FILE)) {
    return readFileSync(MONOREPO_NODE_VERSION_FILE, 'utf8').trim()
  }
...
describe.skipIf(skipTests)('final binary extraction to ~/.socket/_dlx/', () => {
  ...
  describe('cache directory structure', () => {
    ...
    it('should use zstd compression (verified via successful decompression)', async () => {
      ...
      // If we got here and the binary exists, zstd decompression worked.
      expect(existsSync(cachedBinaryPath)).toBeTruthy()
    })
    ...
    it('should have correct byte offset for compressed data', async () => {
      ...
      // Verify TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG is exactly 132 bytes
      // (marker 32 + compressed_size 8 + uncompressed_size 8 + cache_key 16
      // + platform_metadata 3 + integrity_hash 64 + smol_config_flag 1).
      expect(TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG).toBe(132)
    ...
    it('should validate metadata header size calculation', () => {
      // Metadata header = compressed_size(8) + uncompressed_size(8)
      // + cache_key(16) + platform_metadata(3) + integrity_hash(64)
      // + config_flag(1)
      ...
      expect(calculatedSize).toBe(100)
      expect(METADATA_HEADER_SIZE).toBe(100)
    })

    it('should validate total header size calculation', () => {
      // Total header = marker(32) + metadata(100)
      const calculatedTotal = HEADER_SIZES.MAGIC_MARKER + METADATA_HEADER_SIZE
      expect(calculatedTotal).toBe(132)
      expect(TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG).toBe(132)
    })
    ...
    it('should have proper byte alignment for compressed data', async () => {
      ...
      const platformMetadataOffset = cacheKeyOffset + HEADER_SIZES.CACHE_KEY
      const integrityHashOffset =
        platformMetadataOffset + HEADER_SIZES.PLATFORM_METADATA
      const smolConfigFlagOffset =
        integrityHashOffset + HEADER_SIZES.INTEGRITY_HASH
      const dataOffset = smolConfigFlagOffset + HEADER_SIZES.SMOL_CONFIG_FLAG
      ...
      expect(platformMetadataOffset).toBe(markerIndex + 64)
      expect(integrityHashOffset).toBe(markerIndex + 67)
      expect(smolConfigFlagOffset).toBe(markerIndex + 131)
      expect(dataOffset).toBe(markerIndex + 132)
      ...
      expect(actualMetadataHeaderSize).toBe(100)
      ...
      expect(actualTotalHeaderSize).toBe(132)
    })
    ...
    it('should have sequential byte layout without gaps', async () => {
      ...
      // Integrity hash (64 bytes, SHA-512 of compressed data). Only shape
      // can be asserted here — the hash value is data-dependent. Ensure
      // the bytes exist and at least one is non-zero (an all-zero hash
      // would indicate a writer bug rather than a real SHA-512).
      const integrityHash = binaryData.subarray(offset, offset + 64)
      expect(integrityHash.length).toBe(64)
      expect(integrityHash.some(b => b !== 0)).toBeTruthy()
      offset += 64
