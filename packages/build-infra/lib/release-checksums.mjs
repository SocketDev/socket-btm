...
export async function getSocketBtmReleaseChecksums(options) {
  const { preferEmbedded = true, quiet = false, releaseTag, tempDir, tool } = options
 ...
 *
 * Lookup priority for expected checksum:
 * 1. Embedded checksums from release-checksums.json (fast, offline)
 * 2. Downloaded checksums.txt from GitHub releases (network)
 *
 * @param {object} options - Options.
...
export async function verifyReleaseChecksum(options) {
  ...
  // Try embedded checksum first (fast path, works offline).
  const embedded = getEmbeddedChecksum(tool, assetName)
  if (embedded) {
    const actual = await computeFileHash(filePath)
    if (actual !== embedded.checksum) {
      return { actual, expected: embedded.checksum, source: 'embedded', valid: false }
    }
    return { actual, expected: embedded.checksum, source: 'embedded', valid: true }
  }

  // Fall back to network fetch.
  const { checksums, source } = await getSocketBtmReleaseChecksums({
    quiet: true,
    releaseTag,
    tempDir,
    tool,
  ...
  const actual = await computeFileHash(filePath)
  if (actual !== expected) {
    return { actual, expected, source, valid: false }
  }

  return { actual, expected, source, valid: true }
}
