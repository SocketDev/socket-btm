...
// (?:#|\/\/): comment marker; published:\s*(\d{4}-\d{2}-\d{2}): published date capture;
// \s*\|\s*removable:\s*\d{4}-\d{2}-\d{2}: pipe-separated removable date
const ANNOTATION_RE =
...
export function checkSoak(
  published: unknown,
  now: Date = new Date(),
): { daysOld: number; removable: string | undefined; soaked: boolean } {
...
export function computeRemovableIso(published: unknown): string | undefined {
...
export function parseAnnotation(
  line: unknown,
): { published: string; removable: string | undefined } | undefined {
  ...
  return { published, removable: computeRemovableIso(published) }
}
