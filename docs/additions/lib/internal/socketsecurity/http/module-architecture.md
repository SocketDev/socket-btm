...
All files live flat under `internal/socketsecurity/http/` — there is no
`core/` subdirectory. `core.js` is a facade file that lazy-requires the
bootstrap-safe subset (`server`, `fast_response`, `response_writer`,
`header_cache`, `cork_manager`) as siblings.

```
node:smol-http (lib/smol-http.js)
  └── internal/socketsecurity/http/ (all modules are siblings)
        ├── core.js ──────────── facade: lazy-requires the bootstrap-safe subset below
        ├── server.js ─────────── uWS-backed serve() API (bootstrap-safe)
        ...
        ├── pools.js ─────────── connection / buffer pools
