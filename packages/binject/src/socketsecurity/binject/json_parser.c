...
int parse_smol_update_config(cJSON *smol, smol_update_config_t *config) {
    ...
    cJSON *node_version = cJSON_GetObjectItemCaseSensitive(update, "nodeVersion");
    if (node_version && cJSON_IsString(node_version) && node_version->valuestring) {
        config->node_version = node_version->valuestring;
    }
