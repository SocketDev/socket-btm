...
// Populated by `initBuildContext()`, which `main()` awaits before any helper
// below reads them — kept as module-scope `let` bindings (rather than
// top-level `await`) because the CJS bundle target does not support TLA.
let OPENTUI_VERSION
let PLATFORM_ARCH
let BUILD_DIR
let OUTPUT_DIR
let SOURCE_PATCHED_DIR
let getPlatformOutputPath
...
async function initBuildContext() {
  ...
  try {
    ...
    throw new Error(
      `Failed to parse package.json at ${path.join(PACKAGE_ROOT, 'package.json')}: ${errorMessage(e)}`,
      { cause: e },
    )
  }
  ...
  OPENTUI_VERSION = `v${opentuiSource.version}`

  // getCurrentPlatform() now honors PLATFORM_ARCH env itself.
  PLATFORM_ARCH = await getCurrentPlatform()

  const paths = getBuildPaths(BUILD_MODE, PLATFORM_ARCH)
  BUILD_DIR = paths.buildDir
  OUTPUT_DIR = paths.outDir
  SOURCE_PATCHED_DIR = paths.sourcePatchedDir
  getPlatformOutputPath = paths.getPlatformOutputPath
}
...
export async function ensureZig() {
  ...
  const zigBin = ('path' in result && result.path) || 'zig'
  ...
  try {
    ...
    if (testExit !== 0) {
      printError('Zig cannot link on this platform.')
