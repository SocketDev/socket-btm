...
int binject_codesign(const char *binary_path) {
    ...
    if (pid == 0) {
        ...
        execvp("codesign", argv);
        /* If execvp returns, it failed */
        fprintf(stderr, "Error: Failed to execute codesign: %s\n", strerror(errno));
        exit(1);
    }
...
int binject_compress_binary(const char *input_path, const char *output_path, const char *quality) {
    ...
    if (pid == 0) {
        /* Child: run binpress */
        char quality_arg[64];
        snprintf(quality_arg, sizeof(quality_arg), "--quality=%s", quality);
        char *argv[] = {(char*)binpress, (char*)input_path, (char*)output_path, quality_arg, NULL};
        execv(binpress, argv);
        /* If execv returns, it failed */
        fprintf(stderr, "Error: Failed to execute binpress: %s\n", strerror(errno));
        exit(1);
    }
...
int binject_repack_stub(const char *stub_path, const char *compressed_data_path, const char *output_path, size_t uncompressed_size) {
    ...
    size_t compressed_size = ftell(data_fp);
...
int binject_repack_workflow(const char *stub_path, const char *extracted_path, const char *output_path) {
    ...
    if (binject_codesign(extracted_path) != 0) {
        fprintf(stderr, "Warning: Failed to sign modified binary (continuing anyway)\n");
        /* Non-fatal - continue */
    }
    ...
    snprintf(temp_compressed, sizeof(temp_compressed), "%s.compressed", extracted_path);
    ...
    if (binject_codesign(output_path) != 0) {
        fprintf(stderr, "Warning: Failed to sign repacked stub (continuing anyway)\n");
