...
async function clean() {
  logger.info('🧹 Cleaning node-smol-builder…')
  logger.log('')
  ...
  if (cleanBuild) {
    ...
    // Clean downloaded binsuite tools (binpress, binflate, binject).
    const tools = ['binpress', 'binflate', 'binject']
    for (const tool of tools) {
      const toolDir = path.join(BUILD_ROOT, tool)
      if (existsSync(toolDir)) {
        await safeDelete(toolDir)
        logger.success(`Cleaned build/${tool}/`)
      }
    }
