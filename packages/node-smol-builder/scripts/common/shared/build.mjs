 ...
 *
 * @param {string} phase - Build phase ('release', 'stripped', 'compressed', 'final')
 * @returns {string[]} Array of absolute paths to source files for this phase
 */
function collectBuildSourceFiles(phase = 'release') {
  const sources = []
...
async function main() {
  ...
  // ============================================================================
  // PHASE 1: RELEASE - Clone, patch, configure, compile
  // ============================================================================
  const { buildRelease } = await import(
    '../../release/shared/build-release.mjs'
  )
  ...
  const releaseResult = await buildRelease({
    ...
    collectBuildSourceFiles: () => collectBuildSourceFiles('release'),
    packageRoot: PACKAGE_ROOT,
  })

  // If cached or complete, exit early
  if (releaseResult.cached) {
    return
  }
  ...
  const { buildStripped } = await import(
    '../../stripped/shared/build-stripped.mjs'
  )
  // Generate stripped-phase-specific source paths
  const strippedSourcePaths = collectBuildSourceFiles('stripped')
  await buildStripped({
    buildDir: BUILD_DIR,
    packageName: PACKAGE_NAME,
    outputReleaseBinary,
  ...
  const { buildCompressed } = await import(
    '../../compressed/shared/build-compressed.mjs'
  )
  // Generate compressed-phase-specific source paths
  const compressedSourcePaths = collectBuildSourceFiles('compressed')
  ...
  // ============================================================================
  // PHASE 4: FINAL - Copy final binary for distribution
  // ============================================================================
  ...
  // Use 'final' phase for the main cache hash (includes all sources)
  const finalSourcePaths = collectBuildSourceFiles('final')
