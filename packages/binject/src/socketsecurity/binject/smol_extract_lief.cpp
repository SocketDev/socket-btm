// ============================================================================
// smol_extract_lief.cpp — Extract compressed binaries from SMOL stubs
// ============================================================================
//
// WHAT THIS FILE DOES
// Reads the __PRESSED_DATA section from a SMOL-compressed executable using
// LIEF, then decompresses it with zstd. Used by binject's repack workflow
// to extract the original binary before re-injecting with updated config.
//
// WHY IT EXISTS
// When updating a SMOL binary's config (e.g., changing the update URL),
// we need to extract the compressed binary, re-compress with new metadata,
// and re-inject. This file handles the extraction step using LIEF to read
// the section from any format (ELF, Mach-O, PE).
// ============================================================================
...
extern "C" int smol_extract_binary_lief(const char *stub_path, const char *output_path) {
    ...
    /* Validate section size before unsigned subtraction to prevent underflow */
    if (compressed_size < MAGIC_MARKER_LEN) {
        fprintf(stderr, "Error: PRESSED_DATA section too small (%zu bytes, minimum %u)\n",
                compressed_size, MAGIC_MARKER_LEN);
        return -1;
    }
    ...
    if (uncompressed_size == 0 || uncompressed_size > DECOMPRESSOR_MAX_UNCOMPRESSED_SIZE) {
        fprintf(stderr, "Error: Invalid uncompressed size: %llu (max: %d MB)\n",
                (unsigned long long)uncompressed_size,
    ...
    if (stored_compressed_size > uncompressed_size) {
        fprintf(stderr, "Error: Compressed size %llu > uncompressed size %llu (invalid)\n",
                (unsigned long long)stored_compressed_size, (unsigned long long)uncompressed_size);
        return -1;
    }
    ...
    /* METADATA_HEADER_LEN = SIZE_HEADER_LEN(16) + CACHE_KEY_LEN(16) + PLATFORM_METADATA_LEN(3) + SMOL_CONFIG_FLAG_LEN(1) = 36 */
    /* has_smol_config flag is at offset METADATA_HEADER_LEN - 1 = 35 */
    uint8_t has_smol_config = metadata_pos[METADATA_HEADER_LEN - 1];
    size_t config_size = (has_smol_config != 0) ? SMOL_CONFIG_BINARY_LEN : 0;
    const uint8_t *data_start = metadata_pos + METADATA_HEADER_LEN + config_size;
    ...
    if (decompress_result != COMPRESS_OK) {
        fprintf(stderr, "Error: ZSTD decompression failed (code: %d = %s)\n",
                decompress_result, compress_error_string(decompress_result));
        fprintf(stderr, "  Compressed size: %zu bytes, expected uncompressed: %llu bytes\n",
                actual_compressed_size, (unsigned long long)uncompressed_size);

        /* Provide context-aware diagnostics for common failure scenarios */
        if (decompress_result == COMPRESS_ERROR_DECOMPRESS_FAILED) {
            /* Check if data_start points to SMFG config magic instead of compressed data */
            if (actual_compressed_size >= 4) {
                uint32_t first_word;
                memcpy(&first_word, data_start, sizeof(first_word));
                if (first_word == 0x534D4647) {  /* "SMFG" magic */
                    fprintf(stderr, "  Cause: Data offset points to SMFG config instead of compressed data.\n");
                    fprintf(stderr, "  This indicates the has_smol_config flag was not correctly handled.\n");
            ...
            /* Check if input might be already compressed (double-compression attempt) */
            if (has_smol_config == 0 && stored_compressed_size < uncompressed_size / 2) {
                fprintf(stderr, "  Note: If you're trying to compress an already-compressed binary,\n");
                fprintf(stderr, "  use binpress's automatic repack mode which extracts before re-compressing.\n");
            }
        }
...
/*
 * smol_extract_node_version is now in bin-infra/smol_detect_lief.cpp
 * to be shared between binject and binpress packages.
 * The declaration is in socketsecurity/bin-infra/smol_detect.h
 */
