/**
 * @file Tool installation helper using external-tools.json
...
export async function detectPackageManager() {
  ...
  // Linux - check for apt, yum, dnf, apk in parallel (priority order).
  const managers = ['apt', 'yum', 'dnf', 'apk']
  const results = await Promise.allSettled(
    managers.map(mgr =>
      spawn('which', [mgr], { stdio: 'ignore' }).then(() => mgr),
    ),
  )
  // Return first available manager in priority order.
 ...
 * @param {string} [options.packageRoot] - Package root for hierarchical
 *   loading.
 * @param {string} [options.checkpointName] - Checkpoint name.
 * @param {boolean} [options.force] - Force reinstall even if already installed.
 * @param {boolean} [options.skipVersionPin] - Skip version pinning (install
 *   latest)
 *
...
export async function installTool(toolName, options = {}) {
  ...
  try {
    ...
    throw new Error(
      `Failed to install ${toolName}: ${errorMessage(e)}. ` +
...
/**
 * Ensure apt cache is updated (Linux only, run once before installing multiple
 * packages)
 */
export async function updatePackageCache() {
  ...
  if (pkgMgr === 'apt') {
    ...
    try {
      ...
      logger.warn(`Failed to update package cache: ${errorMessage(e)}`)
    }
  }
}
