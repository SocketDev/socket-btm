...
// Strict octal: only digits 0-7, one or more. Rejects "0abc" which
// NumberParseInt silently truncates to 0, letting crafted TAR headers
// hide real file sizes behind a bogus leading-digit.
const OCTAL_STRICT_REGEX = /^[0-7]+$/
...
function parseOctal(buffer, offset, length) {
  ...
  // Reject non-octal characters. NumberParseInt('0abc', 8) silently
  // returns 0, letting a crafted TAR header (e.g. size="0abc") pass
  // NaN-checks downstream. Return NaN so callers' NumberIsNaN guard
  // can reject the header. See R34 scan.
  if (!RegExpPrototypeTest(OCTAL_STRICT_REGEX, str)) {
    return NaN
  }
...
function parseTar(tarBuffer, options = {}) {
  ...
  // Hard-links whose target hadn't been parsed yet when we encountered
  // them. Resolved in a second pass below so TAR archives that list
  // a hard-link before its target (spec allows this) don't silently
  ...
  while (offset < tarBuffer.length) {
    ...
    if (paxAttrs) {
      ...
      if (paxAttrs.size) {
        ...
        if (!NumberIsNaN(parsedSize)) {
          ...
          // Re-check the truncation guard against the overridden size.
          // The earlier `offset + header.size > tarBuffer.length` check
          // at line 544 used the USTAR size, which may be smaller than
          // the PAX override. A crafted archive with small USTAR + huge
          // PAX size would slip past the original guard and the data-
          // extraction read below would slice past end-of-buffer.
          if (offset + header.size > tarBuffer.length) {
            throw new ErrorConstructor(
              `Truncated TAR archive: file "${header.name}" claims ${header.size} bytes (PAX override) ` +
                `but only ${tarBuffer.length - offset} bytes remaining`,
            )
          }
        }
    ...
    if (
      ...
      if (targetEntry && targetEntry.type === 'file') {
        ...
        // Target not yet parsed — record for post-loop resolution.
        // TAR spec allows hard-link entries to appear BEFORE their
        // target. Dropping to a zero-length file here would silently
        // corrupt archives produced by GNU tar with --sparse or by
        // tools that emit hard-links in alphabetical rather than
        // creation order.
        ArrayPrototypePush(pendingHardLinks, {
          __proto__: null,
          name: header.name,
  ...
  // Resolve pending hard-links whose target was parsed after the link.
  // In strict mode, an unresolvable hard-link is a hard error; otherwise
  // fall back to zero-length file for compatibility with the older
  // lenient behavior (the archive itself is malformed).
  for (let i = 0; i < pendingHardLinks.length; i++) {
    const { name, linkname } = pendingHardLinks[i]
    const targetEntry = MapPrototypeGet(files, linkname)
    if (targetEntry && targetEntry.type === 'file') {
      MapPrototypeSet(files, name, {
        __proto__: null,
        content: getContent(targetEntry),
        mode: targetEntry.mode,
        type: 'file',
      })
    } else if (options.strictFormat === true) {
      throw new ErrorConstructor(
        `Hard link target not found for "${name}" -> "${linkname}"`,
      )
    } else {
      MapPrototypeSet(files, name, {
        __proto__: null,
