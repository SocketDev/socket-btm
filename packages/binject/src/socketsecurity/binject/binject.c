// ============================================================================
// binject.c — Core implementation of binary injection operations
// ============================================================================
//
// WHAT THIS FILE DOES
// Implements the main binject operations: detecting executable format (ELF,
// Mach-O, PE), reading/writing resource data, handling compressed stubs,
// and orchestrating inject/extract/list/verify commands across platforms.
//
// WHY IT EXISTS
// This is the "engine" behind binject. It figures out what kind of binary
// you have, reads files into memory, and delegates to the right
// platform-specific code (elf_inject.c, pe_inject.c, macho_inject_lief_wrapper.c).
// ============================================================================
 ...
 * struct stat declarations on Windows. Must be defined before first use. */
#ifdef _WIN32
static int binject_stat(const char* path, struct _stat* st) {
    return _stat(path, st);
}
#define BINJECT_STAT_STRUCT struct _stat
#else
static int binject_stat(const char* path, struct stat* st) {
    return stat(path, st);
}
...
static int binject_is_smol_stub(const char *executable_path) {
    ...
    /* Check if SMOL section exists */
    switch (format) {
        case BINJECT_FORMAT_MACHO:
            return smol_has_pressed_data_macho_impl(executable_path);
        case BINJECT_FORMAT_ELF:
            return smol_has_pressed_data_elf_lief(executable_path);
        case BINJECT_FORMAT_PE:
            return smol_has_pressed_data_pe_lief(executable_path);
        default:
            return 0;
    }
}
