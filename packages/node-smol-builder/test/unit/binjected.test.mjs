...
const buildInfraDir = BUILD_INFRA_DIR
...
describe('compression tools', () => {
  describe('source files', () => {
    it('should have binpress Makefile', () => {
      const makefilePath = path.join(binpressDir, 'Makefile.macos')
      expect(existsSync(makefilePath)).toBe(true)
    })

    it('should have binflate Makefile', () => {
      const makefilePath = path.join(binflateDir, 'Makefile.macos')
      expect(existsSync(makefilePath)).toBe(true)
    })
  ...
  describe('compression tool implementation', () => {
    ...
    it('Windows decompressor should use LZFSE', async () => {
      const sourceFile = path.join(binflateDir, 'src', 'pe_decompress.c')
      const content = await fs.readFile(sourceFile, 'utf-8')

      expect(content).toContain('LZFSE')
      expect(content).toContain('decompress_buffer_sized')
    })
  ...
  describe('Makefile configuration', () => {
    it('binflate Makefile should use static linking for decompressor', async () => {
      // Static linking is configured in platform-linux.mk (included by Makefile.linux)
      const platformLinuxMk = path.join(
        buildInfraDir,
        'make',
        'platform-linux.mk',
      )
      const content = await fs.readFile(platformLinuxMk, 'utf-8')

      // Check for static linking flags in Linux platform config
      expect(content).toContain('-static')
      expect(content).toContain('lzma')
    })

    it('Makefiles should use optimization flags', async () => {
      // Optimization flags are configured in platform-specific .mk files
      const platformMkFiles = [
        {
          path: path.join(buildInfraDir, 'make', 'platform-macos.mk'),
          name: 'macOS',
        },
        {
          path: path.join(buildInfraDir, 'make', 'platform-linux.mk'),
          name: 'Linux',
        },
      ]

      for (const { path: mkPath } of platformMkFiles) {
        // eslint-disable-next-line no-await-in-loop
        const content = await fs.readFile(mkPath, 'utf-8')

        // Should have optimization flags: -O0 (dev), -O2, -O3, or -Os (prod)
        expect(content).toMatch(/-O[023s]/)
      }
    })
  })
  ...
  describe('dlxBinary caching strategy', () => {
    ...
    it('macOS decompressor should follow dlxBinary cache structure', async () => {
      ...
      expect(content).toContain('socket-lib')
