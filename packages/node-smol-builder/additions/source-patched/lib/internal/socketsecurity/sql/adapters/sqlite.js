...
// Detect DML with RETURNING clause (INSERT/UPDATE/DELETE … RETURNING …).
// node:sqlite's .run() discards the returned row set; we must route to
// .all() instead. SQLite's RETURNING support lands in libsqlite 3.35+.
const RETURNING_CLAUSE_REGEX = hardenRegExp(/\bRETURNING\b/i)
...
function getOrCreateStatement(cache, db, text) {
  return lruGetOrCreate(cache, text, STATEMENT_CACHE_MAX_SIZE, () => ({
    __proto__: null,
...
class SQLiteAdapter {
  ...
  async query(text, values = [], format = 'objects') {
    ...
    try {
      ...
      const cached = getOrCreateStatement(
        this[kStatementCache],
        this[kDb],
        text,
      )
...
function isSelectQuery(text) {
  ...
  if (
    StringPrototypeStartsWith(trimmed, 'SELECT') ||
  ...
  // DML with RETURNING produces rows; must use .all() not .run() or
  // returned data is silently dropped.
  return RegExpPrototypeExec(RETURNING_CLAUSE_REGEX, trimmed) !== null
}
