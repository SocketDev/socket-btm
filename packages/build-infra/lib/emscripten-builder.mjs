...
export class EmscriptenBuilder {
  ...
  async build({
    ...
    logger.substep('Building with Emscripten')
    ...
    if (optimize) {
      args.push(...WASM_OPT_FLAGS.split(' '))
    }

    const result = await spawn('emcc', args, {
      cwd: this.sourceDir,
      shell: WIN32,
      stdio: 'inherit',
    })
  ...
  async strip(wasmFile) {
    logger.substep('Stripping debug info')
  ...
  async configureCMake(options = {}) {
    logger.substep('Configuring CMake for Emscripten')
  ...
  async buildWithCMake({ parallel = true, target = 'all' } = {}) {
    logger.substep('Building with emmake')
