...
extern "C" int smol_repack_lief_elf(
    ...
    try {
        ...
        // In ELF, the compressed data is in a section, not a segment.
        // Section name matches Mach-O: __PRESSED_DATA -> .pressed_data (ELF lowercase convention)
        const char* section_name = ELF_SECTION_PRESSED_DATA;
        ...
        new_section.flags(static_cast<uint64_t>(LIEF::ELF::Section::FLAGS::ALLOC));  // ALLOC = loadable into memory (read-only by default)
        new_section.alignment(4);  // 4-byte alignment (matches Mach-O alignment(2) = 2^2)
