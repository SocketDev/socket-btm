...
import type {
  ExternalTool,
  ExternalToolsFile,
} from './external-tools-schema.mts'

const logger = getDefaultLogger()
const __dirname = path.dirname(fileURLToPath(import.meta.url))

/**
 * Hierarchical lookup context for external-tools.json resolution.
 */
export interface LoadExternalToolsOptions {
 ...
 */
export function loadExternalToolsJson(
  jsonPath: string,
  ...
  try {
    ...
    if (visited.has(resolvedPath)) {
      logger.warn(`Circular dependency detected: ${resolvedPath}`)
      return {}
    }
    ...
    if (!validation.ok) {
      ...
      for (let i = 0, { length } = issueLines; i < length; i += 1) {
        const line = issueLines[i]!
        logger.warn(line)
      }
    }

    let tools: ExternalToolsMap = {}
    ...
    logger.fail(
      `Failed to load external-tools.json from ${jsonPath}: ${errorMessage(e)}`,
    )
    return {}
  }
}
...
export function loadExternalTools({
  checkpointName,
  packageRoot,
}: LoadExternalToolsOptions = {}): ExternalToolsMap {
  let tools: ExternalToolsMap = {}
...
/**
 * Pinned tool versions loaded from external-tools.json files.
 *
...
export function getToolPackageSpec(
  ...
  const version = getToolVersion(toolName, options)
  if (!version) {
