 ...
 *
 * Usage:
 *   node scripts/check-consistency.mts              # Check only
 *   node scripts/check-consistency.mts --fix        # Auto-fix all fixable issues
 *   node scripts/check-consistency.mts --interactive # Ask before fixing each issue
 *   node scripts/check-consistency.mts --dry-run    # Show what would be fixed
 *   node scripts/check-consistency.mts --suggest    # Show ML-powered suggestions
 *
 * Features:
...
type IssueLevel = 'error' | 'info' | 'warning'
...
type FixFn = () => Promise<string>

type FixableIssue = Issue & {
  fix: FixFn
  level: IssueLevel
}
...
Usage: node scripts/check-consistency.mts [options]
...
Examples:
  ...
  node scripts/check-consistency.mts --dry-run
    Show what would be fixed without making changes

  node scripts/check-consistency.mts --fix
    Automatically fix all fixable issues

  node scripts/check-consistency.mts --interactive
    Review and approve each fix individually

  node scripts/check-consistency.mts --suggest
    Show ML-powered suggestions based on codebase patterns

  node scripts/check-consistency.mts --fix --suggest
...
const issues: Record<IssueLevel, Issue[]> = {
  error: [],
  info: [],
  warning: [],
}
...
export async function checkBuildOutputStructure(
  ...
  for (let i = 0, { length } = packages; i < length; i += 1) {
    ...
    //   4. native-addon style: build/{mode}/{platform-arch}/out/{platform-arch}/<name>.node (opentui-builder, ultraviolet-builder)
    //
    ...
    const hasFinalUnder = async (modeDir: string): Promise<boolean> => {
      ...
      for (let i = 0, { length } = entries; i < length; i += 1) {
        ...
        // (opentui-builder, ultraviolet-builder write the .node binary into a
