...
async function downloadFile(url, destPath) {
  ...
  const result = await spawn('curl', ['-fSL', '--retry', '3', '-o', destPath, url], {
    stdio: 'inherit',
 ...
 */
export async function downloadAndCache(tool, resolverResult, version, platform, arch) {
  ...
  if (existsSync(binaryPath) && verifyCacheIntegrity(cachePath, resolverResult.sha256)) {
  ...
  try {
    ...
    if (existsSync(binaryPath) && verifyCacheIntegrity(cachePath, resolverResult.sha256)) {
      logger.substep(`Using cached ${tool} ${version} (populated by another process)`)
      return binaryPath
    }
    ...
    const tmpDir = path.join(getCacheDir(), `.tmp-${tool}-${version}-${process.pid}`)
    ...
    try {
      logger.substep(`Downloading ${tool} ${version} (${formatSize(resolverResult.size)})...`)
      ...
      await fs.writeFile(path.join(extractedDir, '.checksum'), resolverResult.sha256)
 ...
 */
export function getCachedToolBinary(tool, version, platform, arch, binary, expectedSha256) {
  const cachePath = getToolCachePath(tool, version, platform, arch)
  const binaryPath = path.join(cachePath, binary)
  if (existsSync(binaryPath) && verifyCacheIntegrity(cachePath, expectedSha256)) {
    return binaryPath
  }
  return undefined
}
