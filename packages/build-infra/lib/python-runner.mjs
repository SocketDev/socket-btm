...
export async function runPythonScriptWithJson(
  ...
  if (!result.stdout || !result.stdout.trim()) {
    throw new Error(
      'Python script produced no JSON output. Expected valid JSON on stdout.',
    )
  }
  ...
  try {
    ...
    // If that fails, try to find JSON in the output (last line is common pattern)
    ...
    throw new Error(
      `Failed to parse Python script JSON output: ${error.message}\n` +
        `Output contained no valid JSON: ${result.stdout.substring(0, 500)}`,
    )
  }
}
