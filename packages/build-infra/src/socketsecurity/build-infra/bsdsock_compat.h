...
#define SOCKET_HANDLE_INVALID (~static_cast<uintptr_t>(0))

/*
 * Error code macros - map Windows WSA errors to BSD-style access.
 */
#define BSD_GET_SOCK_ERROR()   WSAGetLastError()
#define BSD_CLOSE_SOCK(s)      closesocket(static_cast<SOCKET>(s))
 ...
 */
#define BSD_SOCKET_CAST(s) static_cast<SOCKET>(s)
