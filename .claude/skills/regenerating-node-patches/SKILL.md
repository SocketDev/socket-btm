...
description: Regenerates all 15 Node.js patches from pristine upstream source. Triggers when patches fail to apply, after Node.js version update, or when restructuring patches.
user-invocable: true
allowed-tools: Task
---
...
Socket Security maintains 15 security and functionality patches that are applied to upstream Node.js source during the build process. These patches must be regenerated from pristine source when:
...
- Applies 15 patches to Node.js source files (common.gypi, node.gyp, src/_, lib/_)
...
This skill spawns an autonomous agent to handle the complete patch regeneration workflow. The agent will process all 15 patches sequentially.
...
<task>
Your task is to regenerate 15 Socket Security Node.js patches from pristine upstream Node.js v25.5.0 source, ensuring each patch applies cleanly to unmodified upstream files. You must follow the 8-step workflow for EACH patch with proper backups, validation, and cleanup.
</task>
...
7. 007-node-sea-smol-config.patch → src/node_sea.cc
8. 008-node-sea-header.patch → src/node_sea.h
9. 009-node-sea-bin-binject.patch → src/node_sea.cc
10. 010-fix_v8_typeindex_macos.patch → common.gypi
11. 011-vfs_bootstrap.patch → lib/internal/bootstrap/node.js
12. 012-vfs_require_resolve.patch → lib/internal/modules/cjs/loader.js
13. 013-debug-utils-smol-sea-category.patch → lib/internal/util/debuglog.js
14. 014-node-sea-silent-exit.patch → src/node_main.cc
15. 015-fast-webstreams.patch → lib/internal/bootstrap/node.js
</context>
...
For EACH of the 15 patches, execute these 8 steps:
...
- ✅ All 15 patches regenerated from pristine v25.5.0
...
- ✗ Time-consuming for all 15 patches
