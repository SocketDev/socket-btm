...
# Determine platform and set compressor binary
PLATFORM=$(uname -s)
if [ "$PLATFORM" = "Darwin" ] || [ "$PLATFORM" = "Linux" ]; then
    COMPRESSOR="$PACKAGE_DIR/out/binpress"
else
    COMPRESSOR="$PACKAGE_DIR/out/binpress.exe"
fi
...
run_test "compress_basic" "$COMPRESSOR '$TEST_DIR/test.txt' '$TEST_DIR/test.txt.compressed'"
...
run_test "compress_large_file" "$COMPRESSOR '$TEST_DIR/large.bin' '$TEST_DIR/large.bin.compressed'"
...
run_test "error_missing_input" "! $COMPRESSOR '/tmp/nonexistent_file_12345.txt' '$TEST_DIR/out.compressed' 2>/dev/null"

# Test: Error handling - invalid arguments (too few)
run_test "error_invalid_args" "! $COMPRESSOR 2>/dev/null"

# Test: Compression overhead detection - small incompressible file
...
# Compress the small random file
$COMPRESSOR "$TEST_DIR/small_random.bin" "$TEST_DIR/small_random.bin.compressed" > /dev/null 2>&1
run_test "compress_small_incompressible" "[ -f '$TEST_DIR/small_random.bin.compressed' ]"

# Test: Output file should be same size or smaller than input (no size increase)
SMALL_ORIGINAL=$(wc -c < "$TEST_DIR/small_random.bin")
SMALL_COMPRESSED=$(wc -c < "$TEST_DIR/small_random.bin.compressed")
run_test "compression_no_size_increase" "[ $SMALL_COMPRESSED -le $SMALL_ORIGINAL ]"

# Test: When compression doesn't help, output should equal input (not larger)
# This verifies the overhead detection logic copies the original file
run_test "compression_overhead_copies_original" "[ $SMALL_COMPRESSED -eq $SMALL_ORIGINAL ]"
