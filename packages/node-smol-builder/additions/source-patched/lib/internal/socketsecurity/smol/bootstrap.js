...
// Documentation: docs/additions/lib/internal/socketsecurity/smol/bootstrap.js.md
 ...
 *
 * Node 25.7+ compatibility: the ambient `require` inside a CJS SEA entry
 * is replaced by embedder hooks that only resolve built-in module names;
 * external file:// / absolute-path loads would fail with
 * ERR_UNKNOWN_BUILTIN_MODULE. `Module.createRequire(scriptPath)` creates
 * a fresh require bound to the virtual SEA script and bypasses those
 * hooks, so VFS-backed `require()` keeps working. Do NOT replace this
 * with `await import(pathToFileURL(...))`.
 */
...
function setupProcessForSmol() {
  ...
  // Guard against short/empty argv arrays (can happen in some embedding scenarios).
  if (shouldInsertEntrypoint) {
    if (ProcessArgv.length === 0) {
      // Extremely rare: no argv at all - initialize with execPath and entrypoint
      ArrayPrototypePush(ProcessArgv, EXECPATH)
      ArrayPrototypePush(ProcessArgv, DEFAULT_ENTRYPOINT)
    } else if (ProcessArgv.length === 1) {
      // argv only has argv[0], push the entrypoint as argv[1]
      ArrayPrototypePush(ProcessArgv, DEFAULT_ENTRYPOINT)
    } else if (ProcessArgv[1] !== DEFAULT_ENTRYPOINT) {
      // Replace existing argv[1] with the virtual entrypoint
      ProcessArgv[1] = DEFAULT_ENTRYPOINT
    }
  }

  const ENTRYPOINT =
    ProcessArgv.length > 1 ? ProcessArgv[1] : DEFAULT_ENTRYPOINT
