// ============================================================================
// macho_compress_segment.h — Mach-O segment embedding API
// ============================================================================
//
// WHAT THIS FILE DOES
// Declares functions to embed compressed data as a proper Mach-O segment
// (SMOL/__PRESSED_DATA), extract it back, and check if a binary has one.
//
// WHY IT EXISTS
// On macOS, simply appending data to a binary breaks code signatures. This
// API inserts the compressed data BEFORE __LINKEDIT (the last segment),
// which keeps the signature valid and passes Gatekeeper checks.
// ============================================================================
 ...
 * @param binary_path Path to Mach-O binary to check
 * @return 1 if SMOL segment exists, 0 if not, -1 on error
 */
