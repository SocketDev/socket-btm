 ...
 * binaries from build/{dev,prod}/out/{Stripped,Final,Compressed}/node.
 */
...
/**
 * Find the latest binary from build/{dev,prod}/out/{stage}/node.
...
function getLatestBinary(stage) {
  ...
  // Check dev binary
  try {
    const devStat = fs.statSync(devBinaryPath)
  ...
  // Check prod binary
  try {
    const prodStat = fs.statSync(prodBinaryPath)
...
/**
 * Find the latest Stripped binary from build/{dev,prod}/out/Stripped/node.
 *
 * The Stripped binary has debug symbols removed but retains pre-created Mach-O sections
 * (NODE_SEA_BLOB, SOCKSEC_VFS_BLOB) required for binject injection.
 *
 * Returns the path to whichever exists and has the latest modification time.
 *
...
/**
 * Find the latest Final binary from build/{dev,prod}/out/Final/node.
...
/**
 * Find the latest Compressed binary from build/{dev,prod}/out/Compressed/node.
