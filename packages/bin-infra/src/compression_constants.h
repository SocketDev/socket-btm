...
/**
 * Update config format (appears after platform metadata):
 * - 1 byte: has_update_config (0=no config, 1=has config)
 * - 1112 bytes: update_config_binary (if has_update_config=1)
 *
 * Update config binary format:
 * - Magic (4 bytes): 0x55504446 ("UPDF")
 * - Version (2 bytes): 1
 * - Config data (1106 bytes): validated fields
 */
#define UPDATE_CONFIG_FLAG_LEN 1
#define UPDATE_CONFIG_BINARY_LEN 1112

/**
 * Total metadata header size (appears after magic marker, before compressed data).
 * Calculation: SIZE_HEADER_LEN + CACHE_KEY_LEN + PLATFORM_METADATA_LEN + UPDATE_CONFIG_FLAG_LEN
 * = 16 + 16 + 3 + 1 = 36 bytes (without update config data)
 * = 36 + 1112 = 1148 bytes (with update config data)
 *
 * Note: UPDATE_CONFIG_FLAG_LEN is always present, UPDATE_CONFIG_BINARY_LEN is conditional
 */
#define METADATA_HEADER_LEN (SIZE_HEADER_LEN + CACHE_KEY_LEN + PLATFORM_METADATA_LEN + UPDATE_CONFIG_FLAG_LEN)
