...
int binject_extract_stub_to_cache(const char *compressed_stub, const char *extracted_path) {
    ...
    /* Read metadata: [marker][8-byte: compressed_size][8-byte: uncompressed_size][16-byte: cache_key][5-byte: platform_metadata]. */
    ...
    /* Read compression algorithm from platform metadata (4th byte after cache_key). */
    uint8_t compression_algorithm = buffer[metadata_offset + SIZE_HEADER_LEN + CACHE_KEY_LEN + 3];
    ...
    /* Decompress using cross-platform decompressor with detected algorithm. */
    printf("  Decompressing... (algorithm: %d)\n", compression_algorithm);
