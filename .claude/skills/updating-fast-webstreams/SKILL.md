...
description: Vendors fast-webstreams from npm, converts ESM to CommonJS with primordials. Triggers when WebStreams version needs update or build fails with streams errors.
user-invocable: true
...
## Primordials Requirement

**CRITICAL**: All vendored code MUST use primordials to protect against prototype pollution.

The sync script automatically applies primordials transforms during ES→CJS conversion. See `reference.md` for detailed transformation mappings.
...
- Skip full build validation (CI runs builds separately)
- Skip WPT validation (time-consuming, CI runs separately)
- Run sync and quick validation only
- Do NOT push changes (workflow handles push)

**Interactive Mode (default):**
- Run full build and validate
...
### Build (Skip in CI Mode)

```bash
# Detect CI mode
if [ "$CI" = "true" ] || [ -n "$GITHUB_ACTIONS" ]; then
  echo "CI mode: Skipping build (CI will run builds separately)"
else
  # From monorepo root
...
const {
  FastTransformStream,
} = require('internal/deps/fast-webstreams/transform')
