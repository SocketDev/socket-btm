...
// Skip entire suite if binject binary not built
const describeOrSkip = existsSync(BINJECT_PATH) ? describe : describe.skip

describeOrSkip('Cross-platform binary manipulation', () => {
  beforeAll(() => {
    // Verify binject binary exists (redundant but safe)
  ...
  for (const { arch, format, platform } of getSupportedPlatforms()) {
    ...
    //
    // On Windows, LIEF is built with gcc/MinGW (matching binject toolchain)
    const canInject =
      format === 'pe' ||
      // ELF: native on Linux, LIEF on others
      (format === 'elf' && os.platform() === 'linux') ||
      (format === 'elf' && os.platform() !== 'linux') ||
      // Mach-O: native on macOS, LIEF on others
    ...
    describeOrSkip(`${platform}-${arch} (${format})`, () => {
      ...
      test('should handle auto-overwrite on re-injection', async () => {
