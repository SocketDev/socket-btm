...
Outputs to `build/prod/out/Final/binject` (or `binject.exe` on Windows).
...
# Embed update checking config (for node-smol compressed binaries)
binject inject -e ./node-smol -o ./my-app --sea app.blob --update-config update-config.json
...
Embed automatic update checking configuration in compressed node-smol binaries:
...
binject inject -e ./node-smol -o ./my-app --sea sea-config.json --update-config update-config.json
```
...
- Config stored in binary format (1112 bytes, validated at build time)
...
- `--update-config` embeds update checking config in compressed node-smol binaries (1112 bytes, binary format validated at build time)
