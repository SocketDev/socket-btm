...
function colorBuf(color) {
  if (color instanceof RGBA) return color.buffer
...
export class Buffer {
  constructor(width, height, opts) {
    ...
    this._ptr = native.createOptimizedBuffer(width, height, respectAlpha, widthMethod, id)
    this._view = undefined
  }

  get ptr() { return this._ptr }
  get width() { return native.getBufferWidth(this._ptr) }
  get height() { return native.getBufferHeight(this._ptr) }

  get view() {
    if (!this._view) this._view = new BufferView(this._ptr)
    return this._view
  }
  ...
  drawText(text, x, y, fg, bg, attrs = 0) {
    ...
    if (_hasFA) {
      ...
      native.bufferDrawText(this._ptr, text, x, y, fgBuf[0], fgBuf[1], fgBuf[2], fgBuf[3], bgBuf[0], bgBuf[1], bgBuf[2], bgBuf[3], attrs)
    }
  }
  ...
  drawChar(char, x, y, fg, bg, attrs = 0) {
    ...
    if (_hasFA) {
      ...
      native.bufferDrawChar(this._ptr, char, x, y, fgBuf[0], fgBuf[1], fgBuf[2], fgBuf[3], bgBuf[0], bgBuf[1], bgBuf[2], bgBuf[3], attrs)
    }
  }
  ...
  setCell(x, y, char, fg, bg, attrs = 0) {
    ...
    if (_hasFA) {
      ...
      native.bufferSetCell(this._ptr, x, y, char, fgBuf[0], fgBuf[1], fgBuf[2], fgBuf[3], bgBuf[0], bgBuf[1], bgBuf[2], bgBuf[3], attrs)
    }
  }
  ...
  fillRect(x, y, width, height, bg) {
    ...
    if (_hasFA) {
      ...
      native.bufferFillRect(this._ptr, x, y, width, height, bgBuf[0], bgBuf[1], bgBuf[2], bgBuf[3])
    }
  }
  ...
  popScissorRect() { native.bufferPopScissorRect(this._ptr) }
  clearScissorRects() { native.bufferClearScissorRects(this._ptr) }

  pushOpacity(opacity) { native.bufferPushOpacity(this._ptr, opacity) }
  popOpacity() { native.bufferPopOpacity(this._ptr) }
  get opacity() { return native.bufferGetCurrentOpacity(this._ptr) }
...
export class TextBuffer {
  ...
  get ptr() { return this._ptr }
  get length() { return native.textBufferGetLength(this._ptr) }
  get byteSize() { return native.textBufferGetByteSize(this._ptr) }
  get lineCount() { return native.textBufferGetLineCount(this._ptr) }
  ...
  clear() { native.textBufferClear(this._ptr) }
  reset() { native.textBufferReset(this._ptr) }
  ...
  resetDefaults() { native.textBufferResetDefaults(this._ptr) }

  get tabWidth() { return native.textBufferGetTabWidth(this._ptr) }
  set tabWidth(w) { native.textBufferSetTabWidth(this._ptr, w) }

  addHighlight(lineIdx, start, end, styleId, priority = 0, hlRef = 0) {
    native.textBufferAddHighlight(this._ptr, lineIdx, start, end, styleId, priority, hlRef)
  }

  removeHighlightsByRef(hlRef) { native.textBufferRemoveHighlightsByRef(this._ptr, hlRef) }
  clearAllHighlights() { native.textBufferClearAllHighlights(this._ptr) }
  ...
  destroy() { native.destroyTextBuffer(this._ptr) }
}
...
export class EditBuffer {
  ...
  set text(value) { native.editBufferSetText(this._ptr, value) }
  ...
  get cursor() {
    if (this._cursor) return this._cursor.readEditBuffer(this._ptr)
    return native.editBufferGetCursor(this._ptr)
  }

  setCursor(row, col) { native.editBufferSetCursor(this._ptr, row, col) }
  setCursorByOffset(offset) { native.editBufferSetCursorByOffset(this._ptr, offset) }
  gotoLine(line) { native.editBufferGotoLine(this._ptr, line) }

  moveCursorLeft() { native.editBufferMoveCursorLeft(this._ptr) }
  moveCursorRight() { native.editBufferMoveCursorRight(this._ptr) }
  moveCursorUp() { native.editBufferMoveCursorUp(this._ptr) }
  moveCursorDown() { native.editBufferMoveCursorDown(this._ptr) }

  deleteChar() { native.editBufferDeleteChar(this._ptr) }
  deleteCharBackward() { native.editBufferDeleteCharBackward(this._ptr) }
  deleteRange(startRow, startCol, endRow, endCol) {
    native.editBufferDeleteRange(this._ptr, startRow, startCol, endRow, endCol)
  }
  deleteLine() { native.editBufferDeleteLine(this._ptr) }
  newLine() { native.editBufferNewLine(this._ptr) }

  undo() { native.editBufferUndo(this._ptr) }
  redo() { native.editBufferRedo(this._ptr) }
  get canUndo() { return native.editBufferCanUndo(this._ptr) }
  get canRedo() { return native.editBufferCanRedo(this._ptr) }
  clearHistory() { native.editBufferClearHistory(this._ptr) }
  ...
  getTextRangeByCoords(startRow, startCol, endRow, endCol) {
    return native.editBufferGetTextRangeByCoords(this._ptr, startRow, startCol, endRow, endCol)
  }

  clear() { native.editBufferClear(this._ptr) }
  destroy() { native.destroyEditBuffer(this._ptr) }
}
...
export class EditorView {
  ...
  get cursor() {
    if (this._cursor) return this._cursor.readEditorView(this._ptr)
    return native.editorViewGetCursor(this._ptr)
  }

  get visualCursor() { return native.editorViewGetVisualCursor(this._ptr) }
  get viewport() { return native.editorViewGetViewport(this._ptr) }
  get virtualLineCount() { return native.editorViewGetVirtualLineCount(this._ptr) }
  get totalVirtualLineCount() { return native.editorViewGetTotalVirtualLineCount(this._ptr) }
  get text() { return native.editorViewGetText(this._ptr) }

  setViewportSize(width, height) { native.editorViewSetViewportSize(this._ptr, width, height) }
  setViewport(x, y, width, height, moveCursor = false) {
    native.editorViewSetViewport(this._ptr, x, y, width, height, moveCursor)
  }
  clearViewport() { native.editorViewClearViewport(this._ptr) }
  setScrollMargin(margin) { native.editorViewSetScrollMargin(this._ptr, margin) }
  setWrapMode(mode) { native.editorViewSetWrapMode(this._ptr, mode) }

  moveUpVisual() { native.editorViewMoveUpVisual(this._ptr) }
  moveDownVisual() { native.editorViewMoveDownVisual(this._ptr) }

  setSelection(start, end, bgColor, fgColor) {
    const bg = colorBuf(bgColor ?? TRANSPARENT)
    const fg = colorBuf(fgColor ?? WHITE)
    native.editorViewSetSelection(this._ptr, start, end, bg[0], bg[1], bg[2], bg[3], fg[0], fg[1], fg[2], fg[3])
  }

  resetSelection() { native.editorViewResetSelection(this._ptr) }
  deleteSelectedText() { native.editorViewDeleteSelectedText(this._ptr) }

  destroy() { native.destroyEditorView(this._ptr) }
}
...
export class SyntaxStyle {
  ...
  get ptr() { return this._ptr }
  get styleCount() { return native.syntaxStyleGetStyleCount(this._ptr) }

  register(name, fg, bg, attrs = 0) {
    const fgBuf = colorBuf(fg ?? TRANSPARENT)
    const bgBuf = colorBuf(bg ?? TRANSPARENT)
    return native.syntaxStyleRegister(this._ptr, name, fgBuf[0], fgBuf[1], fgBuf[2], fgBuf[3], bgBuf[0], bgBuf[1], bgBuf[2], bgBuf[3], attrs)
  }

  resolve(name) { return native.syntaxStyleResolveByName(this._ptr, name) }
  destroy() { native.destroySyntaxStyle(this._ptr) }
}
...
export class Renderer {
  ...
  get nextBuffer() { return native.getNextBuffer(this._ptr) }
  get currentBuffer() { return native.getCurrentBuffer(this._ptr) }

  render(force = false) { native.render(this._ptr, force) }

  resize(width, height) { native.resizeRenderer(this._ptr, width, height) }
  ...
  get cursorState() {
    if (this._cursor) return this._cursor.readRenderer(this._ptr)
  ...
  setTerminalTitle(title) { native.setTerminalTitle(this._ptr, title) }
  clearTerminal() { native.clearTerminal(this._ptr) }

  enableMouse(enableMovement = true) { native.enableMouse(this._ptr, enableMovement) }
  disableMouse() { native.disableMouse(this._ptr) }

  setupTerminal(useAlternateScreen = true) { native.setupTerminal(this._ptr, useAlternateScreen) }
  suspend() { native.suspendRenderer(this._ptr) }
  resume() { native.resumeRenderer(this._ptr) }
  ...
  checkHit(x, y) { return native.checkHit(this._ptr, x, y) }
  clearHitGrid() { native.clearCurrentHitGrid(this._ptr) }

  destroy() { native.destroyRenderer(this._ptr) }
}
