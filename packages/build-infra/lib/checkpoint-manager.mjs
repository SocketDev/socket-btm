...
import { extractTarball } from './tarball-utils.mjs'
...
export async function createCheckpoint(
  ...
  if (data.artifactPath) {
    ...
    try {
      ...
      if (isDir) {
        try {
          ...
          if (error.code === 'EACCES' || error.code === 'EPERM') {
            throw new Error(
              `Cannot read directory: ${artifactPath} (${error.code}). ` +
      ...
      let lockFile
      try {
        lockFile = await open(lockPath, 'w')
    ...
    if (!checkpointAlreadyExists) {
      ...
      try {
        ...
        // Compute hash from extracted contents
        ...
        artifactHash = await computeSourceHash([extractedArtifactPath])
  ...
  if (data.sourcePaths && Array.isArray(data.sourcePaths)) {
    ...
    // Use 'unset' sentinel and || to treat empty strings same as undefined
    const envVars = [
...
export async function restoreCheckpoint(
  ...
  // Skip validation for source-copied checkpoints (platform-independent source code)
  const isSourceCheckpoint = checkpointName === 'source-copied' || checkpointName === 'source-patched'
    ...
    try {
      ...
      while (extractAttempts < MAX_EXTRACT_ATTEMPTS) {
        ...
        try {
          ...
          // to prevent modifications between extraction and validation
          // eslint-disable-next-line no-await-in-loop
          const extractedHash = await computeSourceHash([targetPath])
