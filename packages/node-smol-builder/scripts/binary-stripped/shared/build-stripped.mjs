...
export async function buildStripped({
  ...
  logger.logNewline()
  logger.success(`Binary copied to ${outputStrippedDir} (ready for stripping)`)
  logger.logNewline()
  ...
  if (IS_WINDOWS) {
    ...
    throw new Error(
      `Unsupported platform for binary stripping: ${platform}. Supported platforms: darwin, linux, linux-musl, win32`,
    )
  }
  ...
  const smokeTestPassed = await smokeTestBinary(outputStrippedBinary, null, {
  ...
  await createCheckpoint(
    ...
    async () => {
      ...
      const validated = await smokeTestBinary(outputStrippedBinary, null, {
        expectedArch: isCrossCompiling ? arch : undefined,
      })
      if (!validated) {
