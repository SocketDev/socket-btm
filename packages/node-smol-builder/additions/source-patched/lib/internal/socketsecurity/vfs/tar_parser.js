 ...
 * Optimized: 3 loops instead of conditional inside loop to avoid 512 branch predictions
 */
function calculateChecksum(buffer, offset) {
  let sum = 0

  // Sum bytes before checksum field (0 to 147)
  for (let i = 0; i < HEADER_OFFSET.CHECKSUM; i++) {
    sum += buffer[offset + i]
  }

  // Checksum field (148 to 155) treated as 8 spaces (ASCII 32)
  sum += 256 // 8 * 32 = 256

  // Sum bytes after checksum field (156 to 511)
  for (let i = HEADER_OFFSET.CHECKSUM + 8; i < HEADER_SIZE; i++) {
    sum += buffer[offset + i]
  }
    ...
    if (firstSlash === -1) {
      ...
      while (pos < filepath.length) {
        ...
        if (slashPos === -1) {
          ...
          const parentDir = pos === 0 ? '' : StringPrototypeSlice(filepath, 0, pos)
          if (entryName) {
            let children = MapPrototypeGet(dirIndex, parentDir)
            if (!children) {
              children = new SafeSet()
