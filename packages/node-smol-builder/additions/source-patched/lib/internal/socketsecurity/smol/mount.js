...
/**
 * Mount (extract) a file or directory from VFS to real filesystem.
 ...
 *
 * @param {string} vfsPath - Path in VFS (e.g., '/snapshot/node_modules/foo/bar.node' or '/snapshot/node_modules/foo')
...
function mount(vfsPath, options = {}) {
  ...
  if (!vfsPath.startsWith(`${vfsBase}/`) && vfsPath !== vfsBase) {
    throw new Error(
  ...
  if (stats?.isDirectory()) {
    return mountDirectory(vfsPath, relativePath, options)
  }
...
/**
 * Recursively extract a directory from VFS to real filesystem.
 ...
 */
function mountDirectory(vfsPath, relativePath, options) {
  const provider = getExtractionProvider()
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      mountDirectory(entryVfsPath, entryRelativePath, options)
    } else {
      // Check if already extracted
      const existing = provider.getExtracted(entryRelativePath)
      if (!existing) {
        // Extract file
        const content = readFileFromVFS(entryVfsPath)
        if (!content) {
          throw new Error(
        ...
        try {
          provider.extract(entryRelativePath, content)
        } catch (err) {
          const vfsConfig = getVFSConfig()
          const mode = vfsConfig?.mode || VFS_MODE_IN_MEMORY
          throw new Error(
  ...
  // For directories, we need to compute the path since extraction provider
  // only tracks individual files, not directory entries
  //
  // Note: relativePath is already relative to VFS base (e.g., /snapshot/node_modules)
  // so we don't add 'node_modules' again. The extraction path is:
  // - With provider: ~/.socket/_dlx/<hash>/<relativePath>
  // - Fallback: <execPath>/<relativePath>
  if (provider._getCacheDir) {
    return path.join(provider._getCacheDir(), normalizedRelativePath)
  }
  // Fallback for providers without _getCacheDir (e.g., in-memory)
  // Extract relative to process.execPath directory
  return path.join(path.dirname(process.execPath), normalizedRelativePath)
}
