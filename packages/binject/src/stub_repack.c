...
int binject_repack_stub(const char *stub_path, const char *compressed_data_path, const char *output_path, size_t uncompressed_size) {
    ...
    /* Detect platform metadata for repacked binary. */
    uint8_t platform_byte = 0;  /* Default to darwin. */
    uint8_t arch_byte = 0;      /* Default to x64. */
    uint8_t libc_byte = 0;      /* Default to n/a. */

#ifdef __APPLE__
    platform_byte = 0;  /* darwin */
    libc_byte = 0;      /* n/a */
#ifdef __arm64__
    arch_byte = 1;      /* arm64 */
#else
    ...
    platform_byte = 1;  /* linux */
#ifdef __aarch64__
    arch_byte = 1;      /* arm64 */
#else
    arch_byte = 0;      /* x64 */
#endif
    /* TODO: Detect musl vs glibc - for now default to glibc. */
    libc_byte = 1;      /* glibc */
#elif defined(_WIN32)
    platform_byte = 2;  /* win32 */
    libc_byte = 0;      /* n/a */
#ifdef _M_ARM64
    arch_byte = 1;      /* arm64 */
#else
    ...
    if (result != 0) {
        fprintf(stderr, "Error: LIEF repack failed\n");
        return -1;
    }
