...
async function compressBinary(
  ...
  // Find binpress binary in tools output directory.
  const binpressPath = path.join(TOOLS_DIR, config.toolName)
  if (!existsSync(binpressPath)) {
    throw new Error(
      `binpress not found at ${binpressPath}. Please build binpress first:\n` +
        '  pnpm --filter binpress run build\n' +
        '  Or download from releases:\n' +
        '  node packages/bin-infra/scripts/download-binsuite-tools.mjs --tool=binpress',
    )
  }
...
/*
  ...
  const dataMarker = Buffer.from(MAGIC_MARKER, 'utf-8')
  const compressedSizeBuffer = Buffer.alloc(HEADER_SIZES.COMPRESSED_SIZE)
  const uncompressedSizeBuffer = Buffer.alloc(HEADER_SIZES.UNCOMPRESSED_SIZE)
  ...
  // Detect platform metadata from build environment.
  const platformByte = PLATFORM_VALUES[hostPlatform] ?? 255
  const archByte = ARCH_VALUES[targetArch] ?? 255

  // Detect libc variant for Linux binaries.
  let libcByte = LIBC_VALUES.na
  if (hostPlatform === 'linux') {
    libcByte = await detectLibc(input)
  }

  // Write platform metadata (3 bytes).
  const platformMetadata = Buffer.from([platformByte, archByte, libcByte])

  // Concatenate: stub + data_marker + sizes + cache_key + platform_metadata + data.
  // Binary format:
  //   [Stub code]
