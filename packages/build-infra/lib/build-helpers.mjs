...
import { existsSync, readFileSync, readlinkSync, promises as fs } from 'node:fs'
...
import {
  ...
  PROC_SELF_EXE,
  WORKSPACE_DIR,
} from './environment-constants.mjs'
...
function isCrossCompiled(options, hostArch) {
  const { expectedArch } = { __proto__: null, ...options }
  return !!(expectedArch && expectedArch !== hostArch)
}
...
/**
 * Check if running inside a container environment where /proc/self/exe might not work.
 * Detects Docker, Buildkit, Depot, and other container runtimes.
 *
 * @returns {boolean}
 */
function isContainerEnvironment() {
  ...
  if (
    process.env.DEPOT_BUILD_ID ||
    process.env.DEPOT_PROJECT_ID ||
    process.env.DEPOT_TOKEN ||
  ...
  // Only check filesystem indicators on Linux
  if (os.platform() !== 'linux') {
    return false
  }

  // Check for /workspace directory (common in Depot and other CI container builds)
  // Combined with process.cwd() starting with /workspace
  if (process.cwd().startsWith(WORKSPACE_DIR) && existsSync(WORKSPACE_DIR)) {
    return true
  }
  ...
  try {
    const cgroup = readFileSync(PROC_CGROUP_FILE, 'utf8')
    if (
      cgroup.includes('docker') ||
      cgroup.includes('kubepods') ||
      cgroup.includes('buildkit')
    ) {
      return true
    }
  } catch {
    // Can't read cgroup - might be restricted or not Linux
  }

  // Check for buildkit-specific indicators
  // In buildkit, /proc/self/exe often points to a non-existent path or special location
  try {
    const exePath = readlinkSync(PROC_SELF_EXE)
    // If we can't resolve the exe path, we're likely in a restricted container
    if (!existsSync(exePath)) {
      return true
    }
  } catch {
    // readlink failed - likely in a container with restricted /proc
    return true
  }
 ...
 * These binaries use /proc/self/exe which may not work in containers.
 *
 ...
 * @param {string} options.expectedArch - Expected architecture (arm64/x64)
 * @param {boolean} options.expectStatic - Expect static linking
 * @returns {Promise<boolean>}
 */
async function staticVerifyBinary(binaryPath, options = {}) {
  const { expectStatic, expectedArch } = { __proto__: null, ...options }
  ...
  try {
    ...
    if (expectedArch) {
      ...
      if (pattern && !pattern.test(fileInfo)) {
        printError(`Expected ${expectedArch} architecture but got: ${fileInfo}`)
        return false
      }
      logger.success(`Confirmed ${expectedArch} architecture`)
    }
 ...
 *
 * @param {string} binaryPath - Path to binary
 * @param {string[]} args - Arguments to pass (optional, defaults to comprehensive tests)
...
export async function smokeTestBinary(binaryPath, args = null, options = {}) {
  ...
  try {
    ...
    if (!crossCompiled) {
      // Self-extracting binaries (compressed) have environment-specific issues:
      // - Linux containers: /proc/self/exe doesn't work (Docker, Buildkit, Depot)
      // - Windows: PE stub may hang during self-extraction
      // Fall back to static verification in these cases.
      // Note: CI environments are now tested with debug output enabled (SOCKET_SMOL_DEBUG=1)
      const isWindows = os.platform() === 'win32'
      if (
        isSelfExtractingBinary(binaryPath) &&
        (isContainerEnvironment() || isWindows)
      ) {
        const isLinux = os.platform() === 'linux'
        let reason
        if (isWindows) {
          reason = 'Windows PE stub may hang during extraction'
        } else if (isLinux && isContainerEnvironment()) {
          reason = '/proc/self/exe may not work in container'
        } else {
          reason = 'self-extraction may hang in this environment'
        }
        logger.log(`  Self-extracting binary detected (${reason})`)
        logger.log('  Using static verification instead...')
        logger.log('')
        return staticVerifyBinary(binaryPath, options)
      }
    ...
    // Detect musl from path or explicit option
    const isMusl = binaryPath.includes('musl') || options.isMusl
    ...
    const isLinuxArm64CrossCompile =
      hostPlatform === 'linux' && expectedArch === 'arm64' && hostArch === 'x64'
