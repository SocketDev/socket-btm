/**
 * @file WHATWG Streams WPT conformance gate (vitest wrapper).
 *   Spawns the rich CLI runner at test/scripts/wpt-streams-runner-main.mts
 ...
 *
 *   1. The runner already has a rich CLI surface (--filter, --verbose, --force) we
 *      want preserved for dev debugging — running it as a subprocess keeps that
 *      contract intact.
 *   2. WPT failures classify against an EXPECTED_FAILURES Map; only UNEXPECTED
 *      regressions should fail the gate. Vitest's per-test reporter doesn't
 *      compose with that classification cleanly; the runner's own report does.
 *      Single pass/fail signal is cleaner. The runner exits non-zero only when
 *      it finds unexpected regressions; expected-but-still-failing tests don't
 *      fail the gate. So a non-zero exit from this test == a real spec
 *      regression that needs eyes.
 */
