 ...
 * (run 'pnpm --filter build-infra sync-checksums' to update).
 *
 * @param {object} options - Options.
...
export async function verifyReleaseChecksum(options) {
  ...
  if (embeddedData?.[tool]?.checksums && Object.keys(embeddedData[tool].checksums).length > 0) {
    if (!quiet) {
      logger.fail(`No embedded checksum for ${assetName} in release-assets.json (tool: ${tool})`)
      logger.fail(`Run 'pnpm --filter build-infra sync-checksums' to update`)
    }
