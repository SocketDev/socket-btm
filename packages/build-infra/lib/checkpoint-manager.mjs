...
export async function shouldRun(
  ...
  if (sourcePaths && Array.isArray(sourcePaths) && sourcePaths.length > 0) {
    ...
    if (checkpointData?.cacheHash) {
      ...
      // Include platform/arch/libc suffix in validation (same logic as createCheckpoint)
      const platform = checkpointData.platform
      const arch = checkpointData.arch
      const libc = checkpointData.libc
      const targetSuffix =
        platform && arch ? `${platform}-${arch}${libc ? `-${libc}` : ''}` : ''
      const currentCacheHash = targetSuffix
        ? `${currentHash}-${targetSuffix}`
        : currentHash
