...
import { ensureCompressionTools } from '../../common/shared/compression-tools.mjs'
...
export async function buildCompressed(config, buildOptions = {}) {
  ...
  // Ensure compression tools exist, downloading if needed.
  // Get the compressor path to pass to compress-binary.mjs (DRY - single source of truth)
  const { compressorPath } = await ensureCompressionTools({
    silent: true,
    targetArch: arch,
  ...
  logger.substep('Compression tools ready')
