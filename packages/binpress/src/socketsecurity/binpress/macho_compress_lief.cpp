// ============================================================================
// macho_compress_lief.cpp — Mach-O compression via LIEF (cross-platform)
// ============================================================================
//
// WHAT THIS FILE DOES
// Compresses a binary and embeds it as a __SMOL segment inside a Mach-O
// stub executable using the LIEF C++ library. This enables building macOS
// self-extracting binaries from Linux or Windows build machines.
//
// WHY IT EXISTS
// Native Mach-O manipulation requires macOS APIs, but CI builds may run on
// Linux. LIEF provides cross-platform Mach-O read/write, so this C++ file
// uses LIEF to parse a stub, add the compressed data segment, and write
// the final executable — all without needing macOS.
// ============================================================================
...
#include "socketsecurity/binpress/smol_extract_lief.h"
}
...
int macho_compress_lief(const char* input_path,
  ...
  if (!input_path || !output_path) {
    ...
    // Check if input is already compressed (for automatic repacking).
    // Must check for __PRESSED_DATA section, not just SMOL segment existence.
    // node-smol binaries may have SMOL segment with only __SMOL_NODE_VER section.
    bool is_repack = false;
    char temp_extracted_path[4096];
    if (smol_has_pressed_data_macho_impl(input_path) == 1) {
        is_repack = true;
        printf("  Detected __PRESSED_DATA section - automatic repacking mode\n");

        // Extract original binary from SMOL segment to temp file
        snprintf(temp_extracted_path, sizeof(temp_extracted_path), "%s.extracted", output_path);
        printf("  Extracting original binary from SMOL segment...\n");

        if (smol_extract_binary_lief(input_path, temp_extracted_path) != 0) {
          fprintf(stderr, "Error: Failed to extract original binary from SMOL segment\n");
          return -1;
        }
    ...
    // If repacking, compress the extracted original; otherwise compress input as-is
    const char* compress_input = is_repack ? temp_extracted_path : input_path;
    compress_context_t context;
    if (compress_lief_common(compress_input, algorithm, &context, target, target_platform, target_arch, target_libc, node_version) != 0) {
      if (is_repack) {
        remove(temp_extracted_path);
      }
      return -1;
    }

    // Clean up temp extracted file after compression
    if (is_repack) {
      remove(temp_extracted_path);
    }
    ...
    // Sign with ad-hoc signature (macOS only, no-op on other platforms).
    printf("\nSigning binary with ad-hoc signature...\n");
    if (smol_codesign(output_path) != 0) {
      fprintf(stderr, "Warning: codesign failed (binary may not run on macOS)\n");
    }
