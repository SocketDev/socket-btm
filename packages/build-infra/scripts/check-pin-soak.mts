...
/**
 * One audited pin occurrence and its soak status.
 */
export interface PinFinding {
  ...
  lineNumber: number
  note: string
  status: 'in-soak' | 'soaked' | 'unannotated'
  surface: string
}
...
export function auditExternalToolsJson() {
  for (const file of walk(
    ...
    for (const [name, entry] of Object.entries(tools)) {
      ...
      const identifier = `${name}@${tool.version}`
      if (typeof tool.published !== 'string') {
        pushUnannotated(
          'external-tools',
          file,
          0,
      ...
      pushFromSoak('external-tools', file, 0, identifier, tool.published, {
...
export function auditGitmodules() {
  ...
  for (let i = 0, { length } = lines; i < length; i += 1) {
    ...
    recordPinWithSeparateAnnotation(file, lines, i, submoduleName, 'gitmodules')
  }
}
...
export function auditPnpmWorkspace() {
  ...
  for (let i = 0, { length } = lines; i < length; i += 1) {
    ...
    pushFromSoak('pnpm-workspace', file, i + 1, pkgLine, annotation.published, {
      hasSeparateAnnotation: true,
    })
  }
}

export function auditWorkflows() {
  // ^\s*-?\s*uses:\s*: YAML action key; ([^@\s]+): action ref; @([a-f0-9]{40}): SHA pin;
  // \s*#\s*[^(]+\((\d{4}-\d{2}-\d{2})\): date comment "(YYYY-MM-DD)"
  const usesPinRe =
  ...
  for (const file of walk(root, p => /\.ya?ml$/.test(p))) {
    ...
    for (let i = 0, { length } = lines; i < length; i += 1) {
      ...
      const [, actionRef, sha, publishedDate] = match
      if (
        actionRef === undefined ||
        sha === undefined ||
...
export function pushFromSoak(
  ...
  findings.push({
    ...
    hasSeparateAnnotation: opts.hasSeparateAnnotation,
    note: soak.soaked
...
export function pushUnannotated(
  surface: string,
  file: string,
  lineNumber: number,
  identifier: string,
...
export function recordPinWithSeparateAnnotation(
  ...
  for (let j = lineIndex - 1; j >= Math.max(0, lineIndex - 8); j -= 1) {
    ...
    if (annotation !== undefined) {
      pushFromSoak(surface, file, j + 1, identifier, annotation.published, {
        hasSeparateAnnotation: true,
      })
      return
    }
...
if (jsonOutput) {
  ...
  printBucket('! pin(s) in 7-day soak (annotated; allowed)', inSoak, {
    alwaysList: true,
  })
  printBucket(
