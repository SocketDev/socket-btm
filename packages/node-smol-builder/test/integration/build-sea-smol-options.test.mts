/**
 * @file Integration tests for node --build-sea with smol options (the Socket
 *   Security smol extensions to Node.js SEA). --build-sea performs FULL
 *   injection (blob generation + binary injection via embedded binject), not
 *   just blob generation. @see
...
// Skip if no binary is available; the LIEF check needs an async spawn, which
// can't resolve before vitest collects this describe.skipIf, so it runs in
// beforeAll and gates each test at runtime via skipUnlessLief() instead.
const skipTests = !finalBinaryPath || !existsSync(finalBinaryPath)

let hasLief = false

function skipUnlessLief(skip) {
  if (!hasLief) {
    skip()
  }
}
...
describe.skipIf(skipTests)('--build-sea with smol options', () => {
  ...
  describe('standard Node.js SEA options', () => {
    it('should build SEA with main and output', async ({ skip }) => {
      skipUnlessLief(skip)
    ...
    it('should build SEA with disableExperimentalSEAWarning', async ctx => {
      skipUnlessLief(ctx.skip)
    ...
    it('should build SEA with assets', async ({ skip }) => {
      skipUnlessLief(skip)
  ...
  describe('smol VFS options', () => {
    it('should build SEA with VFS in on-disk mode', async ({ skip }) => {
      skipUnlessLief(skip)
    ...
    it('should build SEA with VFS in in-memory mode', async ({ skip }) => {
      skipUnlessLief(skip)
    ...
    it('should build SEA with VFS custom prefix', async ({ skip }) => {
      skipUnlessLief(skip)
  ...
  describe('smol update options', () => {
    it('should build SEA with update configuration', async ({ skip }) => {
      skipUnlessLief(skip)
  ...
  describe('combined options', () => {
    it('should build SEA with both VFS and update config', async ({ skip }) => {
      skipUnlessLief(skip)
  ...
  describe('error handling', () => {
    it('should error with invalid config (missing main)', async ({ skip }) => {
      skipUnlessLief(skip)
    ...
    it('should error with nonexistent main file', async ({ skip }) => {
      skipUnlessLief(skip)
