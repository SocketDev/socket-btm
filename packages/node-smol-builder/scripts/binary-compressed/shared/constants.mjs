 ...
 * - Update config present flag (1 byte): 0=no config, 1=has config
 * - Update config binary (1112 bytes, if flag=1):
 *   - Magic (4 bytes): 0x55504446 ("UPDF")
 *   - Version (2 bytes): 1
 *   - Config data (1106 bytes): serialized update_config_t (validated at build time)
...
/**
 * Metadata header size (excluding magic marker, update config, and compressed data).
...
/**
 * Total header size without update config (excluding compressed data).
 * marker (32) + metadata (36) = 68 bytes
 */
export const TOTAL_HEADER_SIZE_WITHOUT_UPDATE_CONFIG =
  HEADER_SIZES.MAGIC_MARKER + METADATA_HEADER_SIZE

/**
 * Total header size with update config (excluding compressed data).
 * marker (32) + metadata (36) + update_config (1112) = 1180 bytes
 */
export const TOTAL_HEADER_SIZE_WITH_UPDATE_CONFIG =
  TOTAL_HEADER_SIZE_WITHOUT_UPDATE_CONFIG + HEADER_SIZES.UPDATE_CONFIG_BINARY

/**
 * Update config binary size.
 */
export const UPDATE_CONFIG_BINARY_SIZE = HEADER_SIZES.UPDATE_CONFIG_BINARY

/**
 * Update config magic number (ASCII "UPDF").
 */
export const UPDATE_CONFIG_MAGIC = 0x55_50_44_46
