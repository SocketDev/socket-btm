 ...
 *
 * Note: All platforms use LZFSE compression
 */
#define PLATFORM_METADATA_LEN 3

/**
 * Smol config format (appears after platform metadata):
 * - 1 byte: has_smol_config (0=no config, 1=has config)
 * - 1192 bytes: smol_config_binary (if has_smol_config=1)
 *
 * Smol config binary format (SMFG v2):
 * - Magic (4 bytes): 0x534D4647 ("SMFG")
 * - Version (2 bytes): 2
 * - Flags (2 bytes): prompt flag, promptDefault
 * - Numeric values (16 bytes): interval, notifyInterval
 * - String fields (1168 bytes): binname, command, url, tag, skipEnv, fakeArgvEnv, nodeVersion
 *
 * v2 adds 16 bytes for nodeVersion field (v1 was 1176 bytes, v2 is 1192 bytes).
 */
#define SMOL_CONFIG_FLAG_LEN 1
#define SMOL_CONFIG_BINARY_LEN 1192

/**
 * Total metadata header size (appears after magic marker, before compressed data).
 * Calculation: SIZE_HEADER_LEN + CACHE_KEY_LEN + PLATFORM_METADATA_LEN + SMOL_CONFIG_FLAG_LEN
 * = 16 + 16 + 3 + 1 = 36 bytes (without smol config data)
 * = 36 + 1192 = 1228 bytes (with smol config data, SMFG v2)
 *
 * Note: SMOL_CONFIG_FLAG_LEN is always present, SMOL_CONFIG_BINARY_LEN is conditional.
 */
#define METADATA_HEADER_LEN (SIZE_HEADER_LEN + CACHE_KEY_LEN + PLATFORM_METADATA_LEN + SMOL_CONFIG_FLAG_LEN)
