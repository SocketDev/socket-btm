 ...
 *
 * Usage:
 *   node download-binsuite-tools.mts [--tool=binpress|binflate|binject|lief] [--force]
 */
...
async function downloadLief(tag, outputPath) {
  ...
  try {
    ...
    await downloadReleaseAsset(tag, assetName, downloadPath, SOCKET_BTM_REPO)
    ...
    if (!checksumResult.valid) {
      ...
      throw new Error(
        'Archive checksum mismatch - file is corrupted.\n' +
          ...
          `Deleted ${downloadPath} to force re-download.`,
      )
    }
    ...
    const tarResult = await spawn(
      ...
      [
        '-xzf',
        WIN32 ? toUnixPath(downloadPath) : downloadPath,
        '-C',
        WIN32 ? toUnixPath(downloadDir) : downloadDir,
      ],
      { stdio: 'inherit' },
    )
    ...
    throw new Error(`Failed to download lief: ${error.message}`, {
      cause: error,
    })
  }
}
...
async function ensureTool(tool, force = false) {
  ...
  // Get latest release tag (cached for reuse below).
  const tag = await getLatestRelease(tool, SOCKET_BTM_REPO)

  // Construct expected binary path for existence check and LIEF downloads.
  ...
  if (!force && existsSync(expectedBinaryPath)) {
    ...
    if (existsSync(versionPath)) {
      ...
      if (currentVersion === tag) {
        logger.success(
          `${tool} ${currentVersion} already exists at ${expectedBinaryPath}`,
        )
        return true
      }
      logger.info(
        `${tool} version mismatch (have: ${currentVersion}, latest: ${tag}), re-downloading...`,
      )
    } else {
  ...
  const targetDir = path.join(downloadDir, tool, platformArch)
...
async function main() {
  ...
  for (const tool of toolsToEnsure) {
    try {
      ...
      logger.fail(`${tool}: ${error.message}`)
      // Log helpful messages if this is a transient GitHub/network error.
      await logTransientErrorHelp(error)
