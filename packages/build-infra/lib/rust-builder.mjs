...
export class RustBuilder {
  ...
  async installWasmTarget() {
    logger.substep('Installing wasm32-unknown-unknown target')
  ...
  async build({
    ...
    logger.substep('Building Rust to WASM with Cargo')
  ...
  async generateBindings({
    ...
    logger.substep('Generating JavaScript bindings with wasm-bindgen')
  ...
  async optimize(wasmFile, { flags = WASM_OPT_SIZE_FLAGS, output } = {}) {
    logger.substep('Optimizing WASM with wasm-opt')
