...
export async function downloadLibpq(options = {}) {
  ...
  const checksumResult = await verifyArchiveChecksum(downloadedArchive, assetName)
...
function getNodeOpenSSLPaths() {
  ...
  const opensslInclude = path.join(nodeUpstream, 'deps', 'openssl', 'openssl', 'include')
...
async function buildLibpq(libpqBuildDir) {
  ...
  const { includeDir: opensslInclude, libDir: opensslLib } = getNodeOpenSSLPaths()

  // Check if Node.js OpenSSL exists
  if (!existsSync(opensslInclude)) {
    logger.warn(`Node.js OpenSSL not found at ${opensslInclude}, will try system OpenSSL`)
  }
  ...
  if (process.platform === 'darwin' && CROSS_COMPILE) {
    const targetTriple = TARGET_ARCH === 'x64' ? 'x86_64-apple-darwin' : 'aarch64-apple-darwin'
...
async function copyDistributionFiles(libpqBuildDir) {
  ...
  const libpqSrc = path.join(libpqBuildDir, 'src', 'interfaces', 'libpq', 'libpq.a')
  ...
  const commonLibSrc = path.join(libpqBuildDir, 'src', 'common', 'libpgcommon.a')
  ...
  const postgresExtHeader = path.join(postgresUpstream, 'src', 'include', 'postgres_ext.h')
  if (existsSync(postgresExtHeader)) {
    await fs.copyFile(postgresExtHeader, path.join(headersDst, 'postgres_ext.h'))
  }

  // Copy pg_config.h from build directory (generated by configure)
  const pgConfigHeader = path.join(libpqBuildDir, 'src', 'include', 'pg_config.h')
  if (existsSync(pgConfigHeader)) {
    await fs.copyFile(pgConfigHeader, path.join(headersDst, 'pg_config.h'))
  }

  // Copy pg_config_ext.h
  const pgConfigExtHeader = path.join(libpqBuildDir, 'src', 'include', 'pg_config_ext.h')
  if (existsSync(pgConfigExtHeader)) {
    await fs.copyFile(pgConfigExtHeader, path.join(headersDst, 'pg_config_ext.h'))
  }
...
async function main() {
  try {
    ...
    const libpqLibPath = path.join(libpqBuildDir, 'src', 'interfaces', 'libpq', 'libpq.a')
    ...
    // Check if PostgreSQL submodule is initialized.
    const postgresConfigure = path.join(postgresUpstream, 'configure')
    const isPostgresBuild = existsSync(postgresConfigure)

    if (!isPostgresBuild) {
      // Not building libpq itself - ensure prebuilt is available.
      logger.info('PostgreSQL submodule not initialized, using prebuilt...')
