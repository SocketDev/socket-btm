...
// Read a JS string argument into a fixed buffer. The first length-only call is
// important: napi_get_value_string_utf8 otherwise truncates a long value and
// still returns napi_ok. A credential must either round-trip byte-for-byte or
// fail; silently storing a prefix would be data loss.
bool readStringArg(napi_env env, napi_value value, char* out, size_t cap) {
  size_t byteLength = 0;
  if (napi_get_value_string_utf8(env, value, nullptr, 0, &byteLength) !=
          napi_ok ||
      byteLength >= cap) {
    return false;
  }
  size_t written = 0;
  if (napi_get_value_string_utf8(env, value, out, cap, &written) != napi_ok ||
      written != byteLength) {
    return false;
  }
  // The C keystore API takes NUL-terminated strings. Reject embedded NULs so a
  // JavaScript value cannot be accepted under one identity and stored under a
  // shorter C identity.
  return std::strlen(out) == written;
}
...
napi_value Set(napi_env env, napi_callback_info info) {
  ...
  if (argc < 3 || !readStringArg(env, argv[0], service, sizeof(service)) ||
    ...
    napi_throw_type_error(env, nullptr,
                          ...
                          "within the supported byte limits and without NULs");
    return nullptr;
  }
