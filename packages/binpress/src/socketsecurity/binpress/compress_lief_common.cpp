// ============================================================================
// compress_lief_common.cpp — Shared compression pipeline (steps 1-3)
// ============================================================================
//
// WHAT THIS FILE DOES
// Implements the compression steps shared by all formats:
//   1. Select the right decompressor stub for the target platform/arch
//   2. Read the input binary and compress it with ZSTD
//   3. Build the SMOL section data (magic marker + sizes + platform metadata
//      + compressed bytes + optional self-update config)
// Also provides helpers for output directory creation and file verification.
//
// WHY IT EXISTS
// Without this shared implementation, each format-specific compressor
// (ELF, PE, Mach-O) would duplicate hundreds of lines of identical
// compression and metadata-building logic. This file is the single source
// of truth for steps 1-3; format-specific files only handle step 4
// (inserting the section into the executable structure).
// ============================================================================
...
int compress_lief_common(
  ...
  // Algorithm is currently always ZSTD; accept the parameter for a future
  // pluggable backend and silence unused-parameter until that lands.
  (void)algorithm;
  ...
  int compress_result = compress_buffer(
  ...
  if (compress_result != COMPRESS_OK) {
    fprintf(stderr, "Error: Compression failed (code: %d = %s)\n",
            compress_result, compress_error_string(compress_result));
  ...
  // Step 4: Extract node version and create SMFG config.
  // This embeds the node version in the compressed binary so binject can
  // verify version compatibility when generating SEA blobs.
  uint8_t* smfg_config_binary = nullptr;
  char* detected_version = nullptr;

  // Use provided version if available, otherwise try to detect.
  if (node_version && node_version[0] != '\0') {
    printf("  Using provided Node.js version: %s\n", node_version);
    smol_update_config_t config;
    smol_config_init(&config);
    config.node_version = node_version;
    smfg_config_binary = serialize_smol_config(&config);
  } else {
    detected_version = smol_extract_node_version(input_path);
    if (detected_version) {
      printf("  Detected Node.js version: %s\n", detected_version);
      smol_update_config_t config;
      smol_config_init(&config);
      config.node_version = detected_version;
      smfg_config_binary = serialize_smol_config(&config);
      free(detected_version);
    }
  }

  // Build section data with optional SMFG config.
  int result = build_smol_section_from_compressed_with_config(
...
/**
 * Build SMOL section from pre-compressed data with optional SMFG config.
 ...
 * @param smfg_config_binary Optional 1192-byte SMFG config binary (or NULL)
 * @param section Output section data
 * @return 0 on success, error code otherwise
 */
int build_smol_section_from_compressed_with_config(
    ...
    const uint8_t* smfg_config_binary,
...
/**
 * Build SMOL section from pre-compressed data (no config).
...
int build_smol_section_from_compressed(
  ...
  return build_smol_section_from_compressed_with_config(
      compressed_data, compressed_size, uncompressed_size,
      platform_override, arch_override, libc_override,
      nullptr, section);
}
...
int compress_data_only(
  ...
  printf("  Input: %s\n", input_path);
  printf("  Output: %s\n", output_data_path);
  printf("  Algorithm: ZSTD\n");

  // Step 1-3: Use common compression logic to build section data
  compress_context_t context;
  if (compress_lief_common(input_path, algorithm, &context, target, target_platform, target_arch, target_libc, node_version) != 0) {
    return -1;
  }
