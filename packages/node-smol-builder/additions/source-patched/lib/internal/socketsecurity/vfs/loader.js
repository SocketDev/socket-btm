...
function findVFSEntry(vfsPath, debug) {
  ...
  // Fall back to implicit-directory detection. TAR archives routinely
  // omit explicit directory entries — `foo/bar.js` lives in implicit
  // `foo/`. realpathSync, readlinkSync, and other callers need to
  // distinguish "nonexistent" from "implicit directory". Synthesize a
  // minimal directory entry so the caller can proceed. existsInVFS /
  // statFromVFS already have this logic; findVFSEntry was the outlier.
  if (isDirectory(vfs, withoutSlash)) {
    return {
      __proto__: null,
      vfsKey: withoutSlash,
      entry: { __proto__: null, type: 'directory', implicit: true },
    }
  }
