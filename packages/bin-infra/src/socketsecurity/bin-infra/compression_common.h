...
static inline const char* compress_error_string(int error_code) {
    switch (error_code) {
        ...
        case COMPRESS_ERROR_DECOMPRESS_FAILED:
            return "DECOMPRESS_FAILED: data may be corrupted or not LZFSE-compressed";
        case COMPRESS_ERROR_UNSUPPORTED_ALGORITHM:
            return "UNSUPPORTED_ALGORITHM: only LZFSE (0) is supported";
