...
const { FsReadFileSync, ProcessEnv } = require('internal/socketsecurity/safe-references');
const { SQLResult } = require('internal/socketsecurity/sql/result');
const { Transaction, Savepoint } = require('internal/socketsecurity/sql/transaction');
const {
  SQLError,
  PostgresError,
  SQLiteError,
...
function detectAdapter(urlOrOptions) {
  if (typeof urlOrOptions === 'string') {
    ...
    if (StringPrototypeStartsWith(url, 'postgres://') ||
...
class SQLQuery {
  ...
  stream(batchSize = 100) {
    ...
    if (!NumberIsInteger(batchSize) || batchSize < 1) {
      throw new ERR_INVALID_ARG_VALUE('batchSize', batchSize, 'must be a positive integer');
    }
    ...
    return {
      ...
      [SymbolAsyncIterator]: async function* streamIterator() {
        ...
        try {
          let batch;
          while ((batch = await adapter.fetchCursor(cursor, batchSize)).length > 0) {
...
class SQL {
  ...
  constructor(urlOrOptions, options) {
    if (urlOrOptions === undefined) {
      ...
      urlOrOptions = ProcessEnv.POSTGRES_URL ||
    ...
    let finalOptions;
    if (typeof urlOrOptions === 'string') {
      finalOptions = ObjectAssign({ __proto__: null, url: urlOrOptions }, options);
    } else {
    ...
    if (adapterType === 'postgres') {
      ...
      throw new ERR_INVALID_ARG_VALUE('adapter', adapterType, 'must be "postgres" or "sqlite"');
    }
    ...
    for (let i = 0, rowsLen = rows.length; i < rowsLen; i++) {
      ...
      for (let j = 0; j < colsLen; j++) {
        ...
        if (!ObjectHasOwn(row, col)) {
          throw new ERR_INVALID_ARG_VALUE(`rows[${i}]`, row, `missing column '${col}'`);
        }
      }
    }

    // Build placeholders and values using shared helper
    const { rowPlaceholders, allValues } = buildRowPlaceholders(rows, cols, paramStyle);

    const colNames = ArrayPrototypeJoin(ArrayPrototypeMap(cols, escapeIdentifier), ', ');
  ...
  async upsert(table, row, options) {
    ...
    const { conflictColumns, updateColumns, returning } = { __proto__: null, ...options };
    if (!ArrayIsArray(conflictColumns) || conflictColumns.length === 0) {
      throw new ERR_INVALID_ARG_VALUE('options.conflictColumns', conflictColumns, 'must be a non-empty array');
    }
    ...
    const updateCols = updateColumns ?? ArrayPrototypeFilter(cols, c => !SetPrototypeHas(conflictSet, c));
    ...
    const conflictCols = ArrayPrototypeJoin(ArrayPrototypeMap(conflictColumns, escapeIdentifier), ', ');
    const escapedUpdateCols = ArrayPrototypeMap(updateCols, escapeIdentifier);

    if (paramStyle === '$') {
      // PostgreSQL: ON CONFLICT ... DO UPDATE SET
      const updates = ArrayPrototypeMap(escapedUpdateCols, col => `${col} = EXCLUDED.${col}`);
  ...
  async updateMany(table, ids, values, idColumn = 'id') {
    ...
    if (cols.length === 0) {
      throw new ERR_INVALID_ARG_VALUE('values', values, 'must have at least one column');
    }
    ...
    for (let i = 0, colsLen = cols.length; i < colsLen; i++) {
      const col = cols[i];
      if (paramStyle === '$') {
        ArrayPrototypePush(setClauses, `${escapeIdentifier(col)} = $${paramIndex++}`);
      } else {
  ...
  static json(value) {
    ...
    const escaped = StringPrototypeReplace(jsonStr, SQL_SINGLE_QUOTE_REGEX, "''");
...
class ReservedConnection {
  ...
  constructor(adapter, conn) {
    ...
    const handler = {
      ...
      get(target, prop, receiver) {
        // Use ObjectHasOwn to prevent prototype pollution attacks.
        if (ObjectHasOwn(self, prop) || ObjectHasOwn(ReservedConnection.prototype, prop)) {
          const value = self[prop];
          if (typeof value === 'function') {
            return FunctionPrototypeBind(value, self);
          }
...
function buildRowPlaceholders(rows, cols, paramStyle) {
  ...
  if (paramStyle === '$') {
    ...
    for (let i = 0, rowsLen = rows.length; i < rowsLen; i++) {
      ...
      ArrayPrototypePush(rowPlaceholders, `(${ArrayPrototypeJoin(placeholders, ', ')})`);
    }
  } else {
