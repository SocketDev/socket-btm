// max-file-lines: builder-pipeline -- single builder pipeline (fetch → patch → build → package); phases share local state and early-return across the full sequence — splitting would fracture control flow
/**
...
// The bin-stub-builder Makefiles reference curl-builder's build output.
...
/**
 * Verify curl archive integrity using SHA256 checksum.
...
async function main() {
  try {
    ...
    const { buildDir, curlBuildDir, mbedtlsBuildDir } =
      getBuildDirs(platformArch)
...
// because that would incorrectly trigger main() when other packages import this module
// from their own build.mts scripts.
const isMainModule =
