  ...
  await createCheckpoint(
    ...
    async () => {
      ...
      if (typeof Yoga.Node?.create !== 'function') {
        throw new Error('Sync wrapper missing Node.create() - wrapper not applied')
      }

      if (typeof Yoga.Config?.create !== 'function') {
        throw new Error('Sync wrapper missing Config.create() - wrapper not applied')
      }
      ...
      for (const name of criticalEnumNames) {
        ...
        if (typeof Yoga[name] !== 'number') {
          throw new Error(`Sync wrapper missing ${name} constant - YGEnums not applied`)
        }
      ...
      logger.substep(`Validated ${allEnumNames.length} enum constants against YGEnums.mjs`)
    },
    {
...
function replaceWrapperExport(code, replacement, isEsm) {
  ...
  if (isEsm) {
    // Find: export default yogaPromise;
    walk.simple(ast, {
      ExportDefaultDeclaration(node) {
        if (
          node.declaration.type === 'Identifier' &&
          node.declaration.name.endsWith('Promise')
        ) {
    ...
    // Find: module.exports = yogaPromise;
    walk.simple(ast, {
      ExpressionStatement(node) {
        if (
          ...
          node.expression.right.name.endsWith('Promise')
        ) {
          s.overwrite(node.start, node.end, replacement)
          replaced = true
        }
  ...
  if (!replaced) {
    throw new Error(`Failed to find ${isEsm ? 'export default' : 'module.exports'} in wrapper`)
  }
...
export default Yoga;`
  ...
  // Replace the export default yogaPromise; with our wrapped version.
...
// Apply wrapper to get official yoga-layout API.
// Note: Despite the name, yogaPromise is synchronous in dev mode (-sWASM_ASYNC_COMPILATION=0).
module.exports = wrapAssembly(yogaPromise);`

  // Replace the module.exports = yogaPromise; with our wrapped version.
