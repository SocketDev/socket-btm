...
// Optional extractDir override (set before first getExtractionProvider call)
let _extractDir

/**
 * Set the extraction directory override.
 * Must be called before the extraction provider is initialized.
 * @param {string} dir - Directory to extract files to
 */
function setExtractDir(dir) {
  if (_extractionProvider) {
    throw new ErrorConstructor(
      'VFS Error: Cannot set extractDir after extraction provider is initialized\n' +
      '  Call setExtractDir() before any mount() or mountSync() calls',
    )
  }
  _extractDir = dir
}
...
function getExtractionProvider() {
  ...
  const options = _extractDir
    ? { __proto__: null, extractDir: _extractDir }
    : undefined

  _extractionProvider = createExtractionProvider(mode, options)
 ...
 * Returns info about the cache mode, directory, and extraction count.
