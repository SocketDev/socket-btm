...
const STOP_PATTERNS: ReadonlyArray<{ label: string; regex: RegExp }> = [
  ...
  },
  {
    ...
    regex:
      /\b(waiting\s+(for|on)\s+(next|the|your)\s+(direction|call|input|decision|word|go-ahead|signal)|wait(ing)?\s+for\s+(you|your)\s+to\s+(choose|decide|direct|pick|say|tell))\b/i,
  },
  {
