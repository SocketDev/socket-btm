...
describe('node:smol-sql TypeScript definitions', () => {
  const typingsPath = join(__dirname, '../../additions/source-patched/typings/node_smol-sql.d.ts')
  let content: string
  ...
  describe('Core SQL classes', () => {
    it('exports SQL main class', () => {
      ...
      expect(content).toContain('constructor(urlOrOptions?: string | SQLOptions')
    })
  ...
  describe('Error classes', () => {
    ...
    it('exports connection state errors', () => {
      expect(content).toContain('export class SQLConnectionClosedError extends SQLError')
      expect(content).toContain('export class SQLTransactionCommittedError extends SQLError')
      expect(content).toContain('export class SQLTransactionRolledBackError extends SQLError')
    })
  })
  ...
  describe('Error type guards', () => {
    it('exports database-agnostic error guards', () => {
      expect(content).toContain('export function isUniqueViolation(err: Error): boolean')
      expect(content).toContain('export function isForeignKeyViolation(err: Error): boolean')
      expect(content).toContain('export function isNotNullViolation(err: Error): boolean')
      expect(content).toContain('export function isCheckViolation(err: Error): boolean')
    })

    it('exports connection and query error guards', () => {
      expect(content).toContain('export function isConnectionError(err: Error): boolean')
      expect(content).toContain('export function isSyntaxError(err: Error): boolean')
      expect(content).toContain('export function isUndefinedTable(err: Error): boolean')
    })
  })
  ...
  describe('SQL builder methods', () => {
    it('provides identifier() for safe table/column names', () => {
      expect(content).toContain('identifier(name: string | string[]): SQLFragment')
    })
