...
# Downloaded curl uses {platform}/curl/assets path structure from downloadSocketBtmRelease.
CURL_DOWNLOADED_DIR = $(CURL_BUILDER_ROOT)/build/downloaded/curl/$(CURL_PLATFORM_ARCH)/curl/assets
CURL_BUILD_DIR = $(CURL_BUILDER_ROOT)/build/$(BUILD_MODE)/out/Final/curl/dist

# Prefer locally built curl over downloaded.
# Don't check during 'clean' target - only check when actually building.
# curl is REQUIRED - stubs need it for HTTPS update checking.
ifneq ($(MAKECMDGOALS),clean)
    ifneq (,$(wildcard $(CURL_BUILD_DIR)/libcurl.a))
        ...
        # curl not found - this is an error.
        # The build.mjs script should have downloaded curl before invoking make.
        $(error curl libraries not found at $(CURL_BUILD_DIR) or $(CURL_DOWNLOADED_DIR). Run 'pnpm run build' to download curl first.)
    endif
else
