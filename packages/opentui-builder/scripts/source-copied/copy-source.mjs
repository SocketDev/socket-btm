...
import { UPSTREAM_PATH, getSharedBuildPaths } from '../paths.mjs'
...
export async function copySource() {
  ...
  const zigSourceDir = path.join(UPSTREAM_PATH, 'packages', 'core', 'src', 'zig')
  ...
  await createCheckpoint(
    ...
    async () => {
      ...
      if (!content.includes('pub fn build')) {
        throw new Error('Invalid OpenTUI source: missing build.zig with build function')
      }
    },
