 ...
 */
function runNpmPkgFix(pkgPath) {
  const pkgDir = path.dirname(pkgPath)
  try {
    execSync('npm pkg fix', { cwd: pkgDir, stdio: 'pipe' })
    return true
  } catch (e) {
    console.error(`Error running npm pkg fix in ${pkgDir}:`, e.message)
    return false
  }
}
...
function main() {
  ...
  for (const { name, path: pkgPath } of packages) {
    ...
    // Run npm pkg fix.
    const wasFixed = runNpmPkgFix(pkgPath)
    if (wasFixed) {
      fixedCount++
