...
declare module 'node:smol-http' {
  ...
  export type RouteHandler = (
    ...
    server: Server,
  ) => Response | Promise<Response> | object | string | null
  ...
  export interface MethodHandlers {
    ...
    /** RFC 10008 QUERY — a safe, idempotent method with a request body. */
    QUERY?: RouteHandler | undefined
  ...
  export interface WebSocketHandlers<T = unknown> {
    ...
    message?:
      | ((ws: ServerWebSocket<T>, message: string | Buffer) => void)
      | undefined
    /** Called when connection closes */
    close?:
      | ((ws: ServerWebSocket<T>, code: number, reason: string) => void)
      | undefined
  ...
  export interface ServeOptions<T = unknown> {
    ...
    /**
     * Only accept IPv6 connections (IPV6_V6ONLY socket option).
     * When false (default), dual-stack mode accepts both IPv4 and IPv6.
     */
    ipv6Only?: boolean | undefined
    /**
     * Allow multiple processes to bind the same port (SO_REUSEPORT).
     * The kernel load-balances connections across processes. Linux only.
     * Use this for multi-process SEA binaries instead of node:cluster.
     */
    ...
    fetch: (
      request: ServeRequest,
      server: Server,
    ) => Response | Promise<Response> | object | string | undefined
  }
  ...
  /**
   * The llhttp-derived method list — the same source `node:http.METHODS`
   * reads, so it includes QUERY. Lets a consumer feature-detect QUERY
   * support instead of hard-coding a method list of its own.
   */
  ...
  /**
   * Create a cache key from a request method + URL. Pass `body` for a
   * method whose response varies with the request body (RFC 10008 §2.7 —
   * QUERY is the motivating case) so two requests with different bodies
   * fold in a SHA-256 digest and never collide on the same key.
   */
   ...
   */
  export function subsetPackument(packument: object, range: string): object
