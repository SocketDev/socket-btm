// Tests for no-structured-clone-prefer-json-guard.
...
const WITH_HOOK_ALLOW = `export function clone(v: unknown) {
  return structuredClone(v) // socket-hook: allow structured-clone
}
`
...
describe('no-structured-clone-prefer-json-guard', () => {
  ...
  test('passes when socket-hook allow marker is present', async () => {
    const result = await runHook({
      tool_name: 'Write',
