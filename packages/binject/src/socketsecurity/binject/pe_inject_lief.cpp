...
extern "C" int binject_pe_lief_batch(
  ...
  if (!executable || !output) {
    ...
    // Inject VFS config resource if provided (VFS_CONFIG_SIZE bytes SVFG format)
    if (vfs_config_data) {
      printf("Injecting VFS config resource: %s (%d bytes)\n", PE_RESOURCE_SMOL_VFS_CONFIG, VFS_CONFIG_SIZE);
      int result = inject_pe_resource(binary.get(), PE_RESOURCE_SMOL_VFS_CONFIG,
                                       vfs_config_data, VFS_CONFIG_SIZE, true);
      if (result != BINJECT_OK) {
        return result;
      }
    }
