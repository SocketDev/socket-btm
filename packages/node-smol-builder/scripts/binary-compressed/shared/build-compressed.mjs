...
const BINJECT_OUT_DIR = path.join(BINJECT_DIR, 'bin')
...
async function ensureCompressionToolsBuilt(platform) {
  ...
  logger.substep(`Directory: ${path.relative(PACKAGE_ROOT, BINPRESS_DIR)}`)
  logger.logNewline()
  ...
  if (exitCode !== 0) {
    printError(
      ...
      `Failed to build compression tools (exit code: ${exitCode})`,
      [
...
async function ensureBinjectBuilt() {
  ...
  const binjectBinaryPath = path.join(BINJECT_OUT_DIR, 'binject')
  ...
  logger.substep('Command: make all')
  ...
  let result
  try {
    result = await spawn(makePath, ['all'], {
      cwd: BINJECT_DIR,
      stdio: 'inherit',
      shell: WIN32,
    })
