...
description: Squashes all commits to single "Initial commit" with backup and integrity verification. Triggers when user mentions "clean history", "squash commits", or "fresh start".
user-invocable: true
...
- Verify code integrity shows zero differences before force push
- Obtain explicit user confirmation before force push
- Operate only on main branch

**Actions:**
- Create timestamped backup branch (e.g., backup-20260206-120000)
- Use `git reset --soft` (preserves code, resets commits)
- Verify code integrity with `git diff` (must be empty)
- Request explicit yes/no confirmation before force push
- Store backup branch name for rollback instructions
</constraints>
