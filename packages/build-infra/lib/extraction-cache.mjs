 ...
 */
export function computeSourceHash(sourcePaths) {
  ...
  for (const sourcePath of sortedPaths) {
    // Handle virtual platform metadata paths (not actual files)
    if (sourcePath.startsWith('__PLATFORM_METADATA__:')) {
      const metadata = sourcePath.substring('__PLATFORM_METADATA__:'.length)
    ...
    try {
      ...
      if (err.code === 'ENOENT') {
        // File deleted - skip it entirely (don't include in hash)
        // The absence of the file hash naturally invalidates the cache
      } else {
