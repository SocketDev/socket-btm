...
int smol_build_section_data(
    ...
    /* Write platform metadata (5 bytes: platform, arch, libc, compression_algorithm, reserved). */
    section->data[offset++] = platform_byte;
    section->data[offset++] = arch_byte;
    section->data[offset++] = libc_byte;
    section->data[offset++] = compression_algorithm;
    section->data[offset++] = 0;  /* Reserved byte (currently unused). */
 ...
 *
 * Uses compiler macros to determine platform, architecture, libc, and
 * compression algorithm. Provides DRY implementation for all tools.
 */
void smol_detect_platform_metadata(
    ...
    *arch_byte = ARCH_X64;
    *libc_byte = LIBC_NA;
    *compression_algorithm = SMOL_COMPRESSION_LZFSE;
    ...
    *compression_algorithm = SMOL_COMPRESSION_LZMA;
    ...
    *libc_byte = LIBC_NA;
    *compression_algorithm = SMOL_COMPRESSION_LZMS;
#endif
