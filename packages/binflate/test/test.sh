...
if [ "$PLATFORM" = "Darwin" ] || [ "$PLATFORM" = "Linux" ]; then
    COMPRESSOR="$BINPRESS_DIR/build/$BUILD_MODE/out/Final/binpress"
    DECOMPRESSOR="$PACKAGE_DIR/build/$BUILD_MODE/out/Final/binflate"
else
    COMPRESSOR="$BINPRESS_DIR/build/$BUILD_MODE/out/Final/binpress.exe"
    DECOMPRESSOR="$PACKAGE_DIR/build/$BUILD_MODE/out/Final/binflate.exe"
fi
...
# Test: Binary contains decompression library symbols (skip for stripped static binaries)
if [ "$PLATFORM" = "Darwin" ]; then
    ...
    # Linux: Static builds are stripped in production, so symbols/linkage checks won't work
    # Check if binary is stripped (no symbols) or statically linked
    if nm "$DECOMPRESSOR" 2>/dev/null | grep -q .; then
        # Has symbols - check for decompression symbols
        run_test "binary_has_decompression_symbols" "nm '$DECOMPRESSOR' 2>/dev/null | grep -q -i 'decompress\\|lzma'"
    elif ldd "$DECOMPRESSOR" 2>&1 | grep -q "not a dynamic executable"; then
        # Statically linked and stripped - skip symbol check (production build)
        echo "  ${YELLOW}⊘${RESET} binary_has_decompression_symbols (skipped: stripped static binary)"
    else
        # Dynamically linked - check for liblzma
        run_test "binary_has_decompression_symbols" "ldd '$DECOMPRESSOR' | grep -q 'liblzma'"
    fi
fi
