...
const SMOL_CONFIG_VERSION = 1
const SMOL_CONFIG_BINARY_SIZE = 1176
 ...
 * @returns {Buffer} - Binary buffer (1176 bytes)
...
export function serializeUpdateConfig(config) {
  ...
  // Wrap flat config in 'update' key for backward compatibility.
  return serializeSmolConfig({ update: normalized })
}

// Backward compatibility: export old error name
export const UpdateConfigValidationError = SmolConfigValidationError
