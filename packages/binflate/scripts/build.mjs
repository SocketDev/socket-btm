...
import { getFileSize } from '@socketsecurity/build-infra/build-helpers'
import {
  createCheckpoint,
  shouldRun,
} from '@socketsecurity/build-infra/checkpoint-manager'
...
async function main() {
  try {
    ...
    if (!(await shouldRun(buildDir, '', 'finalized', forceRebuild))) {
      logger.success('binflate already built (checkpoint exists)')
      return
    }
    ...
    const binaryName =
      process.platform === 'win32' ? 'binflate.exe' : 'binflate'
    const binaryPath = path.join(packageRoot, 'out', binaryName)
    ...
    if (existsSync(binaryPath)) {
      ...
      await createCheckpoint(
        ...
        async () => {
          ...
          if (!result.stdout.includes('binflate')) {
            throw new Error(
              `Binary --version output missing 'binflate': ${result.stdout}`,
            )
          }
        ...
        },
        {
          ...
          artifactPath: path.join(packageRoot, 'out'),
