...
async function main() {
  ...
  // Verify ONNX Runtime source submodule.
  const onnxSourcePath = SUBMODULE_PATH
  if (!existsSync(onnxSourcePath)) {
    logger.fail('ONNX Runtime source submodule not found')
    logger.log('')
  ...
  if (result.code === 0) {
    ...
    logger.log('')
    logger.log('✓ Model inference validated')
    logger.log('✓ Operator compatibility verified')
    logger.log('')
  } else {
