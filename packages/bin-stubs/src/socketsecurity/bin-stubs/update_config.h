 ...
 * Supports both v1 (1176 bytes) and v2 (1192 bytes) formats.
 * Returns 0 on success, -1 on error.
 *
 * Binary format:
 ...
 * - String fields v1 (1152 bytes): binname, command, url, tag, skipEnv, fakeArgvEnv
 * - String fields v2 (1168 bytes): + nodeVersion (16 bytes)
 */
static int update_config_from_binary(update_config_t *config, const uint8_t *data, size_t size) {
    ...
    /* Accept v1 (1176 bytes) or v2 (1192 bytes) sizes. */
    if (size != 1176 && size != 1192) {
        return -1;
    }
    ...
    /* Read node_version for v2 format. */
    if (version >= 2 && size >= 1192) {
        ptr = read_string_field(ptr, config->node_version, UPDATE_CONFIG_MAX_NODE_VERSION_LEN, 0);
        if (!ptr) return -1;
    } else {
        config->node_version[0] = '\0';
    }
