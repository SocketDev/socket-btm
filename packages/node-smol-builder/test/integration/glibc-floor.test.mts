...
const GLIBC_FLOOR = GLIBC_FLOOR_RAW
  ? parseVersionTuple(GLIBC_FLOOR_RAW)
  : undefined

describe.skipIf(process.platform !== 'linux' || !GLIBC_FLOOR_RAW)(
  ...
  () => {
    ...
    test(`built binary does not import GLIBC_ > ${GLIBC_FLOOR_RAW}`, async () => {
      ...
      expect(result.code, `objdump failed: ${result.stderr?.toString()}`).toBe(
        0,
      )
