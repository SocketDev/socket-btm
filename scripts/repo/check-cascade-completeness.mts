 ...
 *      discovered dependency against: A.
 *      `scripts/repo/validate-cache-versions.mts` CASCADE_RULES B. The
 *      consuming workflow's cache-key composition Reports every path that
 *      exists on disk AND is referenced by a builder AND has no matching
 *      cascade rule OR workflow hash. This catches the shape that was the bulk
 *      of R18-R27 scope creep — R18 missed `build-infra/wasm-synced/`, R19
 *      missed `curl-builder/ {docker,lib,scripts}/`, R20 missed
 ...
 *      scripts/repo/check-cascade-completeness.mts node
 *      scripts/repo/check-cascade-completeness.mts --explain node
 *      scripts/repo/check-cascade-completeness.mts --json Allowlist at
...
export function printFinding(f: Finding, options: Options): void {
  ...
  if (opts.explain) {
    ...
    if (f.gap === 'cascade-rule') {
      logger.log(
        `  Fix: add to CASCADE_RULES in scripts/repo/validate-cache-versions.mts`,
      )
      logger.log(
