...
static int dlx_get_cache_base_dir(char *buf, size_t size) {
    ...
    // Priority 4: Platform-specific user-level cache directories.
#if defined(_WIN32)
    // Windows: %LOCALAPPDATA%\.socket\_dlx (per-user, not shared)
    const char *localappdata = getenv("LOCALAPPDATA");
    if (localappdata && localappdata[0] != '\0') {
        int written = snprintf(buf, size, "%s\\.socket\\_dlx", localappdata);
        if (written > 0 && (size_t)written < size) {
            return 0;
        }
    }
    // Windows fallback: %APPDATA%\.socket\_dlx
    const char *appdata = getenv("APPDATA");
    if (appdata && appdata[0] != '\0') {
        int written = snprintf(buf, size, "%s\\.socket\\_dlx", appdata);
        if (written > 0 && (size_t)written < size) {
            return 0;
        }
    }
    ...
    const char *tmpdir_user = getenv("TMPDIR");
    if (tmpdir_user && tmpdir_user[0] != '\0') {
        int written = snprintf(buf, size, "%s.socket/_dlx", tmpdir_user);
        if (written > 0 && (size_t)written < size) {
            return 0;
        }
    }
#elif defined(__linux__)
    // Linux: $XDG_CACHE_HOME/.socket/_dlx (per-user XDG cache)
    const char *xdg_cache = getenv("XDG_CACHE_HOME");
    if (xdg_cache && xdg_cache[0] != '\0') {
        int written = snprintf(buf, size, "%s/.socket/_dlx", xdg_cache);
        if (written > 0 && (size_t)written < size) {
            return 0;
        }
    }
    ...
    // Priority 5: Last resort shared temp directory.
...
static const char* dlx_get_libc(void) {
    ...
    // Uses spawn_command() for defense-in-depth - no shell execution.
    // Note: In highly restricted containers or seccomp-filtered environments,
    // /usr/bin/ldd may not exist. The fallback mechanisms below
    // (musl loader path checks, default to glibc) handle this.
    const char* ldd_args[] = {"/usr/bin/ldd", "--version", NULL};
    char* output = spawn_command("/usr/bin/ldd", ldd_args, 256);
    if (output) {
        ...
        if (strstr(output, "musl")) {
            free(output);
            return "musl";
        }
        if (strstr(output, "glibc") || strstr(output, "gnu")) {
            free(output);
            return "glibc";
        }
 ...
 */
/**
 * JSON-escape a string and append to buffer.
 */
static size_t json_escape_to_buffer(char *buf, size_t buf_size, size_t offset, const char *str) {
    size_t remaining = buf_size - offset;
    size_t written = 0;

    for (const char *p = str; *p && remaining > 1; p++) {
        const char *esc = NULL;
        size_t esc_len = 0;
        ...
        switch (*p) {
            ...
            default:
                if (*p < 32) {
                    if (remaining > 6) {
                        int n = snprintf(buf + offset + written, remaining, "\\u%04x", (unsigned char)*p);
                        if (n > 0) {
                            written += n;
                            remaining -= n;
                        }
                ...
                buf[offset + written++] = *p;
                remaining--;
                continue;
        }

        if (esc && remaining > esc_len) {
            memcpy(buf + offset + written, esc, esc_len);
            written += esc_len;
            remaining -= esc_len;
        }
    }
...
static int dlx_write_metadata(const char *entry_dir, const char *cache_key,
    ...
    /* Build JSON in memory for atomic write. */
    char *json_buf = malloc(8192);
    if (!json_buf) {
        fprintf(stderr, "Error: Out of memory allocating JSON buffer\n");
        return -1;
    }
    ...
    n = snprintf(json_buf + offset, 8192 - offset,
                 "{\n"
                 "  \"version\": \"1.0.0\",\n"
                 "  \"cache_key\": \"%s\",\n"
                 "  \"timestamp\": %lld,\n"
                 ...
                 "    \"type\": \"extract\",\n"
                 "    \"path\": \"",
                 cache_key, timestamp, integrity, (unsigned long)size);
    if (n < 0 || (size_t)n >= 8192 - offset) {
        free(json_buf);
        return -1;
    }
    offset += n;

    /* JSON-escape exe_path. */
    offset += json_escape_to_buffer(json_buf, 8192, offset, exe_path);
    ...
    if (update_check && update_check->latest_known) {
        n = snprintf(json_buf + offset, 8192 - offset,
                     ...
                     "    \"latest_known\": \"%s\"\n"
                     "  }\n",
                     update_check->last_check, update_check->last_notification,
    ...
    /* Write JSON atomically using cross-platform helper with detailed error logging. */
    int result = write_file_atomically(metadata_path, (const unsigned char *)json_buf, offset, 0644);
    free(json_buf);
