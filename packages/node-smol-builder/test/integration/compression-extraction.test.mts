...
describe.skipIf(skipTests)('final binary extraction to ~/.socket/_dlx/', () => {
  ...
  describe('cache directory structure', () => {
    ...
    it('should have correct byte offset for compressed data', async () => {
      ...
      // Verify TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG is exactly 100 bytes
      // (marker 32 + compressed_size 8 + uncompressed_size 8 + cache_key 16
      // + platform_metadata 3 + integrity_hash 32 + smol_config_flag 1).
      expect(TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG).toBe(100)

      const dataOffset = markerIndex + TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG
      expect(dataOffset).toBe(markerIndex + 100)
    ...
    it('should validate metadata header size calculation', () => {
      ...
      // + cache_key(16) + platform_metadata(3) + integrity_hash(32)
      // + config_flag(1)
      ...
      expect(calculatedSize).toBe(68)
      expect(METADATA_HEADER_SIZE).toBe(68)
    })

    it('should validate total header size calculation', () => {
      // Total header = marker(32) + metadata(68)
    ...
    it('should have proper byte alignment for compressed data', async () => {
      ...
      expect(smolConfigFlagOffset).toBe(markerIndex + 99)
      expect(dataOffset).toBe(markerIndex + 100)
      ...
      expect(actualMetadataHeaderSize).toBe(68)
      ...
      expect(actualTotalHeaderSize).toBe(100)
    })
    ...
    it('should have sequential byte layout without gaps', async () => {
      ...
      // Integrity hash (32 bytes, SHA-256 of compressed data). Only shape
      ...
      // would indicate a writer bug rather than a real SHA-256).
      const integrityHash = binaryData.subarray(offset, offset + 32)
      expect(integrityHash.length).toBe(32)
