...
NOTE: SOURCE_PATCHED and SOURCE_COPIED checkpoints intentionally omit platform/arch/libc
...
- Use default imports for Node.js built-in modules: `import os from 'node:os'` instead of `import { platform } from 'node:os'`
- Use `import path from 'node:path'` and call `path.join()`, `path.resolve()`, etc. instead of cherry-picking individual functions
...
- Cherry-picked imports: `import { platform, arch } from 'node:os'` → use `import os from 'node:os'`
- Cherry-picked path: `import { join, resolve } from 'node:path'` → use `import path from 'node:path'`

Why this matters:
- Consistent logging across all packages (formatting, levels, CI integration)
- Clearer code showing which module provides the function (os.platform() vs platform())
- Easier refactoring and IDE navigation
- Avoids naming conflicts (e.g., `platform` parameter vs `platform()` function)
</pattern>
...
**CRITICAL: For submodule versions (LIEF, Node.js, etc.):**
- ALWAYS check .gitmodules file for the authoritative version (look for # vX.Y.Z comments)
- NEVER rely on `git describe --tags` output from submodule directories (may be inaccurate)
- The .gitmodules comment (e.g., `# v0.17.0`) is the source of truth
- Cross-reference with .node-version for Node.js version
- If documentation says a different version than .gitmodules, the DOCUMENTATION is wrong
</pattern>
