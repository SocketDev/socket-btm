...
async function downloadPrebuiltStub(options = {}) {
  ...
  try {
    const tarballPath = await downloadSocketBtmRelease({
      asset: assetName,
...
export async function ensureStubs(options = {}) {
  ...
  if (!force && stubExistsAt(stubOutDir)) {
    const localPath = path.join(stubOutDir, stubBinary)
  ...
  try {
    ...
    return path.join(stubOutDir, stubBinary)
  } catch (error) {
...
async function main() {
  try {
    ...
    const stubAvailable =
      stubExistsAt(stubOutDir) || stubExistsAt(downloadedDir)
