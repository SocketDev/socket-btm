 ...
 * downloadSocketBtmRelease with tool: 'lief' creates {downloadDir}/lief/assets/
 *
...
export function getLiefLibPath(buildMode) {
  ...
  return getLiefLibPathAt(localDir)
}
  ...
  if (WIN32) {
    ...
    try {
      ...
      if (e?.code === 'EACCES' || e?.code === 'EPERM') {
        throw new Error(
          `robocopy completed but cannot read source directory: ${e?.message || 'Unknown error'}`,
        )
      }
      throw e
    }
...
async function downloadPrebuiltLIEF(options = {}) {
  ...
  try {
    ...
    const liefIncludeDir = path.join(extractDir, 'include')
    const liefLibDir = path.join(extractDir, 'lib')
    if (existsSync(liefIncludeDir)) {
      await safeDelete(liefIncludeDir)
    }
    if (existsSync(liefLibDir)) {
      await safeDelete(liefLibDir)
    }
