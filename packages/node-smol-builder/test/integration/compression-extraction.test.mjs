...
function extractCompressedData(binaryData) {
  ...
  const dataOffset = markerIndex + TOTAL_HEADER_SIZE
...
describe.skipIf(skipTests)('Final binary extraction to ~/.socket/_dlx/', () => {
  ...
  describe('Cache directory structure', () => {
    ...
    it('should use universal LZFSE compression algorithm', async () => {
      if (!testCacheDir || !existsSync(testCacheDir)) {
        return
      }
    ...
    it('should have correct byte offset for compressed data', async () => {
      ...
      // Verify TOTAL_HEADER_SIZE is exactly 67 bytes
      expect(TOTAL_HEADER_SIZE).toBe(67)
    ...
    it('should have compression ratio greater than 1.0', async () => {
      if (!testCacheDir || !existsSync(testCacheDir)) {
        return
      }
      ...
      // Compression ratio should be > 1.0 (meaning compressed is smaller)
      expect(metadata.extra.compression_ratio).toBeGreaterThan(1.0)

      // Verify ratio calculation: uncompressed / compressed
    ...
    it('should have proper byte alignment for compressed data', async () => {
      ...
      expect(dataOffset).toBe(markerIndex + TOTAL_HEADER_SIZE)
      ...
      expect(actualTotalHeaderSize).toBe(TOTAL_HEADER_SIZE)
    ...
    it('should have sequential byte layout without gaps', async () => {
      ...
      expect(offset).toBe(markerIndex + TOTAL_HEADER_SIZE)
