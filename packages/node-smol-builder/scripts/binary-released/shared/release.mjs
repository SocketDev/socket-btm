...
const PACKAGE_NAME = PACKAGE_JSON.name.replace('@socketbin/', '')
...
async function findBinary(platform, arch) {
  ...
  const finalBinary = path.join(BUILD_DIR, 'out', 'Final', binaryName)
  if (
    platform === osPlatform() &&
    arch === osArch() &&
 ...
 *
 * Format: SMOL_SPEC:@socketbin/{packageName}-{version}-{platform}-{arch}\n
 *
 * This enables deterministic cache keys when the binary is used.
 */
async function embedSmolSpec(binaryPath, platform, arch, packageName, version) {
  const spec = `SMOL_SPEC:@socketbin/${packageName}-${version}-${platform}-${arch}\n`
...
async function createReleaseArchive(platform, arch, packageName, version) {
  ...
  await embedSmolSpec(tempBinary, platform, arch, packageName, version)
  ...
  // Create archive: {packageName}-{VERSION}-{PLATFORM}-{ARCH}.{EXT}
  const archiveName = `${packageName}-${version}-${platform}-${arch}.${config.ext}`
