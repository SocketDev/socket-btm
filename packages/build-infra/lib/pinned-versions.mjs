...
import { validateExternalTools } from './external-tools-schema.mjs'
...
export const PYTHON_PACKAGE_EXTRAS = (() => {
  const extras = {}
  for (const [name, config] of Object.entries(TOOL_VERSIONS)) {
    if (config.packageManager === 'pip' && config.extras && config.extras.length > 0) {
...
/**
 * Get pinned tool version by version key.
 *
 * @param {string} toolName - Tool name (e.g., 'cmake', 'ninja', 'python')
 * @param {string} versionKey - Version key (e.g., 'apt', 'brew', 'recommendedVersion', 'emsdk')
 * @param {object} [options] - Loading options for hierarchical lookup
 * @returns {string|undefined} Pinned version or undefined if not found
 */
export function getToolVersion(toolName, versionKey, options) {
  const tools = options ? loadExternalTools(options) : TOOL_VERSIONS
  const tool = tools[toolName]
  if (!tool || !tool.versions) {
    return undefined
  }
  return tool.versions[versionKey] || undefined
}
