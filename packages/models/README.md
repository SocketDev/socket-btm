...
This package uses incremental checkpoints for each model:

### MiniLM Pipeline:
1. **downloaded-minilm-l6** - Model downloaded from HuggingFace
2. **converted-minilm-l6** - Converted to ONNX format
3. **quantized-minilm-l6-int4** - INT4 quantization (prod) or INT8 (dev)

### CodeT5 Pipeline:
1. **downloaded-codet5** - Model downloaded from HuggingFace
2. **converted-codet5** - Converted to ONNX format (encoder + decoder)
3. **quantized-codet5-int4** - INT4 quantization (prod) or INT8 (dev)

### Unified Checkpoints:
1. **downloaded** - All models downloaded
2. **converted** - All models converted to ONNX
3. **quantized** - All models quantized
