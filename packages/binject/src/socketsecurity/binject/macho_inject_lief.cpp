...
extern "C" int binject_macho_lief_batch(
    ...
    // Add VFS config section if provided (SMOL_CONFIG_SIZE bytes SMFG format)
    // Note: Despite the name "VFS config", this section stores the SMOL config (1192 bytes)
    // which includes nodeVersion and other update config fields used at runtime.
    if (vfs_config_data) {
      printf("Creating VFS config section %s with %d bytes...\n", MACHO_SECTION_SMOL_VFS_CONFIG, SMOL_CONFIG_SIZE);

      LIEF::MachO::Section vfs_config_section(MACHO_SECTION_SMOL_VFS_CONFIG);
      std::vector<uint8_t> vfs_config_content(vfs_config_data, vfs_config_data + SMOL_CONFIG_SIZE);
