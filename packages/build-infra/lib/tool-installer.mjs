...
export async function ensureToolInstalled(
  ...
  if (binPath) {
    ...
    const verifyArgs = config?.verifyArgs || ['--version']
    ...
    const platform = getPlatform()
    const packageInfo = config?.packages?.[platform]
    const pkgName = packageInfo?.brew || packageInfo?.apt || tool
    ...
    if (platform === 'darwin' && packageInfo?.brew) {
      logger.substep(`Try: brew reinstall ${pkgName}`)
    } else if (platform === 'linux' && packageInfo?.apt) {
      logger.substep(`Try: sudo apt-get install --reinstall ${pkgName}`)
    }
