#!/usr/bin/env node
/**
 * Sync checksums from GitHub releases to release-assets.json.
 *
 * This script fetches checksums.txt from the latest release of each tool
 * and updates the embedded checksums in release-checksums.json.
 *
 * Usage:
 *   node scripts/sync-checksums.mjs [--tool=<tool>] [--force]
 *
 * Options:
...
async function main() {
  ...
  if (updated > 0 && !dryRun) {
    ...
    const output = {
      ...
      $comment: "SHA-256 checksums for GitHub release assets. Run 'pnpm --filter build-infra sync-checksums' to update.",
      ...currentData,
    }
