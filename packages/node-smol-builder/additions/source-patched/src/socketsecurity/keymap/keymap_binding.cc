...
//
// Contract: single-letter modifier aliases (`c` for ctrl, `s` for
// shift) resolve ONLY in modifier position — a non-terminal token,
// i.e. one followed by another `+`-separated token. The LAST token of
// a step is always the key name, taken literally, never alias-matched.
// Without that split, `ctrl+s` canonicalized to `ctrl+shift+` with an
// empty key name (the trailing `s` was consumed as a shift alias),
// making the literal `s` and `c` keys unbindable.
std::string CanonicalizeStep(const std::string& token) {
  ...
  while (start < token.size()) {
    ...
    const bool is_terminal = plus == std::string::npos;
    std::string part;
    if (is_terminal) {
    ...
    if (is_terminal) {
      // The last token is always the key name, taken literally — never
      // matched against a modifier alias, even when it collides with
      // one (`s`, `c`).
      if (!part.empty()) {
        keyname = part;
      }
