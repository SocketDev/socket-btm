...
const MIN_ZIG_MAJOR = 0;
const MIN_ZIG_MINOR = 15;
const MIN_ZIG_PATCH = 2;
...
fn checkZigVersion() void {
    ...
    // Accept any Zig >= MIN version (allows 0.15.2, 0.15.3, 0.16.x, etc.)
    const too_old = v.major < MIN_ZIG_MAJOR or
        (v.major == MIN_ZIG_MAJOR and v.minor < MIN_ZIG_MINOR) or
        (v.major == MIN_ZIG_MAJOR and v.minor == MIN_ZIG_MINOR and v.patch < MIN_ZIG_PATCH);
    if (too_old) {
        std.debug.print(
            "\x1b[31mError: Zig >= {}.{}.{} required, found {}.{}.{}\x1b[0m\n",
            .{ MIN_ZIG_MAJOR, MIN_ZIG_MINOR, MIN_ZIG_PATCH, v.major, v.minor, v.patch },
        );
        std.process.exit(1);
    }
}
