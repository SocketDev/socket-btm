...
// Pre-computed buffers, status codes, regex patterns for zero-allocation hot paths.
...
const HTTP_413_BODY = '{"error":"Payload Too Large","message":"Request body exceeds maxBodySize limit"}';
...
const CONTENT_LENGTH_CACHE = new Array(CONTENT_LENGTH_CACHE_SIZE);
for (let i = 0; i < CONTENT_LENGTH_CACHE_SIZE; i++) {
  CONTENT_LENGTH_CACHE[i] = BufferFrom(i + '\r\n\r\n');
}
...
const CT_JSON_KEEPALIVE = BufferFrom('Content-Type: application/json\r\nConnection: keep-alive\r\n\r\n');
const CT_TEXT_KEEPALIVE = BufferFrom('Content-Type: text/plain\r\nConnection: keep-alive\r\n\r\n');
const CT_HTML_KEEPALIVE = BufferFrom('Content-Type: text/html\r\nConnection: keep-alive\r\n\r\n');
