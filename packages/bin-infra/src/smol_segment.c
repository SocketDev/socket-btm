...
int smol_build_section_data(
    ...
    /* Calculate total section size: marker + metadata header + [update_config] + data. */
    /* Check for integer overflow before calculation. */
    size_t update_config_size = update_config_binary ? UPDATE_CONFIG_BINARY_LEN : 0;
    if (compressed_size > SIZE_MAX - MAGIC_MARKER_LEN - METADATA_HEADER_LEN - update_config_size) {
        fprintf(stderr, "Error: Compressed size too large (would overflow)\n");
        return -1;
    }
    section->size = MAGIC_MARKER_LEN + METADATA_HEADER_LEN + update_config_size + compressed_size;
    ...
    /* Write update config flag and binary (if provided). */
    if (update_config_binary) {
        section->data[offset++] = 1;  /* has_update_config = 1 */
        memcpy(section->data + offset, update_config_binary, UPDATE_CONFIG_BINARY_LEN);
        offset += UPDATE_CONFIG_BINARY_LEN;
    } else {
        section->data[offset++] = 0;  /* has_update_config = 0 */
    }
