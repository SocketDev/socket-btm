...
// Documentation: docs/additions/lib/internal/socketsecurity/vfs/fs.js.md
...
function realpathSync(filepath, options, _depth = 0) {
  ...
  if (entry && entry.type === 'symlink' && entry.linkTarget) {
    ...
    // matching POSIX realpath semantics. TAR stores symlink targets
    // relative to the link itself — treating them as VFS-root-absolute
    // causes `usr/lib64 -> lib` to resolve to `$prefix/lib` instead of
    // `$prefix/usr/lib`.
    const prefix = getVFSPrefix()
    const targetPath = StringPrototypeStartsWith(entry.linkTarget, '/')
      ? `${prefix}${entry.linkTarget}`
      : `${prefix}/${resolveRelativeVFSPath(vfsKey, entry.linkTarget)}`
...
/**
 * Resolve a relative TAR symlink target against the VFS key of the symlink.
 * Returns a cleaned VFS-root-relative path (no leading slash).
 * Example: resolveRelativeVFSPath('usr/lib64', 'lib') → 'usr/lib'.
 */
function resolveRelativeVFSPath(symlinkVfsKey, linkTarget) {
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    if (part === '..') {
      ArrayPrototypePop(out)
      continue
    }
    ArrayPrototypePush(out, part)
  }
