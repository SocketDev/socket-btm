...
function filterLintableFiles(files: string[]): string[] {
  ...
  const lintableExtensions = new Set(['.js', '.mts', '.cjs'])
...
async function main(): Promise<void> {
  try {
    ...
    const parsedValues = values as ParsedLintValues
    ...
    if (parsedValues.help) {
      ...
      logger.log('  pnpm lint scripts/*.mts     # Lint specific file(s)')
      process.exitCode = 0
      return
    }

    const quiet = isQuiet(parsedValues)
    ...
    if (positionals.length > 0) {
      ...
      exitCode = await runLintOnFiles(files, {
        fix: parsedValues.fix,
        quiet,
      })
    } else {
      // Get files to lint based on flags
      const { files, mode, reason } = await getFilesToLint(parsedValues)
