 ...
 *   staleness loud. Usage: node scripts/repo/check-publish-prereq.mts <package>
...
//
// Tier-1b builders (codet5-models, minilm) publish
...
async function main(): Promise<void> {
  ...
  if (!target) {
    logger.fail('Usage: node scripts/repo/check-publish-prereq.mts <package>')
...
// Skip execution when imported (e.g. scripts/repo/build-status.mts reuses
// the gh-release + git-ancestry helpers above). The CLI entry is
// direct `node scripts/repo/check-publish-prereq.mts <package>` invocation.
