...
async function checkExternalTools(packages) {
  ...
  for (const pkgName of C_PACKAGES) {
    ...
    // Validate schema
    try {
      const tools = JSON.parse(await fs.readFile(externalToolsPath, 'utf8'))
...
function generateSuggestions(packages) {
  ...
  for (const [scriptName, stats] of Object.entries(patternStats.scripts)) {
    ...
    if (confidence >= 75) {
      ...
      if (missingPackages.length > 0) {
        ...
        const mostCommonValue = Object.entries(stats.values).toSorted(
          ([, a], [, b]) => b - a,
        )[0][0]
  ...
  for (const [depName, stats] of Object.entries(patternStats.dependencies)) {
    ...
    if (confidence >= 75) {
      ...
      if (missingPackages.length > 0) {
        const mostCommonVersion = Object.entries(stats.versions).toSorted(
          ([, a], [, b]) => b - a,
        )[0][0]
