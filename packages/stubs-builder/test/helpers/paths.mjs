...
export function getLatestFinalBinary() {
  ...
  const devBinaryPath = path.join(packageDir, 'build', 'dev', PLATFORM_ARCH, 'out', 'Final')
  const prodBinaryPath = path.join(packageDir, 'build', 'prod', PLATFORM_ARCH, 'out', 'Final')
