 ...
 */
int binject_embed_compressed_segment(
    ...
    pid_t pid = fork();
    if (pid == -1) {
        fprintf(stderr, "Warning: Failed to fork for codesign\n");
        return 0;  // Non-fatal
    }
    ...
    if (pid == 0) {
        ...
        fprintf(stderr, "Warning: Failed to execute codesign\n");
        exit(1);
    }
    ...
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
        ...
        fprintf(stderr, "Warning: codesign failed (continuing anyway)\n");
    }
    #endif
 ...
 */
int binject_extract_compressed_segment(
