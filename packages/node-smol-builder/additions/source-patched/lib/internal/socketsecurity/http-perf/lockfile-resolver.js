...
const LockfileFormats = {
  ...
  parsePackageLock(content) {
    try {
      ...
      return { format: 'package-lock', packages };
    } catch (e) {
      return { error: 'Invalid package-lock.json', format: 'package-lock' };
    }
  },
  ...
  parseYarnLock(content) {
    ...
    return { format: 'yarn.lock', packages };
  },
  ...
  parsePnpmLock(content) {
    ...
    for (const line of lines) {
      ...
      if (match) {
        currentPackage = { name: match[1], version: match[2] };
  ...
  parseCargoLock(content) {
    ...
    return { format: 'Cargo.lock', packages };
  },
  ...
  parseGemfileLock(content) {
    ...
    return { format: 'Gemfile.lock', packages };
  },
};
...
class LockfileResolver {
  ...
  parseLockfile(content) {
    ...
    switch (format) {
      ...
      default:
        return { error: 'Unknown lockfile format', format: 'unknown' };
    }
  }
