...
export default defineConfig({
  test: {
    ...
    // globals:true (repo override of the template default). The fleet-dir
    // segmentation retired the old shared base (.config/vitest.config.mts,
    // globals:true) in favor of this self-contained config (template default
    // globals:false). btm's ~100 existing test files across 19 packages were
    // written against globals:true and use bare describe/it/expect; flipping to
    // false would break them all. This is an EXPECTED_FILES entry (existence-
    // checked, body varies per repo), so this override is sanctioned and won't
    // drift against the cascade. Migrating the test files to explicit `import
    // { describe } from 'vitest'` is the longer-term path.
    globals: true,
    environment: 'node',
