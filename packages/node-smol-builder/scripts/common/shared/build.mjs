...
async function collectBuildSourceFiles(phase = 'binary-released') {
  ...
  for (const patchDir of existingPatchDirs) {
    ...
    // Validate files exist and are readable before adding to sources
    // This prevents race conditions during file collection
    for (const patchFile of patchFiles) {
      try {
        await fs.access(patchFile, fs.constants.R_OK)
        sources.push(patchFile)
      } catch {
        logger.warn(`Skipping unreadable patch file: ${patchFile}`)
      }
    }
  }
