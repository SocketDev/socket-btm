...
describe('@socketbin/node-smol-builder package', () => {
  describe('package.json validation', () => {
    it('should have valid package.json metadata', async () => {
      ...
      expect(pkgJson.name).toBe('@socketbin/node-smol-builder')
  ...
  describe('build script structure', () => {
    ...
    it('build.mjs should read Node.js version from .node-version', async () => {
      const buildPath = path.join(scriptsDir, 'common/shared/build.mjs')
      const content = await fs.readFile(buildPath, 'utf-8')

      // Should import NODE_VERSION_FILE from paths.mjs
      expect(content).toContain('NODE_VERSION_FILE')
      expect(content).toContain('nodeVersionRaw')
    })
