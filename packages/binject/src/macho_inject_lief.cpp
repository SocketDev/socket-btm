...
extern "C" int binject_macho_lief(const char* executable,
  ...
  try {
    ...
    if (!binary->has_segment(segment_name)) {
      ...
      // LIEF will automatically calculate addresses for new sections
    ...
    // This removes the LC_CODE_SIGNATURE load command that LIEF added during parsing.
  ...
  try {
    ...
    if (vfs_data || vfs_compat_mode) {
      if (vfs_compat_mode && vfs_size == 0) {
        printf("Creating empty VFS section __SMOL_VFS_BLOB (0 bytes, compatibility mode)...\n");
      } else {
...
extern "C" int binject_macho_repack_smol_lief(
  ...
  if (!stub_path || !section_data || !output_path || section_size == 0) {
    fprintf(stderr, "Error: Invalid arguments to repack function\n");
    return BINJECT_ERROR_INVALID_ARGS;
  }
  ...
  try {
    ...
    if (!binary->has_segment("SMOL")) {
      fprintf(stderr, "Error: SMOL segment not found in stub\n");
      return BINJECT_ERROR_SECTION_NOT_FOUND;
    }

    printf("  Found SMOL segment, removing and recreating with new size...\n");

    // Remove the existing SMOL segment entirely.
    ...
    if (!found) {
      fprintf(stderr, "Error: Could not find SMOL segment index\n");
      return BINJECT_ERROR_SECTION_NOT_FOUND;
    }

    // Remove the SMOL segment.
    if (!binary->remove_command(smol_index)) {
      fprintf(stderr, "Error: Failed to remove SMOL segment\n");
      return BINJECT_ERROR;
    }
    ...
    LIEF::MachO::SegmentCommand new_smol("SMOL");
    ...
    LIEF::MachO::Section pressed_section("__PRESSED_DATA");
    ...
    // Remove existing code signature (will be invalid after changes).
    ...
    if (!sign_binary_adhoc(output_path)) {
      fprintf(stderr, "⚠ Warning: Failed to sign repacked stub\n");
    }
