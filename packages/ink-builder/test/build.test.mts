...
// These are integration tests on the built `dist/` artifact. When the
// package hasn't been built yet (e.g. running `pnpm --recursive run test`
// before any `pnpm --filter ink-builder build`), skip gracefully instead
// of failing — matches how yoga-layout-builder tests behave via
// `createWasmTestHelpers#skipIfNotBuilt`.
const HAS_DIST = existsSync(DIST_DIR)

describe.skipIf(!HAS_DIST)('ink build', () => {
