 ...
 *
 * This script runs INSIDE the built node-smol binary, which is built
 * --without-amaro, so it must stay .mjs (no TypeScript support). That
 * constraint is why it lives under test/fixtures/ rather than scripts/.
 * globalThis.ReadableStream etc are the patched fast-webstreams versions.
 */
  ...
  function createHarnessGlobals() {
    ...
    async function promise_rejects_dom(_t, name, promise, msg) {
      try {
        ...
        if (
          e instanceof Error &&
          e.message.startsWith('promise_rejects_dom:')
        ) {
          throw e
        }
