...
1. **curl + mbedtls** - Independent, can be parallel
2. **cJSON + libdeflate + lzfse** - Used by binject/stubs (update before binject consumers)
3. **yoga, onnxruntime** - Independent packages
4. **Node.js** - Update Node.js FIRST before LIEF (Node.js deps determines LIEF version)
5. **LIEF** - MUST match Node.js deps version (run AFTER Node.js update)
6. **fast-webstreams** - Vendor sync, independent

**CRITICAL: LIEF Version Dependency:**
- LIEF version is determined by Node.js deps: `node/deps/LIEF/include/LIEF/version.h`
- ALWAYS update Node.js BEFORE running updating-lief
- The updating-lief skill reads the version from Node.js deps and syncs to match
- NEVER update LIEF independently of Node.js
...
- `updating-yoga` - Updates Yoga layout library source, bumps yoga-layout cache
- `updating-ink` - Updates ink TUI framework from npm, bumps ink cache (depends on yoga-layout-builder)
...
- Follow dependency order: curl → other libs → Node.js → LIEF
- Target stable releases only (exclude -rc, -alpha, -beta tags)
- LIEF version MUST match Node.js deps (update Node.js first, then LIEF)
...
- LIEF: `X.Y.Z` (e.g., 0.17.0) - **MUST match Node.js deps version**
...
**CRITICAL ORDER: Node.js MUST be updated BEFORE LIEF**
- LIEF version is determined by Node.js deps
- The updating-lief skill reads version from Node.js deps and syncs to match

#### 5.1: Update curl and mbedtls (if newer available)
...
#### 5.2: Update cJSON (if newer available)
...
#### 5.3: Update libdeflate (if newer available)
...
#### 5.4: Update LZFSE (if newer available)
...
#### 5.5: Update Yoga (if newer available)
...
#### 5.6: Update ink (if newer available)

<action>
Use the updating-ink skill (AFTER yoga since ink depends on yoga-layout-builder):
</action>

**IMPORTANT:** This MUST run AFTER updating-yoga. The ink package bundles yoga-sync.mjs from yoga-layout-builder.

```
Skill({ skill: "updating-ink" })
```

Wait for skill completion before proceeding.

#### 5.8: Update ONNX Runtime (if newer available)
...
#### 5.9: Update Node.js (if newer available)
...
#### 5.10: Update LIEF (sync to match Node.js deps)

<action>
Use the updating-lief skill to sync LIEF submodule to match Node.js deps version:
</action>

**IMPORTANT:** This MUST run AFTER updating-node. The updating-lief skill reads the LIEF version from Node.js deps and syncs the submodule to match.
...
#### 5.11: Update fast-webstreams (if needed)
...
#### 5.12: Update checksums (always run last)

<action>
Use the updating-checksums skill to sync SHA-256 checksums from GitHub releases:
</action>

**IMPORTANT:** This MUST run AFTER all other updates. The updating-checksums skill fetches checksums from GitHub releases and updates release-assets.json.
...
- Uses `updating-checksums` skill for release asset checksums
