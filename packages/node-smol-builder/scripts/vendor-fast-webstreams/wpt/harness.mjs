 ...
 *
 * Adapted from vercel-labs/fast-webstreams
 */
...
export function createHarnessGlobals(streamGlobals) {
  ...
  function assert_object_equals(a, b, msg) {
    ...
    for (const key of keysA) {
      if (!Object.is(a[key], b[key])) {
        throw new Error(
          `assert_object_equals: mismatch at key "${key}": ${JSON.stringify(a[key])} vs ${JSON.stringify(b[key])}${formatMsg(msg)}`,
        )
      }
    }
  }
  ...
  function assert_throws_js(Type, fn, msg) {
    ...
    try {
      ...
      if (e?.constructor?.name !== Type.name) {
        throw new Error(
          `assert_throws_js: expected ${Type.name} but got ${e?.constructor?.name}: ${e?.message}${formatMsg(msg)}`,
        )
      }
    }
    if (!threw) {
  ...
  async function promise_rejects_js(_t, Type, promise, msg) {
    try {
      ...
      if (e?.constructor?.name !== Type.name) {
        throw new Error(
          `promise_rejects_js: expected ${Type.name} but got ${e?.constructor?.name}: ${e?.message}${formatMsg(msg)}`,
        )
      }
    }
  }
  ...
  async function promise_rejects_dom(_t, name, promise, msg) {
    try {
      ...
      if (!(e instanceof DOMException)) {
        throw new Error(
          `promise_rejects_dom: expected DOMException but got ${e?.constructor?.name}: ${e?.message}${formatMsg(msg)}`,
        )
      }
      if (e.name !== name) {
        throw new Error(
          `promise_rejects_dom: expected DOMException with name "${name}" but got "${e.name}"${formatMsg(msg)}`,
        )
      }
    }
  }
  ...
  async function promise_rejects_exactly(_t, val, promise, msg) {
    try {
      ...
      if (e !== val) {
        throw new Error(
          `promise_rejects_exactly: rejection value does not match expected${formatMsg(msg)}`,
        )
      }
    }
  }
  ...
  const globals = {
    ...
    ReadableStreamBYOBReader:
      streamGlobals.ReadableStreamBYOBReader ||
    ...
    ReadableStreamDefaultReader:
      streamGlobals.ReadableStreamDefaultReader ||
      globalThis.ReadableStreamDefaultReader,
    WritableStreamDefaultWriter:
      streamGlobals.WritableStreamDefaultWriter ||
