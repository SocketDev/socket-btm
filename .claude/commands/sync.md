...
Invoke the updating-node skill to synchronize with upstream Node.js:
...
Use the Skill tool to invoke: `updating-node`

**Alternative:** Use `/sync-status` to check current version vs latest available.

## Success Criteria

- ✅ Skill `updating-node` invoked successfully
...
Upon successful completion, the `updating-node` skill emits:
...
For comprehensive edge case handling, refer to the `updating-node` skill documentation.

**Common issues:**

- **Upstream not initialized**: Skill initializes submodule automatically
- **Patches fail to apply**: Phase 5 retry loop (MAX_ITERATIONS=3) with auto-retry
- **Build fails**: Phase 6 retry loop with lint auto-fix and retry
...
- `packages/node-smol-builder/patches/source-patched/` - Socket patches (16 total)

**Related Skills:**

- `updating-node` - The skill invoked by this command
