...
describe('cache-key', () => {
  describe('generateCacheKey', () => {
    it('should generate cache key with basic options', () => {
      ...
      expect(key).toMatch(/^v24\.10\.0-darwin-arm64-[a-f0-9]{8}-215$/)
    })
    ...
    it('should generate different hashes for different content files', async () => {
      ...
      try {
        ...
        expect(key1).toMatch(/^v24\.10\.0-darwin-arm64-[a-f0-9]{8}-215$/)
        expect(key2).toMatch(/^v24\.10\.0-darwin-arm64-[a-f0-9]{8}-215$/)
      } finally {
    ...
    it('should use default platform and arch if not provided', () => {
      ...
      expect(key).toMatch(/^v24\.10\.0-\w+-\w+-[a-f0-9]{8}-215$/)
    })
    ...
    it('should include cache-busting dependencies if provided', async () => {
      ...
      try {
        ...
        await fs.writeFile(
          ...
          JSON.stringify({
            dependencies: {
              '@socketsecurity/lib': '1.0.1', // Different version
              '@socketsecurity/packageurl-js': '2.0.0',
            },
          }),
        )
  ...
  describe('parseCacheKey', () => {
    it('should parse valid cache key', () => {
      const key = 'v24.10.0-darwin-arm64-b71671ba-215'
    ...
    it('should return null for invalid cache key format', () => {
      ...
      expect(parseCacheKey('24.10.0-darwin-arm64-b71671ba-215')).toBeNull() // Missing 'v'
    })

    it('should handle different node versions', () => {
      const key1 = 'v20.0.0-darwin-arm64-abc12345-100'
      const key2 = 'v24.10.0-darwin-arm64-abc12345-100'

      const parsed1 = parseCacheKey(key1)
      const parsed2 = parseCacheKey(key2)

      expect(parsed1?.nodeVersion).toBe('20.0.0')
      expect(parsed2?.nodeVersion).toBe('24.10.0')
    })

    it('should handle different platforms and architectures', () => {
      const darwinKey = 'v24.10.0-darwin-arm64-abc12345-100'
      const linuxKey = 'v24.10.0-linux-x64-abc12345-100'
      const win32Key = 'v24.10.0-win32-x64-abc12345-100'
    ...
    it('should restore package version with dots', () => {
      const key = 'v24.10.0-darwin-arm64-abc12345-215'
      const parsed = parseCacheKey(key)
        ...
        await fs.writeFile(
          ...
          JSON.stringify({
            dependencies: {
              '@socketsecurity/lib': '1.0.0',
              '@socketsecurity/packageurl-js': '2.0.0',
              '@socketsecurity/other': '3.0.0', // Should be ignored
            },
          }),
        )
        ...
        await fs.writeFile(
          ...
          JSON.stringify({
            dependencies: {
              '@socketsecurity/lib': '1.0.0',
              '@socketsecurity/packageurl-js': '2.0.0',
              '@socketsecurity/other': '4.0.0', // Changed but should be ignored
            },
          }),
        )
        ...
        await fs.writeFile(
          ...
          JSON.stringify({
            dependencies: {
              '@socketsecurity/lib': '1.0.0',
              '@socketsecurity/packageurl-js': '2.0.0',
              '@socketsecurity/sdk': '3.0.1', // Changed
              '@socketsecurity/registry': '4.0.0',
            },
          }),
        )
