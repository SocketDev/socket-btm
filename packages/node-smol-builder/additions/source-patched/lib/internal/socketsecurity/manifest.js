...
function parsePackageLock(content) {
  ...
  if (data.dependencies) {
    ...
    // Cap recursion depth. The `visited` set prevents cycles but not
    // pathological linear nesting — an attacker-crafted lockfile with
    // ".dependencies.A.dependencies.B…" 20k levels deep would blow
    // the JS engine stack with a RangeError that unwinds past all
    // caller try/catch. Real npm nesting is ~10 levels; 64 is a safe
    // ceiling that surfaces malformed input as a thrown Error.
    const MAX_LOCKFILE_DEPTH = 64
    const flatten = (deps, parentPath = '', depth = 0) => {
  ...
  while (pos < content.length) {
    ...
    if (inImporters) {
      ...
      if (currentImporter && indent > importerIndent) {
        if (StringPrototypeIndexOf(trimmed, 'devDependencies:') === 0) {
          ...
          //
          // pnpm v9 nests each dep as a block:
          //   pkg:                <- this is the dep name (indent = importerIndent+4)
          //     specifier: ^1     <- sub-property (indent = importerIndent+6)
          //     version: 1.0.0    <- sub-property (indent = importerIndent+6)
          // Without this guard the deeper sub-property lines were parsed
          // as `specifier` / `version` / `resolution` "dependencies",
          // polluting every pnpm v9 SBOM with fake entries.
          if (
    ...
    if (currentPkg && indent > currentIndent) {
      if (StringPrototypeIndexOf(trimmed, 'dev:') === 0) {
        ...
        // Leaving the dependencies: block once a sibling header appears at
        // the same indent tells us we're no longer capturing dep names.
        // Without this, a subsequent `peerDependencies:` / `engines:` /
...
// Maximum user-supplied regex source length. Prevents trivial ReDoS
// patterns like `^(a+)+$` combined with long input; the regex
// construction itself scales with source length and complex patterns
// amplify that against every package.name they're tested against.
const FIND_PACKAGES_PATTERN_MAX_LEN = 256

// Find packages matching pattern
function findPackages(lockfile, pattern) {
  let regex
  if (pattern instanceof RegExpCtor) {
    // Apply the same length cap to RegExp-input that the string-input
    // branch enforces. Without this, an attacker-supplied `^(a+)+$`
    // with a 1 MB lockfile's package-name surface catastrophically
    // backtracks on every test — remote event-loop stall. hardenRegExp
    // protects prototype tampering, not pathological backtracking.
    if (pattern.source.length > FIND_PACKAGES_PATTERN_MAX_LEN) {
      throw new RangeErrorCtor(
        `pattern exceeds maximum length (${FIND_PACKAGES_PATTERN_MAX_LEN})`,
      )
    }
    // Harden prototype so a polluted RegExp.prototype.test / .exec can't
    // hijack the call. hardenRegExp returns the original regex with
    // fast-path methods snapshot-locked.
    regex = hardenRegExp(pattern)
  } else {
    ...
    regex = hardenRegExp(new RegExpCtor(pattern))
  }
