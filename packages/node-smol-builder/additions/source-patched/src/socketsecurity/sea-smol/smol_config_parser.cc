...
// Cross-platform POSIX compatibility (POSIX_UNLINK, etc.)
...
std::optional<std::vector<uint8_t>> SerializeVfsConfig(
  ...
  // Copy to vector (serialize_vfs_config returns fixed VFS_CFG_SIZE-byte buffer).
  std::vector<uint8_t> result(binary, binary + VFS_CFG_SIZE);
  free(binary);
...
bool InjectSeaAndVfs(const std::string& executable_path,
  ...
  if (written != sea_blob.size()) {
    ...
    fclose(f);
    POSIX_UNLINK(sea_tmp);  // Remove incomplete file
    return false;
  }

  // Note: Explicit fsync/FlushFileBuffers removed to avoid blocking indefinitely.
  // The fclose() below is sufficient - it flushes stdio buffers to kernel,
  // and the kernel will sync to disk asynchronously. For build tooling,
  // the rare power-failure data loss risk is acceptable.
  ...
  if (fclose(f) != 0) {
    ...
    POSIX_UNLINK(sea_tmp);  // Remove potentially incomplete file
    return false;
  }
