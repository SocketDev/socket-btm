...
/* find_marker_in_ptnote removed - using shared implementation from ptnote_finder.h */
 ...
 *   1. TMPDIR/TMP/TEMP environment variables, fallback to /tmp (via get_tmpdir)
 *   2. /dev/shm (tmpfs - RAM-based, faster for decompression)
 */
static int create_temp_file(char *template_path) {
    // Try directories in order: env vars + Node.js fallback, then /dev/shm optimization.
    const char *temp_dirs[] = {
        get_tmpdir(NULL),      // Node.js default: TMPDIR→TMP→TEMP→/tmp
        get_tmpdir("/dev/shm") // Optimize with tmpfs (RAM) fallback
    };
    ...
    for (size_t i = 0; i < sizeof(temp_dirs) / sizeof(temp_dirs[0]); i++) {
        ...
        // Validate directory security before using it.
        struct stat dir_stat;
        if (stat(temp_dirs[i], &dir_stat) != 0) {
            continue;  // Directory doesn't exist or not accessible
        }

        // Check if directory is owned by root or current user.
        uid_t current_uid = getuid();
        if (dir_stat.st_uid != 0 && dir_stat.st_uid != current_uid) {
            continue;  // Skip directories owned by other users
        }

        // Check if sticky bit is set (prevents symlink attacks).
        if (!(dir_stat.st_mode & S_ISVTX)) {
            continue;  // Skip directories without sticky bit
        }
...
int main(int argc, char *argv[], char *envp[]) {
    ...
    // This ensures consistency with stubs, binpress, and binject
    long data_offset = find_marker_in_ptnote(source_fd, MAGIC_MARKER_PART1, MAGIC_MARKER_PART2, MAGIC_MARKER_PART3, 1);
    if (data_offset == -1) {
    ...
    if (compressed_size > SIZE_MAX || uncompressed_size > SIZE_MAX) {
        fprintf(stderr, "Error: Size exceeds system limits\n");
        goto cleanup;
    }
    ...
    if (cache_hit) {
        ...
        // Set environment variables for JS bootstrap
        setenv("NODE_SMOL_CACHE_KEY", cache_key, 1);
        if (exe_path[0] != '\0') {
            setenv("SOCKET_SMOL_STUB_PATH", exe_path, 1);
        }
    ...
    if (!cache_written) {
        ...
        // Set environment variables for JS bootstrap
        // Note: cache_key might be empty for temp fallback
        if (cache_key[0] != '\0') {
            setenv("NODE_SMOL_CACHE_KEY", cache_key, 1);
        }
        if (exe_path[0] != '\0') {
