...
function createServer(opts) {
  ...
  return {
    ...
    get url() {
      return new URL(`http://${hostname === '0.0.0.0' ? 'localhost' : hostname}:${actualPort}/`);
    },
...
function serve(options) {
  ...
  // Monitor workers — restart on unexpected exit.
  for (const child of workerProcesses) {
    child.on('exit', (code) => {
      if (code !== 0) {
        const idx = workerProcesses.indexOf(child);
  ...
  return {
    ...
    get url() {
      return new URL(`http://${hostname === '0.0.0.0' ? 'localhost' : hostname}:${port}/`);
    },
