 ...
 *   --from-checkpoint=<name>    Skip to specific build phase
 *                               Valid: binary-stripped, binary-compressed, finalized
 *
 * Usage Patterns:
...
async function main() {
  ...
  if (skipToStripped) {
    // Verify required checkpoint exists
    const strippedCheckpoint = path.join(
      BUILD_DIR,
      'checkpoints',
      'binary-stripped.json',
    )
    if (!existsSync(strippedCheckpoint)) {
      throw new Error(
        `Cannot skip to ${FROM_CHECKPOINT}: binary-stripped checkpoint not found at ${strippedCheckpoint}`,
      )
    }
    logger.log('')
    logger.log(
