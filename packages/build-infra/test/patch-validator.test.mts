...
import {
  analyzePatchContent,
  checkPatchConflicts,
} from '../lib/patch-validator.mts'

describe('patch-validator', () => {
  let tempDir: string

  beforeEach(async () => {
    tempDir = path.join(os.tmpdir(), `patch-validator-test-${Date.now()}`)
...
`
      ...
      expect(analysis.modifiesV8Includes).toBeTruthy()
    })
...
`
      ...
      expect(analysis.modifiesSEA).toBeTruthy()
    })
...
`
      ...
      expect(analysis.modifiesBrotli).toBeTruthy()
    })
...
`
      ...
      expect(analysis.modifiesV8Includes).toBeFalsy()
      expect(analysis.modifiesSEA).toBeFalsy()
      expect(analysis.modifiesBrotli).toBeFalsy()
    })
  })
