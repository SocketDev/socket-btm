...
🚨 **Package manager: `pnpm`** — scripts via `pnpm run foo --flag` (never `foo:bar`); after `package.json` edits, `pnpm install`. NEVER `npx` / `pnpm dlx` / `yarn dlx` — use `pnpm exec` or `pnpm run` # socket-hook: allow npx. NEVER `--experimental-strip-types` to Node (enforced by `.claude/hooks/fleet/no-experimental-strip-types-guard/`).
...
🚨 **Supply-chain hygiene.** New deps Socket-scored at edit time (`.claude/hooks/fleet/check-new-deps/`); 7-day `minimumReleaseAge` soak is malware protection (bypass `Allow minimumReleaseAge bypass`); soak-bypass entries need `# published: YYYY-MM-DD | removable: YYYY-MM-DD` annotations. Dep overrides in `pnpm-workspace.yaml`, never `package.json` `pnpm.overrides` (bypass `Allow package-json-overrides bypass`). **Never weaken a trust gate** (`trustPolicy: no-downgrade`, `--config.trustPolicy=trust-all`, `blockExoticSubdeps`) — fix stale lockfiles via the soak/exclude entry; the bypass `Allow trust-downgrade bypass` is single-use and not persisted (enforced by `.claude/hooks/fleet/minimum-release-age-guard/` + `.claude/hooks/fleet/soak-exclude-date-annotation-guard/` + `.claude/hooks/fleet/no-package-json-pnpm-overrides-guard/` + `.claude/hooks/fleet/trust-downgrade-guard/`).
...
- `codex-no-write-guard` — blocks `codex` / `codex-rescue` invocations with write-intent flags. Use Codex for advice not code. Bypass: `Allow codex-write bypass` (enforced by `.claude/hooks/fleet/codex-no-write-guard/`).
- `concurrent-cargo-build-guard` — blocks a second `cargo build --release` while one is in flight (8 LLVM threads × 8-22GB = OOM). Cargo-only. Bypass: `Allow concurrent-cargo-build bypass` (enforced by `.claude/hooks/fleet/concurrent-cargo-build-guard/`).
- `broken-hook-detector` — SessionStart probe: surfaces missing-dep → `pnpm i <pkg>` fix instead of `package_json_reader:314` spam (enforced by `.claude/hooks/fleet/broken-hook-detector/`).
- `answer-passing-questions-reminder` + `answer-status-requests-reminder` — Stop hooks: when user asks a passing question OR explicitly requests status mid-task, fire if you brush past with "later" / "too soon" / "skipping". Lend a few tokens, then continue (enforced by `.claude/hooks/fleet/answer-passing-questions-reminder/` + `.claude/hooks/fleet/answer-status-requests-reminder/`).

<!-- END FLEET-CANONICAL -->

## 🏗️ BTM-Specific

socket-btm builds Socket's customized Node.js binary ("smol Node") with embedded C++ acceleration modules (`node:smol-*`), packs it into a SEA, and publishes per-platform artifacts that downstream fleet members (socket-cli, socket-addon) consume via SHA-pinned trusted-publisher releases.

🚨 **Hot-path invariants** — full detail in [`docs/claude.md/repo/architecture.md`](docs/claude.md/repo/architecture.md):

- **Publish dispatch order:** curl + lief PARALLEL → stubs → binsuite → node-smol. Bump `cache-versions.json` before re-dispatch.
- **Additions `-fno-exceptions`:** allocate `new (std::nothrow) T(...)` + null-check + `isolate->ThrowException(...)`. STL containers `.reserve(N)` + size-cap before `.resize(n)`.
- **1 patch, 1 file. 1 file, 1 patch.** Unified diff, NEVER `git format-patch`. Headers: `# @<project>-versions: vX.Y.Z` + `# @description: ...`. Inline explanatory comments belong in the patch header, never inside hunks ([`docs/references/btm-source-patches.md`](docs/references/btm-source-patches.md)).
- **SEA entry:** Node 25.7+ replaces ambient `require` in CJS SEA. Always `Module.createRequire(scriptPath)`.
- **gypi source paths** are relative to the **gypi's own location**, not the parent gyp file. A `.gypi` at `<root>/deps/yoga.gypi` listing `'deps/yoga/Foo.cpp'` resolves to `<root>/deps/deps/yoga/Foo.cpp` (double-`deps/`, missing-file error). Sources should read `'yoga/Foo.cpp'` (no `deps/` prefix) when the gypi already lives under `deps/`. Auto-generated by `prepare-external-sources.mts`; new gypi emitters should compute `path.relative(path.dirname(gypiOut), sourceAbs)` (enforced by `.claude/hooks/fleet/gypi-source-path-guard/`).

Deep references (per-tier): [`docs/references/btm-*.md`](docs/references/) (additions-cpp, additions-js, source-patches, check-gates, build-conventions, glossary).
