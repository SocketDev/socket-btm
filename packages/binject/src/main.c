...
static void print_usage(const char *program) {
    ...
    printf("Usage:\n");
    printf("  %s inject -e <executable> -o <output> [--sea <path>] [--vfs <path>] [--no-compress] [--overwrite]\n", program);
#ifdef __APPLE__
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(command, "inject") == 0) {
        ...
        return binject_inject_batch(executable, output, sea_resource, vfs_resource, compress, overwrite);
    }
