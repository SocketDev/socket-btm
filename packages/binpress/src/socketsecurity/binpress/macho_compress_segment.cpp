// ============================================================================
// macho_compress_segment.cpp — Segment insertion for valid macOS signatures
// ============================================================================
//
// WHAT THIS FILE DOES
// Uses LIEF to insert a new SMOL segment (containing compressed binary data)
// into a Mach-O stub BEFORE the __LINKEDIT segment. This is the only way
// to add data to a Mach-O binary while keeping its code signature valid.
// After insertion, the binary is ad-hoc signed with codesign.
//
// WHY IT EXISTS
// macOS Gatekeeper rejects binaries with invalid signatures. Appending data
// after __LINKEDIT (the naive approach) breaks the signature. This file
// implements the correct insertion technique so the output binary passes
// all macOS security checks.
// ============================================================================
...
int binpress_segment_embed(
    ...
    // Check if SMOL segment with __PRESSED_DATA section exists - inline repack logic.
    // Must check for __PRESSED_DATA, not just SMOL segment existence.
    // node-smol binaries may have SMOL segment with only __SMOL_NODE_VER section.
    bool has_pressed_data = false;
    if (binary->has_segment(MACHO_SEGMENT_SMOL)) {
        LIEF::MachO::SegmentCommand* smol_seg = binary->get_segment(MACHO_SEGMENT_SMOL);
        if (smol_seg) {
            for (const auto& sec : smol_seg->sections()) {
                if (sec.name() == MACHO_SECTION_PRESSED_DATA) {
                    has_pressed_data = true;
                    break;
                }
            }
        }
    ...
    if (has_pressed_data) {
        printf("  Found existing __PRESSED_DATA section - repacking inline (single-pass)...\n");
...
int binpress_segment_extract(
    ...
    for (const auto& section : sections) {
        ...
        if (section_name == MACHO_SECTION_PRESSED_DATA) {
            ...
            // Read sizes (use memcpy to avoid unaligned access)
            uint64_t compressed_size, uncompressed_size;
            memcpy(&compressed_size, content.data() + marker_len, sizeof(uint64_t));
            memcpy(&uncompressed_size, content.data() + marker_len + sizeof(uint64_t), sizeof(uint64_t));
