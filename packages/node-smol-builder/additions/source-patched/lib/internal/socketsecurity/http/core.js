...
// The llhttp-derived method list node:http.METHODS exposes (http.js does
// `methods.toSorted()` off the SAME binding) — mirrored here rather than
// hand-maintained so a QUERY-class addition to llhttp's method table shows
// up in both without a second edit. Copy-then-sort: the binding's `methods`
// array backs node:http's internal method-index mapping, so sorting it in
// place would corrupt that unrelated consumer.
function getMethods() {
  if (!methodsCache) {
    const { methods } = internalBinding('http_parser')
    methodsCache = ArrayPrototypeSort(ArrayPrototypeSlice(methods))
  }
  return methodsCache
}
...
module.exports = {
  ...
  // METHODS — llhttp's method table (includes QUERY), same source
  // node:http.METHODS reads.
  get METHODS() {
    return getMethods()
  },
