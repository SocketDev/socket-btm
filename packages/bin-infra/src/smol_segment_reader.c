...
int smol_read_metadata(int fd, smol_metadata_t *metadata) {
    ...
    char cache_key_raw[16];
    if (read(fd, cache_key_raw, 16) != 16) {
        fprintf(stderr, "Error: Failed to read cache key\n");
        return -1;
    }
    /* Copy to output and null-terminate. */
    memcpy(metadata->cache_key, cache_key_raw, 16);
    metadata->cache_key[16] = '\0';

    /* Read platform metadata (3 bytes). */
    if (read(fd, metadata->platform_metadata, 3) != 3) {
...
int smol_validate_metadata(const smol_metadata_t *metadata, size_t max_size) {
    ...
    if (strlen(metadata->cache_key) != 16) {
        fprintf(stderr, "Error: Cache key must be exactly 16 characters (got %zu)\n",
                strlen(metadata->cache_key));
        return -1;
    }
