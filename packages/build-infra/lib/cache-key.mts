 ...
 * v{nodeVersion}-{platform}-{arch}-{contentHash}-{pkgVersion} Example:
 * v24.12.0-darwin-arm64-b71671ba-2.1.5.
 *
 * Platform and arch should be specified explicitly for cross-compilation
 * builds. Defaults to current system values if not provided.
 *
 * Cache-busting dependencies: - Bootstrap: @socketsecurity/lib,
 *
 * @socketsecurity/packageurl-js - CLI: @socketsecurity/lib,
 *
 * @socketsecurity/packageurl-js, @socketsecurity/sdk, @socketsecurity/registry
...
export function generateCacheKey({
  ...
  // Validate packageVersion is a non-empty string before processing
  if (!packageVersion || typeof packageVersion !== 'string') {
    throw new Error(
      `Invalid package version: expected string, got ${typeof packageVersion}`,
    )
  }

  const versionParts = packageVersion.split('.')
  // Validate version format before generating cache key
  if (versionParts.some(p => !p || !/^\d+$/.test(p) || p.length > 10)) {
    throw new Error(
      `Invalid package version format: "${packageVersion}". ` +
        'Expected numeric semver format (e.g., "2.1.5" or "1.0.0") with max 10 digits per component',
    )
  }
 ...
 * @param {object} currentOptions - Current build options (same as
 *   generateCacheKey)
 *
 * @returns {boolean}
 */
...
export function parseCacheKey(cacheKey) {
  ...
  // Note: componentLengthStr is guaranteed non-empty by regex pattern -([\d,]+)\.
  // which requires at least one digit or comma
