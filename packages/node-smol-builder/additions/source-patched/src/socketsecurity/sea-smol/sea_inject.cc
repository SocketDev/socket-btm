...
std::optional<std::vector<uint8_t>> SerializeVfsConfig(
  ...
  // Allocate SVFG binary blob.
  svfg_config_t svfg;
  memset(&svfg, 0, sizeof(svfg));

  // Copy fields from C++ struct to C struct.
  strncpy(svfg.mode, config.mode.c_str(), sizeof(svfg.mode) - 1);
  strncpy(svfg.source, config.source.c_str(), sizeof(svfg.source) - 1);
  strncpy(svfg.prefix, config.prefix.c_str(), sizeof(svfg.prefix) - 1);

  // Serialize to binary.
  uint8_t* data = nullptr;
  size_t size = 0;
  int rc = svfg_serialize(&svfg, &data, &size);
