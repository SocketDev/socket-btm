    ...
    async () => {
      ...
      for (const name of criticalEnumNames) {
        ...
        if (expectedValue === undefined) {
          throw new Error(
            `YGEnums.mts missing ${name} - source file may need updating`,
          )
        }
        ...
        if (Yoga[name] !== expectedValue) {
          throw new Error(
            `Sync wrapper ${name} value mismatch: ` +
              `expected ${expectedValue} (from YGEnums.mts), got ${Yoga[name]}`,
          )
        }
      }
      ...
      logger.substep(
        `Validated ${allEnumNames.length} enum constants against YGEnums.mts`,
      )
    },
    {
