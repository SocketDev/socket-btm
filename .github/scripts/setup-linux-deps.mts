 ...
 * - Linux/macOS: No additional dependencies (zstd compiled from bundled sources)
 */
...
async function runCommand(command, args, forceShell = false) {
  const result = await spawn(command, args, {
    stdio: 'inherit',
    shell: forceShell || isWindows,
  })
...
async function setupWindows() {
  ...
  await runCommand(
    'choco',
    ['install', '-y', 'cmake', '--installargs', 'ADD_CMAKE_TO_PATH=System'],
    true,
  )
...
async function main() {
  try {
    ...
    logger.fail(
      `Failed to install dependencies: ${error?.message || 'Unknown error'}`,
    )
    process.exitCode = 1
  }
}
