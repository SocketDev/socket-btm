...
int compress_buffer(const uint8_t *input, size_t input_size,
    ...
    /* Skip shrinking - buffer will be used and freed shortly.
     * The extra memory is minimal compared to realloc copy cost. */
    *output = dst_buffer;
...
int decompress_buffer(const uint8_t *input, size_t input_size,
    ...
    /* Start with conservative estimate: LZFSE worst-case is ~1.01x + small overhead.
     * Use input_size * 1.1 + 1024 to handle typical cases without retry. */
    ...
    size_t dst_size = input_size + (input_size / 10) + 1024;
    ...
    for (int attempt = 0; attempt < 3; attempt++) {
        ...
        if (dst_size > SIZE_MAX / 2) {
            free(dst_buffer);
            fprintf(stderr, "Error: Buffer size would overflow on retry\n");
            return COMPRESS_ERROR_SIZE_LIMIT_EXCEEDED;
        }
        dst_size *= 2;
    }
