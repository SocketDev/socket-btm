...
// Writes a text/plain response directly to the UV stream via uv_try_write.
...
void WritePrecomputed(const FunctionCallbackInfo<Value>& args) {
  Environment* env = Environment::GetCurrent(args);
...
void Initialize(Local<Object> exports,
  ...
  // Fast response writers (write directly to UV stream via uv_try_write)
  ...
  SetMethod(context, exports, "writePrecomputed", WritePrecomputed);
...
void RegisterExternalReferences(ExternalReferenceRegistry* registry) {
  ...
  registry->Register(WritePrecomputed);
