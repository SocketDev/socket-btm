...
function parseMaven(version) {
  ...
  for (let i = 0; i < qualifierParts.length; i++) {
    ...
    const numMatch = RegExpPrototypeExec(/^([a-z]+)(\d+)$/, q);
    if (numMatch) {
...
function parsePypi(version) {
  ...
  const releaseParts = StringPrototypeSplit(rest, /[._-]/);
  const release = [];
  let suffixStart = 0;
  ...
  for (let i = 0; i < releaseParts.length; i++) {
    ...
    if (isDigits(part)) {
      ...
      const numPrefixMatch = RegExpPrototypeExec(/^(\d+)(a|alpha|b|beta|c|rc|pre|preview|post|rev|r|dev)/i, part);
      if (numPrefixMatch) {
  ...
  let remaining = ArrayPrototypeJoin(ArrayPrototypeSlice(releaseParts, suffixStart), '.');
  if (inlineSuffix) {
  ...
  if (remaining) {
    ...
    const preMatch = RegExpPrototypeExec(
      /^(a|alpha|b|beta|c|rc|pre|preview)\.?(\d+)?/, normalized);
    if (preMatch) {
    ...
    const postMatch = RegExpPrototypeExec(
      /(?:^|[._-])(post|rev|r)\.?(\d+)?/, normalized);
    if (postMatch) {
      hasPost = true;
    ...
    // Dev release: dev
    const devMatch = RegExpPrototypeExec(
      /(?:^|[._-])dev\.?(\d+)?/, normalized);
    if (devMatch) {
      hasDev = true;
...
function parseGem(version) {
  ...
  let normalized = ArrayPrototypeJoin(StringPrototypeSplit(version, '-'), '.pre.');
  ...
  for (let i = 0; i < dotParts.length; i++) {
    ...
    const subParts = StringPrototypeSplit(part, /(\d+)/);
...
function compareGem(va, vb) {
  ...
  while (i1 < sa.length || i2 < sb.length) {
    ...
    while (i1 < sa.length && sa[i1] === 0 &&
           i2 < sb.length && typeof sb[i2] === 'string') {
      i1++;
    }
    while (i2 < sb.length && sb[i2] === 0 &&
           i1 < sa.length && typeof sa[i1] === 'string') {
      i2++;
    }
...
function compare(a, b, ecosystem = 'npm') {
  ...
  if (va.prerelease.length === 0 && vb.prerelease.length === 0 &&
      va._packed >= 0 && vb._packed >= 0 &&
      !va.hasDev && !vb.hasDev && !va.hasPost && !vb.hasPost) {
...
function parseComparator(comp, ecosystem) {
  ...
  if (wildMatch) {
    return {
      ...
      minor: wildMatch[2] && !RegExpPrototypeTest(/[xX*]/, wildMatch[2]) ? parseDigits(wildMatch[2]) : undefined,
      patch: wildMatch[3] && !RegExpPrototypeTest(/[xX*]/, wildMatch[3]) ? parseDigits(wildMatch[3]) : undefined,
    };
  }
