...
Binject now automatically embeds smol configuration when injecting SEA blobs from `sea-config.json` files. The configuration is extracted from the `smol` section of `sea-config.json` and serialized to a 1192-byte SMFG v2 binary format.
...
## Binary Format (SMFG v2)

**Total size**: 1192 bytes
...
3. **String fields** (1168 bytes):
   ...
   - Node version: 16 bytes (1-byte length prefix + 15 data bytes)
...
3. Serializes it to 1192-byte SMFG v2 binary format
...
- `packages/stubs-builder/src/socketsecurity/stubs-builder/update_config.h` - Contains `update_config_from_binary()` function

**Stub integration**:
- `packages/stubs-builder/src/socketsecurity/stubs-builder/elf_stub.c` - Linux ELF stub (reads SMFG config)
- `packages/stubs-builder/src/socketsecurity/stubs-builder/macho_stub.c` - macOS Mach-O stub
- `packages/stubs-builder/src/socketsecurity/stubs-builder/pe_stub.c` - Windows PE stub
