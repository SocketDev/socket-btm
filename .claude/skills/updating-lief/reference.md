...
find packages/{binject,binpress,lief-builder}/src \
  -type f \( -name "*.cpp" -o -name "*.hpp" -o -name "*.h" -o -name "*.cc" \) \
  -exec grep -l "LIEF::" {} \;

# Alternative: Use ripgrep for speed
rg -l "LIEF::" packages/{binject,binpress,lief-builder}/src
...
- ✅ No MH\_\* constant issues (after fixes)
...
- `packages/lief-builder/src/socketsecurity/lief-builder/stub_smol_repack_lief.cpp:86`
...
- `packages/lief-builder/src/socketsecurity/lief-builder/elf_note_utils.hpp:662-672`
...
grep -rn "LIEF::MachO::" packages/{binject,binpress,lief-builder}/src

# Audit only ELF APIs
grep -rn "LIEF::ELF::" packages/{binject,binpress,lief-builder}/src

# Audit only PE APIs
grep -rn "LIEF::PE::" packages/{binject,binpress,lief-builder}/src
```
...
**LIEF Submodule:**
- Location: \`packages/lief-builder/upstream/lief\`
...
**CRITICAL: LIEF Version Source of Truth:**
- LIEF version is determined by Node.js deps: \`packages/node-smol-builder/upstream/node/deps/LIEF/include/LIEF/version.h\`
- The [TARGET_VERSION] passed to this agent was extracted from that file
- NEVER use a different version than what Node.js deps specifies
- If you need a newer LIEF, first update Node.js using the updating-node skill

**Workflow Overview:**
1. Verify target version matches Node.js deps (already done by skill)
2. Fetch target LIEF tag from submodule
3. Update submodule to target version
...
# Stage changes
git add .gitmodules packages/lief-builder/upstream/lief
...
- packages/lief-builder/upstream/lief → \$NEW_VERSION (\$NEW_COMMIT)
...
### Step 8.5: Bump Cache Versions

<action>
Bump cache versions in .github/cache-versions.json for all packages that use LIEF:
</action>

**Skip this step if:** LIEF submodule SHA did not change (already at target version). In this case, no changes were made and no cache bump is needed.

**If LIEF was updated:** Read current cache versions and bump all five versions:
- `lief`: vN → v(N+1)
- `binflate`: vN → v(N+1)
- `binject`: vN → v(N+1)
- `binpress`: vN → v(N+1)
- `node-smol`: vN → v(N+1) (node-smol uses binject)

\`\`\`bash
echo "Bumping cache versions..."

# Read current cache versions and bump: lief, binflate, binject, binpress, node-smol
# Use Edit tool to increment each version number
...
<action>
Create commit with API fixes, cache versions, and attach audit report:
</action>

\`\`\`bash
# Stage all changes (API fixes + additions sync + cache versions)
...
- ✅ Cache versions bumped: lief, binflate, binject, binpress
