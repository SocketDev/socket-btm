...
import { BYTES } from 'build-infra/lib/constants'
import { NODE_VERSION } from 'build-infra/lib/node-version'
...
const TARGET_LIBC = values.libc || null
...
// NODE_SHA is derived from upstream/node at build time (see clone-source.mjs)
...
async function main() {
  ...
  const skipReleasePhase =
    FROM_CHECKPOINT === 'binary-released' ||
    FROM_CHECKPOINT === 'binary-stripped' ||
    FROM_CHECKPOINT === 'binary-compressed' ||
    FROM_CHECKPOINT === 'finalized'
  const skipToCompressed =
    FROM_CHECKPOINT === 'binary-compressed' || FROM_CHECKPOINT === 'finalized'
  const skipToFinalized = FROM_CHECKPOINT === 'finalized'

  let releaseResult = { cached: false, isCrossCompiling: false }

  if (skipReleasePhase) {
    // Verify required artifact exists based on checkpoint
    if (FROM_CHECKPOINT === 'binary-released') {
    ...
    const { buildRelease } = await import(
      '../../binary-released/shared/build-released.mjs'
    )
    ...
    releaseResult = await buildRelease(
      {
        ...
        collectBuildSourceFiles: () =>
          collectBuildSourceFiles('binary-released'),
        packageRoot: PACKAGE_ROOT,
      },
      { skipCheckpoint: BUILD_ONLY === 'binary-released' },
    )
  }
  ...
  if (STOP_AT === 'binary-released' || BUILD_ONLY === 'binary-released') {
    logger.log('')
    if (BUILD_ONLY) {
      logger.skip(
  ...
  if (skipToCompressed || skipToFinalized) {
    ...
    await buildStripped(
      ...
      { skipCheckpoint: BUILD_ONLY === 'binary-stripped' },
    )
  }

  // Stop early if --stop-at=binary-stripped or --build-only=binary-stripped was specified
  if (STOP_AT === 'binary-stripped' || BUILD_ONLY === 'binary-stripped') {
    logger.log('')
    if (BUILD_ONLY) {
      logger.skip(
  ...
  if (skipToFinalized) {
    ...
    const result = await buildCompressed(
      ...
      { skipCheckpoint: BUILD_ONLY === 'binary-compressed' },
    )
    compressed = result.compressed
  }

  // Stop early if --stop-at=binary-compressed or --build-only=binary-compressed was specified
  if (STOP_AT === 'binary-compressed' || BUILD_ONLY === 'binary-compressed') {
    logger.log('')
    if (BUILD_ONLY) {
      logger.skip(
  ...
  // ============================================================================
  const { finalizeBinary } = await import(
    '../../finalized/shared/finalize-binary.mjs'
  )
  ...
  const finalSourcePaths = await collectBuildSourceFiles('finalized')
