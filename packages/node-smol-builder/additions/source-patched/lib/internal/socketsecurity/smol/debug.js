...
function parseDebugPatterns() {
  ...
  cachedPatterns = debugEnv.split(',').map(p => {
    const trimmed = p.trim()
    if (trimmed.startsWith('-')) {
      return { pattern: trimmed.slice(1), negated: true }
    }
...
function createDebug(ns) {
  ...
  return (msg, ...args) => {
    ...
    formatted = formatted.replace(/%[sdo]/g, match => {
    ...
    if (argIndex < args.length) {
      formatted += ` ${args.slice(argIndex).join(' ')}`
    }

    // Write to stderr with namespace prefix
    process.stderr.write(`[${ns}] ${formatted}\n`)
  }
}
