...
describe.skipIf(skipTests)('cross-Platform Decompression', () => {
  ...
  describe('platform-specific stub, universal data', () => {
    it('should have platform-specific stub code', () => {
      ...
      // But under 2MB (the stub statically links libcurl + mbedtls + zstd +
      // libdeflate + LIEF; those libraries grow over time, so this bound
      // only catches runaway regressions — e.g. a forgotten -s strip step
      // or accidental debug-symbol inclusion — not natural growth).
      expect(stubSize).toBeLessThan(2 * 1024 * 1024)
    })
  ...
  describe('platform metadata purpose', () => {
    it('should use platform metadata only for cache organization', () => {
      ...
      const metadataPlatformByte = binaryData[metadataOffset]
      const metadataArchByte = binaryData[metadataOffset + 1]
      const metadataLibcByte = binaryData[metadataOffset + 2]

      // All bytes should be valid
      expect([0, 1, 2]).toContain(metadataPlatformByte)
      expect([0, 1, 2, 3]).toContain(metadataArchByte)
      expect([0, 1, 255]).toContain(metadataLibcByte)
