...
Modifier name aliases (case-insensitive), single-letter forms apply
only in modifier position — a token followed by another `+`-joined
token. The last token of a step is always the key name, taken
literally, so `ctrl+s` binds the `s` key and a bare `"s"` step binds
the literal `s` key, not shift:
