// ============================================================================
// binpress_main.c — Shared main() entry point for all platform builds
// ============================================================================
//
// WHAT THIS FILE DOES
// Provides the single main() function used by all three platform-specific
// binpress builds (macOS, Linux, Windows). Parses arguments, selects the
// right stub, and calls binpress_platform_route() which each platform
// binary implements to dispatch to the right compression function.
//
// WHY IT EXISTS
// Without this shared file, each platform binary (elf_compress.c,
// macho_compress.c, pe_compress.c) would duplicate the same 100+ lines
// of argument handling and stub selection. This file keeps that logic in
// one place; each platform only provides a short routing function.
// ============================================================================
...
int binpress_main(int argc, char *argv[]) {
    ...
    // Handle data-only mode (no executable stub, just compressed data).
    if (!config.output_path && config.output_data_path) {
        return compress_data_only(
            config.input_path,
            config.output_data_path,
            0,
            config.target,
    ...
    return binpress_platform_route(&config, stub, config.output_path);
}
