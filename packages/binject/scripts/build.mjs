...
async function main() {
  try {
    // Determine binary name and path first for checkpoint validation
    ...
    // On Windows, LIEF is always built with gcc/g++ (MinGW) for ABI consistency,
    // regardless of CC/CXX environment variables. This avoids ABI mismatch between
    // clang (MSVC ABI) and MinGW toolchain used by binject.
    //
    // LIEF is now built in bin-infra (shared with binpress).
    // LIEF checkpoint is mode-specific (build/{mode}/) following node-smol pattern.
    const binInfraBuildDir = path.join(
      packageRoot,
      '../bin-infra/build',
      BUILD_MODE,
    )
    const liefLibUnix = path.join(binInfraBuildDir, 'lief', 'libLIEF.a')
    const liefLibMSVC = path.join(binInfraBuildDir, 'lief', 'LIEF.lib')
    const liefLibPath = existsSync(liefLibUnix) ? liefLibUnix : liefLibMSVC
    const liefIncludeDir = path.join(binInfraBuildDir, 'lief', 'include')
    ...
    if (
      ...
      await runCommand(
        'node',
        [path.join(packageRoot, '../bin-infra/scripts/build-lief.mjs')],
        packageRoot,
      )
      logger.info('')
    } else {
