...
function nativeAcquireRequest() { return smolHttp().acquireRequest(); }
function nativeReleaseRequest(req) { return smolHttp().releaseRequest(req); }
function nativeAcquireResponse() { return smolHttp().acquireResponse(); }
function nativeReleaseResponse(res) { return smolHttp().releaseResponse(res); }
function nativeGetObjectPoolStats() { return smolHttp().getObjectPoolStats(); }

// Use Node.js built-in llhttp parser (lazy).
let _httpParserBinding;
function getHTTPParser() {
  if (!_httpParserBinding) _httpParserBinding = internalBinding('http_parser').HTTPParser;
...
function acquireRequest() {
  ...
  // Layer JS-specific fields onto the native object.
  // The C++ pool pre-sets method/url/headers; we reset them to JS
  // conventions (empty strings instead of null) and add fields the
  // C++ side doesn't know about.
  req.method = '';
  req.url = '';
  req.pathname = '';
