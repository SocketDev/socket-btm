...
function rewriteOverlayPath(path) {
  ...
  if (
    ...
    !StringPrototypeStartsWith(resolved, `${extractDirResolved}/`)
  ) {
  ...
  readFileSync(path, options) {
    ...
    if (isVFSPath(path)) {
      ...
      if (rewritten && smolVfsState.originalReadFileSync) {
        return ReflectApply(smolVfsState.originalReadFileSync, shimmedFs, [rewritten, options])
      }
    }
  ...
  existsSync(path) {
    ...
    if (isVFSPath(path)) {
      ...
      if (rewritten && smolVfsState.originalExistsSync) {
        return ReflectApply(smolVfsState.originalExistsSync, shimmedFs, [rewritten])
      }
    }
  ...
  statSync(path, _options) {
    ...
    if (isVFSPath(path)) {
      ...
      if (rewritten && smolVfsState.originalStatSync) {
        return ReflectApply(smolVfsState.originalStatSync, shimmedFs, [rewritten, _options])
      }
    }
    // Special case: VFS prefix path itself (e.g., '/snapshot')
  ...
  lstatSync(path, _options) {
    ...
    if (isVFSPath(path)) {
      ...
      if (rewritten && smolVfsState.originalLstatSync) {
        return ReflectApply(smolVfsState.originalLstatSync, shimmedFs, [rewritten, _options])
      }
    }
    // Special case: VFS prefix path itself (e.g., '/snapshot')
    // This enables glob to recognize the VFS mount point as a directory
    ...
    if (isVFSPath(path)) {
      ...
      if (smolVfsState.overlay) {
        ...
        if (rewritten && smolVfsState.originalReadlinkSync) {
          return ReflectApply(smolVfsState.originalReadlinkSync, shimmedFs, [rewritten, _options])
        }
      }
  ...
  readdirSync(path, options) {
    ...
    if (isVFSPath(path)) {
      ...
      if (rewritten && smolVfsState.originalReaddirSync) {
        return ReflectApply(smolVfsState.originalReaddirSync, shimmedFs, [rewritten, options])
      }
    }
    // Special case: VFS prefix path itself (e.g., '/snapshot')
    ...
    if (path === '/' && hasVFS() && smolVfsState.originalReaddirSync) {
      ...
      const realEntries = ArrayFrom(smolVfsState.originalReaddirSync('/', options))
  ...
  realpathSync(path, _options) {
    ...
    if (isVFSPath(path)) {
      ...
      if (rewritten && smolVfsState.originalRealpathSync) {
        return ReflectApply(smolVfsState.originalRealpathSync, shimmedFs, [rewritten, _options])
      }
    }
    // Special case: VFS prefix path itself (e.g., '/snapshot')
    if (hasVFS() && path === getVFSPrefix()) {
    ...
    if (isVFSPath(path)) {
      ...
      if (smolVfsState.overlay) {
        ...
        if (rewritten && smolVfsState.originalAccessSync) {
          return ReflectApply(smolVfsState.originalAccessSync, shimmedFs, [rewritten, _mode])
        }
      }
      throw createENOENT('access', path)
    }
