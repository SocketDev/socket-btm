...
/**
 * Fetch the canonical SFW bypass list from socket-registry and write it
 * to ~/.socket/sfw/env.sh so a user can `source` it before invoking any
 * package manager that goes through the local sfw shim.
 *
 * In CI the setup action already exports SFW_CUSTOM_REGISTRIES from the
 * same source file; this gives local-dev parity without each engineer
 * having to copy the list into their shell rc.
 */
async function writeBypassEnv({ quiet }: { quiet: boolean }): Promise<void> {
  const url =
    'https://raw.githubusercontent.com/SocketDev/socket-registry/main/.github/actions/setup/sfw-bypass-list.txt'
  let body: string
  try {
    body = await httpText(url)
  } catch (e) {
    if (!quiet) {
      logger.warn(`Skipping sfw env.sh — fetch error: ${errorMessage(e)}`)
    }
    return
  }
  const bypass = body
