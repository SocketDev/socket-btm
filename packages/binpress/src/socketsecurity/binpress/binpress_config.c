// ============================================================================
// binpress_config.c — CLI argument parsing implementation
// ============================================================================
//
// WHAT THIS FILE DOES
// Implements the argument parser for binpress: walks through argv, matches
// flags like --output, --target, --node-version, and populates a
// binpress_config struct. Also validates that required fields are present.
//
// WHY IT EXISTS
// C does not have a built-in argument parser like Node.js's parseArgs().
// This file manually processes argv (the raw command-line strings) to
// fill in the config struct used by the rest of binpress.
// ============================================================================
...
void binpress_print_usage(const char *program) {
    ...
    printf("  --node-version VERSION      Node.js version to embed (e.g., 24.12.0)\n");
...
int binpress_parse_args(int argc, char *argv[], binpress_config *config) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        } else if (strcmp(argv[i], "--node-version") == 0) {
            if (i + 1 < argc) {
                config->node_version = argv[++i];
            } else {
                fprintf(stderr, "Error: --node-version requires a version argument (e.g., 24.12.0)\n");
                return -1;
            }
...
int binpress_validate_config(const binpress_config *config) {
    ...
    if (!config->output_path && !config->output_data_path) {
        fprintf(stderr, "Error: Must specify at least one output: -o or -d\n");
        return -1;
    }
