...
/**
 * Detect if binary is a SMOL compressed stub.
...
/**
 * Extract SMOL stub to uncompressed binary.
 */
static int binject_extract_smol_stub(const char *stub_path, const char *extracted_path) {
    printf("Extracting SMOL stub...\n");
    printf("  Stub: %s\n", stub_path);
    printf("  Output: %s\n", extracted_path);

    int result = smol_extract_binary_lief(stub_path, extracted_path);
    if (result != 0) {
        fprintf(stderr, "Error: Failed to extract SMOL stub\n");
        return -1;
    }

    struct stat st;
    if (stat(extracted_path, &st) != 0) {
        fprintf(stderr, "Error: Extracted binary not found\n");
        return -1;
    }

    printf("  ✓ Extracted (%zu MB)\n", (size_t)(st.st_size / 1024 / 1024));
    return 0;
}
...
int binject_batch(const char *executable, const char *output,
    ...
    /* Check if this is a compressed stub or SMOL stub */
    /* Use cache-based extraction for both types */
    int is_compressed = binject_is_compressed_stub(injection_target);
    if (!is_compressed && !skip_repack) {
        is_smol_stub = binject_is_smol_stub(executable);
        if (is_smol_stub == 1) {
            /* SMOL stubs are also compressed - treat them as such */
            is_compressed = 1;
    ...
    if (is_compressed) {
        ...
        if (stat(extracted_path, &st) != 0) {
            ...
            rc = BINJECT_ERROR_FILE_NOT_FOUND;
            goto cleanup;
        }
