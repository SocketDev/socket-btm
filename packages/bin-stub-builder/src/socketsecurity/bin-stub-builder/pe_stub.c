 ...
 *   3. Decompresses inline using ZSTD
...
#include "socketsecurity/bin-infra/stub_decompress.h"
#include "socketsecurity/bin-infra/stub_exe_path.h"
...
#include "socketsecurity/bin-stub-builder/update_integration.h"
...
static int extract_and_execute(int self_fd, const char *exe_path, int argc, char *argv[],
    ...
    if (smol_read_metadata_pe(self_fd, &metadata) != 0) {
        return 1;
    }
    ...
    if (stub_zstd_decompress(compressed_data, compressed_size, decompressed_data, uncompressed_size) != 0) {
...
int main(int argc, char *argv[], char *envp[]) {
    ...
    // Get path to current executable.
    if (stub_executable_path(exe_path, sizeof(exe_path)) != 0) {
        return 1;
    }
