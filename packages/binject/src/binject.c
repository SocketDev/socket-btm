...
#define COMPRESSED_STUB_SEARCH_SIZE (64 * 1024)  /* Search first 64KB for marker */
...
int binject_is_compressed_stub(const char *executable) {
    ...
    /* Search for magic marker (built at runtime to avoid false positives) */
    char marker[MAGIC_MARKER_LEN + 1];
    snprintf(marker, sizeof(marker), "%s%s%s", MAGIC_MARKER_PART1, MAGIC_MARKER_PART2, MAGIC_MARKER_PART3);
    ...
    size_t required_size = marker_len + 8 + 8 + 16;

    for (size_t i = 0; i <= read_size - required_size; i++) {
        if (memcmp(buffer + i, marker, marker_len) == 0) {
            ...
            size_t cache_key_offset = i + marker_len + 8 + 8;
...
int binject_get_extracted_path(const char *compressed_stub, char *extracted_path, size_t path_size) {
    ...
    /* Check if buffer is large enough to contain marker */
    if (read_size < marker_len) {
        free(buffer);
        fprintf(stderr, "Error: File too small to contain marker\n");
        return BINJECT_ERROR;
    }
    ...
    if (lstat(home, &home_st) != 0) {
        fprintf(stderr, "Error: Cache directory not accessible: %s\n", home);
        return BINJECT_ERROR;
    }

    /* Reject symlinks to prevent TOCTOU attacks */
    if (S_ISLNK(home_st.st_mode)) {
...
#ifdef _WIN32
        /* Windows: Use _spawnvp to avoid command injection via system() */
        char *stub_argv[] = {(char*)compressed_stub, "--version", NULL};
        intptr_t pid = _spawnvp(_P_NOWAIT, compressed_stub, (const char* const*)stub_argv);
        if (pid == -1) {
            fprintf(stderr, "Error: Failed to spawn stub process\n");
            return BINJECT_ERROR;
        }
...
int binject_batch(const char *executable, const char *output,
    ...
    if (format == BINJECT_FORMAT_MACHO) {
#ifdef HAVE_LIEF
        rc = binject_macho_lief_batch(target_binary, injection_output, sea_data, sea_size, vfs_data, vfs_size);
#else
        fprintf(stderr, "Error: Mach-O injection requires LIEF library on %s\n",
#if defined(_WIN32)
                "Windows"
        ...
        rc = binject_batch_elf(target_binary, injection_output, sea_data, sea_size, vfs_data, vfs_size);
    } else if (format == BINJECT_FORMAT_PE) {
        rc = binject_batch_pe(target_binary, injection_output, sea_data, sea_size, vfs_data, vfs_size);
    } else {
