...
async function main() {
  try {
    ...
    // Require binpress path to be passed (DRY - resolved by caller via compression-tools.mts)
    if (!binpressPath) {
      throw new Error(
        'Missing --binpress-path argument.\n' +
          'This script should be called from build-compressed.mts which resolves the binpress path.',
      )
    }
    ...
    logger.log('Socket Binary Compression (zstd)')
    logger.log('==================================')
