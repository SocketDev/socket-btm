# update-node Reference Documentation

This document provides detailed edge cases, troubleshooting procedures, and advanced topics for the update-node skill in socket-btm.
...
if [ -n "$(git status --porcelain)" ]; then
  ...
  git status --short
  echo ""
  echo "Please handle these before syncing. Do NOT `git stash` — that"
  echo "uses a shared stash store that parallel Claude sessions can"
  echo "clobber. Commit on a branch or work in an isolated worktree."
  echo "  git checkout -b wip/before-node-sync"
  echo "  git add <specific-files> && git commit -m 'wip: before sync'"
  echo "  git checkout main"
  echo "  # or:"
  echo "  git worktree add ../socket-btm-node-sync main"
  echo "  cd ../socket-btm-node-sync  # do the sync here instead"
  exit 1
fi
```

**User Action Required:** Commit specific files on a WIP branch, or use a separate worktree, before running sync. Do NOT stash.
...
**Why Needed:** `pnpm run build` can fail transiently due to:
...
for i in $(seq 1 $MAX_RETRIES); do
  ...
  if pnpm run build 2>&1 | tee /tmp/patch-output-$i.log; then
...
```bash
cd packages/node-smol-builder
if ! pnpm run build 2>&1 | tee /tmp/patch-output.log; then
...
2. **Test patch manually:**
   ...
   patch --dry-run < ../../patches/source-patched/001-common-gypi-lto.patch
   ```
...
### Step 4: Update .node-version, .gitmodules checksum, and commit

<action>
Update .node-version, fetch and store source tarball checksum in .gitmodules, and create first commit:
</action>
...
const result = await fetchNodeChecksum(NEW_VERSION)
if ('hash' in result) {
  // Update .gitmodules: # node-NEW_VERSION sha256:<hash>
}
```

Or via Edit tool, replace the `# node-OLD_VERSION ...` line in `.gitmodules` with:
`# node-NEW_VERSION sha256:<fetched_hash>`

Then verify the round-trip with `verifyNodeChecksum()`:

```javascript
import { verifyNodeChecksum } from 'build-infra/lib/version-helpers'

const verification = await verifyNodeChecksum({ version: NEW_VERSION })
if (!verification.valid) {
  throw new Error(`Checksum mismatch: ${verification.expected} !== ${verification.actual}`)
}
```

```bash
# Stage changes
git add .node-version .gitmodules packages/node-smol-builder/upstream/node
...
- .gitmodules: node version and sha256 checksum
...
3. Do all patches apply cleanly?
   - Expected patches: [count from patches directory]
...
Changes:
- All patches applied cleanly to Node.js v$NEW_VERSION
...
### Step 7.5: Bump Cache Version (Only if Updated)
...
git add .github/cache-versions.json
git commit --amend --no-edit  # Add to previous commit
echo "✅ Cache version bumped: node-smol"
\`\`\`
...
All steps completed successfully. Post-update skills should now be triggered.
</validation>
...
- ✅ Ready for post-update skills and push to remote
  </success_criteria>
