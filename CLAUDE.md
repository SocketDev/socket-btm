...
- **`packageManager` field** — always include the sha512 integrity hash in corepack hex form: `pnpm@<version>+sha512.<128-char-hex>`. The hex digest comes from npm's `dist.integrity` (base64) decoded to hex — it's **NOT** the same encoding as `external-tools.json`'s SRI `integrity` (which IS base64). RFC 2018 mandates hex for corepack; SRI mandates base64 for the npm registry. Same algorithm, different encoding, both are required where they live. Bare `pnpm@X.Y.Z` without the hash is forbidden — corepack will install whatever the registry serves without integrity check.
