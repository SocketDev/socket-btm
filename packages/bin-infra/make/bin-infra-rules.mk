...
$(BUILD_DIR)/compression_common.o: $(BIN_INFRA_SRC)/compression_common.c | $(BUILD_DIR)
	$(CC) $(CFLAGS) -c $< -o $@

$(BUILD_DIR)/binary_format_finder.o: $(BUILD_INFRA_SRC)/binary_format_finder.c | $(BUILD_DIR)
	$(CC) $(CFLAGS) -c $< -o $@
...
$(BUILD_DIR)/process_exec.o: $(BUILD_INFRA_SRC)/process_exec.c | $(BUILD_DIR)
	$(CC) $(CFLAGS) -c $< -o $@
...
$(BUILD_DIR)/smol_detect_lief.o: $(BIN_INFRA_SRC)/smol_detect_lief.cpp | $(BUILD_DIR)
	$(CXX) $(CXXFLAGS) -c $< -o $@

# smol_config from binject (used by binpress for SMFG config serialization).
# Only define rule when BINJECT_SRC is set (binpress) - binject uses its own SRC_DIR.
ifdef BINJECT_SRC
$(BUILD_DIR)/smol_config.o: $(BINJECT_SRC)/smol_config.c | $(BUILD_DIR)
	$(CC) $(CFLAGS) -c $< -o $@
endif
