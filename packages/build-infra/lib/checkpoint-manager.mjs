...
export async function createCheckpoint(
  ...
  if (data.artifactPath) {
    ...
    // Include timestamp to prevent PID reuse collisions
    const tempTarballPath = `${tarballPath}.tmp.${process.pid}.${Date.now()}`
  ...
  if (data.sourcePaths && Array.isArray(data.sourcePaths)) {
    // Include platform/arch/libc INSIDE the hash computation to prevent collisions
    // This ensures platform changes properly invalidate the cache
    const platformMetadata =
      platform && arch ? `${platform}-${arch}${libc ? `-${libc}` : ''}` : ''
    const pathsWithPlatform = platformMetadata
      ? [...data.sourcePaths, `__PLATFORM_METADATA__:${platformMetadata}`]
      : data.sourcePaths
    cacheHash = computeSourceHash(pathsWithPlatform)
...
export async function shouldRun(
  ...
  if (sourcePaths && Array.isArray(sourcePaths) && sourcePaths.length > 0) {
    ...
    // Include platform/arch INSIDE the hash computation using CURRENT runtime platform
    // (not checkpoint's stored platform) to ensure cross-platform cache invalidation
    const platform = process.platform
    const arch = process.arch
    // Use stored libc for consistency if available
    const libc = checkpointData.libc
    const platformMetadata =
      platform && arch ? `${platform}-${arch}${libc ? `-${libc}` : ''}` : ''
    const pathsWithPlatform = platformMetadata
      ? [...sourcePaths, `__PLATFORM_METADATA__:${platformMetadata}`]
      : sourcePaths
    const currentCacheHash = computeSourceHash(pathsWithPlatform)
