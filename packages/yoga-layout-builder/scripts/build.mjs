...
async function cloneYogaSource() {
  ...
  logger.substep(`Cloning Yoga ${YOGA_VERSION} (${YOGA_SHA.slice(0, 8)})...`)
  ...
  if (clonedSha !== YOGA_SHA) {
    throw new Error(
      `SHA mismatch: expected ${YOGA_SHA}, got ${clonedSha}. ` +
        `The tag ${YOGA_VERSION} may have been updated. Please update sources.yoga.ref in package.json.`,
    )
  }

  logger.success(
    `Yoga ${YOGA_VERSION} cloned and verified (${YOGA_SHA.slice(0, 8)})`,
  )
...
async function extractSourceForMode() {
  ...
  logger.step(`Extracting Yoga Source to ${BUILD_MODE} Build`)
...
async function build() {
  ...
  logger.success(`JS glue code created: ${jsOutput}`)
  logger.success(`WASM module created: ${wasmOutput}`)
...
async function optimize() {
  ...
  if (BUILD_MODE === 'dev') {
    logger.substep('Skipping wasm-opt (dev mode - faster builds)')
    return
  }
  ...
  logger.step('Optimizing WASM with wasm-opt (prod mode)')
