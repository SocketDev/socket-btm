...
// Entries are grouped by build domain / blast-radius (bin-infra, then
// binflate/binject/binpress, then build-infra, then upstream submodule
// bumps, …) with explanatory comments tying adjacent entries together.
// socket-lint: allow object-property-order
export const CASCADE_RULES = {
  ...
  //     binpress/bin-stub-builder/libpq-builder)
  ...
  'packages/bin-stub-builder/docker/': ['stubs', 'binpress', 'node-smol'],
  'packages/bin-stub-builder/make/': ['stubs', 'binpress', 'node-smol'],
  'packages/bin-stub-builder/scripts/': ['stubs', 'binpress', 'node-smol'],
  'packages/bin-stub-builder/src/': ['stubs', 'binpress', 'node-smol'],
  ...
  // imported by both bin-stub-builder/scripts/repo/build.mts and
  ...
  // lief-builder/lib/ensure-lief.mts re-exports from scripts/repo/build.mts
  ...
  // Upstream submodule bumps (packages/*/upstream/<name>/). A submodule
  // bump changes exactly the vendored source tree at this path; the
  // builder(s) that compile against it need cache invalidation the same
  // as a first-party source edit above. One entry per submodule so a
  // bump to any single upstream can't cross-invalidate unrelated
  // builders. Keep in lock-step with the `upstreams` submodule paths in
  // .config/lockstep.json — a new lockstep version-pin row over a
  // submodule needs a matching entry here (enforced by
  // test/validate-cache-versions.test.mts).
  'packages/node-smol-builder/upstream/node/': ['node-smol'],
  'packages/node-smol-builder/upstream/liburing/': ['node-smol'],
  'packages/node-smol-builder/upstream/uSockets/': ['node-smol'],
  'packages/node-smol-builder/upstream/uWebSockets/': ['node-smol'],
  'packages/lsquic-infra/upstream/lsquic/': ['node-smol'],
  'packages/lsquic-infra/upstream/ls-qpack/': ['node-smol'],
  'packages/node-smol-builder/upstream/libqrencode/': ['node-smol'],
  'packages/node-smol-builder/upstream/md4c/': ['node-smol'],
  'packages/node-smol-builder/upstream/tree-sitter/': ['node-smol'],
  'packages/node-smol-builder/upstream/semver/': ['node-smol'],
  'packages/curl-builder/upstream/curl/': ['curl', 'stubs', 'node-smol'],
  'packages/curl-builder/upstream/mbedtls/': ['curl', 'stubs', 'node-smol'],
  'packages/onnxruntime-builder/upstream/onnxruntime/': ['onnxruntime'],
  'packages/dawn-builder/upstream/dawn/': ['dawn'],
  'packages/opentui-builder/upstream/opentui/': ['opentui', 'node-smol'],
  'packages/yoga-layout-builder/upstream/yoga/': ['yoga-layout'],
  ...
  'packages/binject/upstream/libdeflate/': ['binject', 'binpress', 'node-smol'],
  'packages/binject/upstream/cJSON/': ['binject', 'binpress', 'node-smol'],
  ...
  'packages/libpq-builder/upstream/postgres/': ['libpq'],
  'packages/boringssl-builder/upstream/boringssl/': ['boringssl'],
}
