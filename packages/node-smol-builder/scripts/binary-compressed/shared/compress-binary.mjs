...
function parseArgs() {
  ...
  if (args.length < 2) {
    logger.error(
      'Usage: compress-binary.mjs <input> <output> [--quality=lzma|lzfse|xpress]',
    )
    logger.error('')
...
async function ensureToolBuilt(config) {
  const toolsOutDir = path.join(TOOLS_DIR, 'out')
  const toolPath = path.join(toolsOutDir, config.toolName)
