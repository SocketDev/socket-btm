 ...
 *   2. A planted secret in the bundle IS detected in the extracted region
 *      (the leak detector works).
 *   3. A clean bundle yields NO secret hits in the extracted region — even
 *      though the whole binary does contain Node's own crypto strings (proving
 *      the extraction, not the binary, is what we scan).
 *
