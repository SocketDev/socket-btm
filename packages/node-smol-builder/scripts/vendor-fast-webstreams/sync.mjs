...
function wireNativeChunkPool(content, filename) {
  ...
  try {
    ...
    logger.warn('  Skipping chunk pool wiring: acorn parse failed for reader.js')
    return content
  }
  ...
  if (!targetNode) {
    logger.warn('  Skipping chunk pool wiring: _resolveReadResult not found in reader.js')
    return content
  }
  ...
  if (useStrictEnd !== -1) {
    content = content.slice(0, useStrictEnd + 1) + poolImport + content.slice(useStrictEnd + 1)
  }
