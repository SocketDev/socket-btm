...
All packages use the `BUILD_MODE` environment variable to select between dev and prod builds:

```bash
# Dev build (default for local development)
pnpm run build

# Prod build (default in CI)
BUILD_MODE=prod pnpm run build
```

**How it works** (in `lib/constants.mjs`):
- If `BUILD_MODE` env var is set, use that value
- If `CI` env var is set, default to `'prod'`
- Otherwise, default to `'dev'`

**Output directories**: `build/dev/out/Final/` or `build/prod/out/Final/`

| Mode | Debug Symbols | Size | Use Case |
|------|---------------|------|----------|
| `dev` | Enabled | Larger | Local development |
| `prod` | Stripped | Smaller | CI/CD releases |
...
### WASM Pipeline
- `wasm-pipeline` - WebAssembly build orchestration (uses native WebAssembly API)
