...
**CRITICAL for Node.js internal code (additions/, fast-webstreams/):**
...
- Check additions/source-patched/lib/internal/socketsecurity/smol/*.js for SafePromiseAllSettled usage
- Check additions/source-patched/deps/fast-webstreams/*.js for SafePromiseAllReturnVoid usage
- Verify sync scripts (vendor-fast-webstreams/sync.mjs) inject primordials when syncing from upstream
</pattern>

<pattern name="cross_platform_binary_bugs">
**CRITICAL for binary tooling (binject, binpress, bin-stubs):**
...
**CRITICAL for Windows binary tooling:**
...
**CRITICAL for test reliability and CI stability:**
...
Cross-platform binary processing bugs (CRITICAL - causes silent failures):
...
- Critical for: binject, binpress, bin-stubs, any code manipulating binaries cross-platform
...
Windows PE binaries missing required resource sections:
...
- Example fix for Makefile.windows:
  ```makefile
  RESOURCE_RC = src/socketsecurity/bin-stubs/stub.rc
...
buildBinSuitePackage({
  ...
  beforeBuild: async () => {
    ...
    await ensureCurlForStubs()  // bin-stubs
...
- `ensureCurlForStubs()` - Downloads curl+mbedTLS from releases (bin-stubs)
