...
describe.skipIf(skipTests)('metadata Reading Correctness', () => {
  ...
  beforeAll(async () => {
    ...
    // Execute binary to trigger extraction to cache (needed by libc/metadata tests)
    const result = await spawn(binaryPath, ['--version'], {
      env: { ...process.env, SOCKET_HOME: testCacheDir },
      timeout: 30_000,
    })
    if (result.code !== 0) {
  ...
  it('should correctly read platform byte from binary', async () => {
    ...
    // Verify the platform byte is valid (0=linux, 1=darwin, 2=win32)
    const expectedPlatform =
      platformByte === 0 ? 'linux' : platformByte === 1 ? 'darwin' : 'win32'
    expect(expectedPlatform).toBe(
  ...
  it('should correctly read architecture byte from binary', async () => {
    ...
    // Verify the arch byte is valid and matches current platform
    const expectedArch =
      archByte === 0
        ? 'x64'
        : archByte === 1
          ...
          : archByte === 2
            ? 'ia32'
            : 'arm'
    expect(expectedArch).toBe(os.arch())
  })
  ...
  it('should match current platform metadata', async () => {
    ...
    expect(platformByte).toBe(PLATFORM_VALUES[os.platform()])
    expect(archByte).toBe(ARCH_VALUES[os.arch()])
  ...
  it('should write metadata that matches binary bytes exactly', async () => {
    ...
    const platformMapping: Record<number, string> = { 0: 'linux', 1: 'darwin', 2: 'win32' }
    const archMapping: Record<number, string> = { 0: 'x64', 1: 'arm64', 2: 'ia32', 3: 'arm' }
    expect(platformMapping[platformByte]).toBe(
      process.platform === 'win32' ? 'win32' :
      process.platform === 'darwin' ? 'darwin' : 'linux',
    )
    expect(archMapping[archByte]).toBe(os.arch())

    // Verify cache metadata has expected structure
