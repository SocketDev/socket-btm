 ...
 * Node.js compilation and contains just the version string (e.g., "25.8.0").
 *
 * Section locations:
 ...
 * The section contains a null-terminated version string (e.g., "25.8.0").
 *
...
static char* extract_version_from_section_content(const uint8_t *data, size_t size) {
    ...
    /* Validate version string length (e.g., "25.8.0" = 6 chars). */
    if (len == 0 || len > 15) {
        return NULL;
    }
