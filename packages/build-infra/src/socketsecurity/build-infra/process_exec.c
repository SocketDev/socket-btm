...
char* spawn_command(const char* command, const char* const args[], size_t max_output_size) {
    ...
    // First pass: count args and cache their lengths (avoid redundant strlen calls)
    int argc = 0;
    while (args[argc] != NULL) {
        argc++;
    }

    size_t* arg_lengths = malloc(argc * sizeof(size_t));
    if (!arg_lengths) {
        fprintf(stderr, "Error: Failed to allocate arg lengths buffer\n");
        return NULL;
    }
    ...
    for (int i = 0; i < argc; i++) {
        ...
        cmdline_size += arg_lengths[i] * 2 + 4;  // *2 for escaping, +4 for quotes+space
    }
