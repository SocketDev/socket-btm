...
async function main(): Promise<void> {
  try {
    ...
    await runPnpm(['--filter', 'bin-stub-builder', 'build'])
    ...
    const results = await Promise.allSettled([
      ...
      // onnxruntime + yoga both bootstrap the shared ~/.emsdk via
      // ensureEmscripten, and emsdk install/activate is not concurrency-safe
      // across processes: activate races the sibling's in-flight install and
      // fails with "tool is not installed and therefore cannot be activated".
      // Run the WASM builders sequentially; they still overlap binsuite.
      (async () => {
        await runPnpm(['--filter', 'onnxruntime-builder', 'build'])
        await runPnpm(['--filter', 'yoga-layout-builder', 'build'])
      })(),
    ])
