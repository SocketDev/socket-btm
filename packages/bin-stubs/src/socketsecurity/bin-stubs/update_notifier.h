...
static void print_box_line(int width, bool utf8, const char *left, const char *right,
     ...
     * Max width ~50, so 256 bytes is plenty. */
    char line[256];
    char *p = line;
    ...
    if (utf8) {
        ...
        for (int i = 0; i < width; i++) {
            p += sprintf(p, "%s", BOX_HORIZONTAL);
        }
        sprintf(p, "%s\n", right);
    } else {
        p += sprintf(p, "%s", left_ascii);
        for (int i = 0; i < width; i++) {
...
static void print_box_content(int width, bool utf8, const char *content) {
    ...
    if (utf8) {
        p += sprintf(p, "%s  %s", BOX_VERTICAL, content);
    } else {
        p += sprintf(p, "%s  %s", BOX_VERTICAL_ASCII, content);
    }

    /* Add padding spaces directly to buffer. */
