...
function parsePypi(version) {
  ...
  for (let i = 0; i < releaseParts.length; i++) {
    ...
    if (isDigits(part)) {
      ...
      const numPrefixMatch = RegExpPrototypeExec(
  ...
  if (remaining) {
    ...
    // Dev release: dev
    const devMatch = RegExpPrototypeExec(/(?:^|[._-])dev\.?(\d+)?/, normalized)
    if (devMatch) {
      hasDev = true
...
function parseComparator(comp, ecosystem) {
  ...
  const wildMatch = RegExpPrototypeExec(
    /^(\d+)(?:\.(\d+|x|X|\*))?(?:\.(\d+|x|X|\*))?$/,
    comp,
  )
  if (wildMatch) {
    return {
      ...
      minor:
        wildMatch[2] && !RegExpPrototypeTest(/[xX*]/, wildMatch[2])
          ? parseDigits(wildMatch[2])
          : undefined,
      patch:
        wildMatch[3] && !RegExpPrototypeTest(/[xX*]/, wildMatch[3])
