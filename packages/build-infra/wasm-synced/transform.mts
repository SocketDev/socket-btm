...
export async function applyCommonTransforms(options) {
  ...
  ancestor(ast, {
    ...
    FunctionDeclaration(node) {
      ...
      // NOTE: async-keyword removal below hoisted into a shared helper
      // to avoid the off-by-one that ate the following '(' when the
      // minifier omitted the space (e.g. `async()=>{...}` became
      // `=>{...}` with a missing open paren).
