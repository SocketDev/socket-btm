...
function extractCompressedData(binaryData) {
  ...
  // Compressed data starts after: marker (32 bytes) + compressed_size (8 bytes) + uncompressed_size (8 bytes) + cache_key (16 bytes) + platform_metadata (4 bytes) = 68 bytes total.
  ...
  describe('Stub binary structure', () => {
    ...
    it.skipIf(!IS_MACOS)(
      ...
      async () => {
        ...
        const smolSegment = segments.find(s => s.segmentName === 'SMOL')
        ...
        const pressedDataSection = smolSegment.sections.find(
          s => s.sectionName === '__PRESSED_DATA',
        )
        expect(pressedDataSection).toBeDefined()
        expect(pressedDataSection.segmentName).toBe('SMOL')
  ...
  describe('Node binary extraction', () => {
    ...
    it.skipIf(!IS_MACOS)(
      ...
      async () => {
        ...
        const nodeSeaSegment = segments.find(s => s.segmentName === 'NODE_SEA')
