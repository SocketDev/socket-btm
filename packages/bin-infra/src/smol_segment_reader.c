...
int smol_read_metadata(int fd, smol_metadata_t *metadata) {
    ...
    /* Read platform metadata (PLATFORM_METADATA_LEN bytes: platform, arch, libc, compression_algorithm, reserved). */
...
int smol_validate_metadata(const smol_metadata_t *metadata, size_t max_size) {
    ...
    uint8_t compression = metadata->platform_metadata[3];
    uint8_t reserved = metadata->platform_metadata[4];
    ...
    if (compression > SMOL_COMPRESSION_LZMS) {
        fprintf(stderr, "Error: Invalid compression algorithm: %u (expected 0-2)\n", compression);
        return -1;
    }

    /* Validate reserved byte is 0 (for forward compatibility). */
    if (reserved != 0) {
        fprintf(stderr, "Error: Invalid reserved byte (expected 0, got %u)\n", reserved);
        return -1;
    }
