...
extern "C" int binject_elf_lief_batch(
  ...
  if (!executable || !output) {
    ...
    // Add VFS config note if provided (VFS_CONFIG_SIZE bytes SVFG format)
    if (vfs_config_data) {
      printf("Preparing SMOL_VFS_CONFIG note with %d bytes...\n", VFS_CONFIG_SIZE);
      std::vector<uint8_t> vfs_config_vec(vfs_config_data, vfs_config_data + VFS_CONFIG_SIZE);
      int result = elf_note_utils::replace_or_add(binary.get(), ELF_NOTE_SMOL_VFS_CONFIG, vfs_config_vec);
      if (result != 0) {
        fprintf(stderr, "Error: Failed to add SMOL_VFS_CONFIG note\n");
        return BINJECT_ERROR;
      }
    }
