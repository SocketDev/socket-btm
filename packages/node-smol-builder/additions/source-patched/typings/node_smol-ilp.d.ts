 ...
 *
 * A high-performance client for sending time-series data using the
 * InfluxDB Line Protocol (ILP). Compatible with QuestDB, InfluxDB,
 * and other ILP-compatible databases.
 */
declare module 'node:smol-ilp' {
  ...
  export const TimeUnit: Readonly<{
    ...
    convert(value: number | bigint, fromUnit: TimeUnitValue, toUnit: TimeUnitValue): bigint;
  }>;
  ...
  export class Sender extends EventEmitter {
    ...
    timestampColumn(name: string, value: number | bigint, unit?: TimeUnitValue): this;
     ...
     */
    static sendOnce(options: SenderOptions, tableName: string, rowsOrRow: RowData | RowData[]): Promise<void>;
    ...
    on<K extends keyof SenderEvents>(event: K, listener: (...args: SenderEvents[K]) => void): this;
    once<K extends keyof SenderEvents>(event: K, listener: (...args: SenderEvents[K]) => void): this;
    off<K extends keyof SenderEvents>(event: K, listener: (...args: SenderEvents[K]) => void): this;
    removeListener<K extends keyof SenderEvents>(event: K, listener: (...args: SenderEvents[K]) => void): this;
    emit<K extends keyof SenderEvents>(event: K, ...args: SenderEvents[K]): boolean;
