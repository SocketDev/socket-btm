...
describe('build-env', () => {
  ...
  describe(commandExists, () => {
    it('should return true for known commands', async () => {
      ...
      await expect(commandExists('node')).resolves.toBeTruthy()
    })
  ...
  describe(checkPython, () => {
    it('should detect Python installation', async () => {
      ...
      if (result.available) {
        ...
        expectTypeOf(result.meetsRequirement).toBeBoolean()
      }
    })
    ...
    it('should check version requirements', async () => {
      ...
      if (result.available) {
        ...
        if (major >= 3 && minor >= 8) {
          ...
          expect(result.meetsRequirement).toBeFalsy()
        }
      }
    })
  })
  ...
  describe('environment detection consistency', () => {
    ...
    it('should detect platform-appropriate commands', async () => {
      ...
      if (platform === 'win32') {
        ...
        await expect(commandExists('cmd')).resolves.toBeTruthy()
      } else {
