 ...
 * (separating "where does the binary live" from "how do I exec it"), adapted
 * for our pnpm-exec-driven fleet.
 *
 * One place to change when the underlying tool swaps. When the fleet migrates
 * esbuild → rolldown, only `resolveBundler()` changes; every caller continues
 * to invoke the same resolver and the swap is transparent.
 *
 * Usage: const { args, envs } = resolveLinter({ mode: 'check' }) await
 * spawn('pnpm', ['exec', ...args], { env: { ...process.env, ...envs } })
 *
 * Or via the convenience runner: await runResolved(resolveLinter({ mode:
 * 'check' }), { cwd })
 *
 * Tool selection is a single fleet-wide decision per resolver, not per-repo. If
 ...
 * including the tool name as the first element. `envs` is environment variables
 * the tool needs (e.g. `OXLINT_LOG=warn`).
 */
...
/**
 * Resolve the fleet's bundler. Returns esbuild today; flips to rolldown when
 * the migration documented in `socket-packageurl-js/docs/rolldown-migration.md`
 * lands fleet-wide.
 *
...
/**
 * Convenience: run a `ResolvedTool` via `pnpm exec` and return the result.
 * Throws `SpawnError` on non-zero exit unless `capture` is true (then the
...
/**
 * Best-effort detection: is the named tool resolvable from the given cwd's
