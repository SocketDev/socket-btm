 ...
 *
 * Usage:
 *   node scripts/benchmark-startup.mts ./my-app [--runs 30] [--warmup 0]
 *   node scripts/benchmark-startup.mts ./app1 ./app2 --runs 50
 *
...
for (let i = 0; i < args.length; i++) {
  ...
  if (arg === '--runs' || arg === '-r') {
    ...
    if (Number.isNaN(runs) || runs < 1) {
      logger.error(
        `Invalid --runs value: ${args[i]}. Must be a positive number.`,
      )
      throw new Error(
        `Invalid --runs value: ${args[i]}. Must be a positive number.`,
      )
    ...
    if (Number.isNaN(warmup) || warmup < 0) {
      logger.error(
        `Invalid --warmup value: ${args[i]}. Must be a non-negative number.`,
      )
      throw new Error(
        `Invalid --warmup value: ${args[i]}. Must be a non-negative number.`,
      )
...
Usage:
  node scripts/benchmark-startup.mts <binary1> [binary2...] [options]
...
Examples:
  node scripts/benchmark-startup.mts ./my-app
  node scripts/benchmark-startup.mts ./app-with-cache ./app-no-cache --runs 50
  node scripts/benchmark-startup.mts ./my-app --warmup 3 --runs 100
...
async function main() {
  ...
  if (binaries.length === 0) {
    throw new Error(
      'No binaries specified\nUsage: node scripts/benchmark-startup.mts <binary1> [binary2...] [options]\nRun with --help for more information',
    )
  }
