// ============================================================================
// lief_write_diagnostics.h — Debug helpers for LIEF binary write failures
// ============================================================================
//
// WHAT THIS FILE DOES
// Provides inline diagnostic functions that check disk space, memory, and
// directory permissions before LIEF attempts to write a modified binary.
// Output is controlled by the DEBUG=1 environment variable.
//
// WHY IT EXISTS
// LIEF's binary->write() can fail silently on some platforms (especially
// musl-based Alpine Linux). These helpers print useful system info so
// developers can figure out why a write failed — was it a full disk,
// a read-only directory, or something else?
// ============================================================================
...
static inline void lief_check_system_resources(void) {
    ...
    if (DEBUG_IS_ENABLED()) {
        // SECURITY: These system() calls are SAFE because:
        // 1. Debug-only code (not production)
        // 2. Completely hardcoded commands - no user input
        // 3. Return values explicitly ignored
        // 4. Used only for diagnostics during development
