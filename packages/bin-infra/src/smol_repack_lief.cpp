...
extern "C" int smol_repack_lief(
    ...
    try {
        ...
        fprintf(stderr, "Error: Unknown exception during LIEF repack\n");
        return -1;
    }
}
 ...
 * Similar to Mach-O version but handles ELF sections instead of segments.
 */
extern "C" int smol_repack_lief_elf(
    ...
    try {
        printf("Repacking SMOL section with LIEF (ELF)...\n");
        printf("  Stub: %s\n", stub_path);
        printf("  New content size: %zu bytes\n", section_size);
        printf("  Output: %s\n", output_path);

        // Parse the compressed stub as ELF.
        std::unique_ptr<LIEF::ELF::Binary> binary =
            LIEF::ELF::Parser::parse(stub_path);
        ...
        // Match postject's spirit: minimal Note creation, then set name+description, then add
        const char* note_name = ELF_SECTION_PRESSED_DATA;

        // Find and remove existing PT_NOTE segment
        ...
        if (!existing_note) {
            fprintf(stderr, "Error: %s PT_NOTE not found in stub\n", note_name);
            return -1;
        }

        printf("  Found existing %s PT_NOTE, removing and recreating...\n", note_name);

        // CRITICAL: Remove old PT_NOTE BEFORE adding new one (same as postject remove+add)
        binary->remove(*existing_note);

        printf("  Removed old %s PT_NOTE\n", note_name);

        // Create new PT_NOTE with updated content - match postject's EXACT pattern (lines 71-74)
        std::vector<uint8_t> new_content(section_data, section_data + section_size);

        LIEF::ELF::Note note;
        note.name(note_name);
        note.description(new_content);
        binary->add(note);

        printf("  Created new SMOL PT_NOTE with %zu bytes\n", section_size);
        ...
        // Write modified binary.
        printf("  Writing modified ELF binary...\n");

        // CRITICAL: Explicitly set config.notes = false to avoid PHT relocation
        //
        // WHY NOT config.notes = true?
        // We are ONLY updating the .pressed_data section, NOT modifying PT_NOTE segments.
        // The stub already has standard PT_NOTE segments (.note.gnu.build-id, .note.gnu.property)
        // from the build process. The application PT_NOTE segments (NODE_SEA_BLOB, SMOL_VFS_BLOB)
        // are INSIDE the compressed data (added during injection step), not in the stub's program
        // headers.
        //
        // Setting config.notes = true causes LIEF to rebuild/relocate PT_NOTE segments and the
        // program header table. This relocation breaks the dynamic linker's expectations about
        // where to find program headers, causing segmentation faults at runtime.
        ...
        //
        // This approach works because binary->remove() and binary->add() modify the in-memory
        // representation, and raw() serializes that directly without rebuilding segments.
        std::vector<uint8_t> raw_binary = binary->raw();
