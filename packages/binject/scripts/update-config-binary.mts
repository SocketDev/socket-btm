...
const SMOL_CONFIG_BINARY_SIZE = 1192
 ...
 */
export function parseAndSerialize(jsonString: string) {
  let config
  try {
 ...
 * @throws {SmolConfigValidationError} - If validation fails
 */
export function serializeSmolConfig(config: Record<string, unknown>) {
 ...
 */
export function serializeUpdateConfig(config: Record<string, unknown>) {
 ...
 */
export function validatePromptDefault(
  name: string,
  value: unknown,
  defaultValue: string,
) {
