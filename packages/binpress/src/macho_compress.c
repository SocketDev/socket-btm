...
static compression_algorithm parse_algorithm(const char *quality, const char **name) {
    if (quality == NULL || strcmp(quality, "lzfse") == 0) {
        ...
        *name = "ZLIB";
        ...
        fprintf(stderr, "⚠ Unknown quality '%s', defaulting to LZFSE\n", quality);
        *name = "LZFSE";
        return COMPRESSION_LZFSE;
    }
}
...
static int parse_args(int argc, char *argv[], compress_config *config) {
    if (argc >= 2) {
        ...
        if (strcmp(argv[1], "--help") == 0 || strcmp(argv[1], "-h") == 0) {
            printf("Usage: %s <input> <output> [--quality=lzfse|lzma|lz4|zlib]\n", argv[0]);
            printf("\nCompress Mach-O binaries using Apple Compression framework.\n");
            printf("\nArguments:\n");
            printf("  input   Path to input binary\n");
            printf("  output  Path to output compressed binary\n");
            printf("\nOptions:\n");
            printf("  --quality=ALGO  Compression algorithm (lzfse, lzma, lz4, zlib)\n");
            printf("                  Default: lzfse\n");
