...
export async function installPythonPackage(
  ...
  if (consumerPackageJsonPath) {
    ...
    } = loadPythonVersions(consumerPackageJsonPath)
    const version = consumerVersions[packageName]
    if (!version) {
      throw new Error(
        `No pinned version found for ${packageName}. Add to externalTools in build-infra/package.json or ${consumerPackageJsonPath} with type: "python"`,
      )
    }
