 ...
 * Usage: node scripts/repo/update-node-version.mts <new-version>
 * Example: node scripts/repo/update-node-version.mts 24.12.0.
 */
...
async function main(): Promise<void> {
  ...
  if (!newVersion) {
    ...
    logger.fail('Example: node scripts/repo/update-node-version.mts 24.12.0')
    process.exitCode = 1
    return
  }
  ...
  try {
    ...
    if (!shaLine) {
      ...
      // Matches the `# node-<version> [sha256:<hex>]` comment line
      // immediately preceding the node submodule stanza, capturing the
      // `[submodule "..."]` header (group 1) so it can be re-emitted
      // unchanged after the version comment is replaced.
