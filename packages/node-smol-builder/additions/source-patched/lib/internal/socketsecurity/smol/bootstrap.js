...
const { mount } = require('internal/socketsecurity/smol/mount')
...
function hasVFSCompat() {
  try {
    // eslint-disable-next-line no-undef
    const vfsBinding = internalBinding('smol_vfs')
    return !!(vfsBinding && vfsBinding.hasVFSBlob())
  } catch {
    return false
  }
}
...
function getCacheDir() {
  ...
  // Priority 3: Default $HOME/.socket/_dlx (os.homedir() checks HOME/USERPROFILE)
...
function setupProcessForSmol() {
  ...
  const fakeArgvEnvName = process.env.SMOL_FAKE_ARGV_NAME || 'SMOL_FAKE_ARGV'
  const fakeArgvEnv = process.env[fakeArgvEnvName]
  ...
  if (!process.smol) {
    ...
    function getVFSLoader() {
      if (!vfsLoaderCache) {
        vfsLoaderCache = require('internal/socketsecurity/vfs/loader')
      }
    ...
    process.smol = {
      __proto__: null,
      // Path properties
      stubPath: process.env.SMOL_STUB_PATH || EXECPATH,
      execPath: EXECPATH,
      cacheDir: getCacheDir(),
      cacheKey: process.env.SMOL_CACHE_KEY,
      // Core properties
  ...
  delete process.env.SMOL_CACHE_KEY
  delete process.env.SMOL_STUB_PATH

  // Delete the fake argv control variable (may have custom name).
  if (fakeArgvEnvName) {
    delete process.env[fakeArgvEnvName]
    delete process.env.SMOL_FAKE_ARGV_NAME
  }
