...
-> vfs/sqlite_provider.js (SmolSqliteProvider — lazy-loaded)
-> vfs/pg_provider.js (SmolPgProvider — lazy-loaded)

Additional exports: `SmolSqliteProvider` and `SmolPgProvider` classes
are attached as lazy getters on the public module. They let SEA apps
plug VFS-backed storage into `node:smol-sql` without paying startup
cost when unused.
