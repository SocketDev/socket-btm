...
binject_format_t binject_detect_format(const char *executable) {
    ...
    struct stat st;
    int stat_valid = (stat(path_to_open, &st) == 0);
...
#ifdef _WIN32
/* Windows: map stat() to _stat() for basic file existence checks.
 * Note: For symlink-safe checks, use POSIX_LSTAT from posix_compat.h. */
#define stat _stat
#endif
