...
// Documentation: docs/additions/lib/internal/socketsecurity/purl.js.md
...
class PurlError extends ErrorCtor {
...
ObjectSetPrototypeOf(PurlError.prototype, ErrorCtor.prototype)
...
function decodeQualifierValue(str) {
  ...
  // Reject control chars / null bytes / CRLF AFTER decoding, matching
  // the sanitization urlDecode() applies to other components. Without
  // this, a qualifier like `?arch=%00mal` or `?flag=%0D%0A...` would
  // flow unsanitized into SBOM JSON, logs, headers — sanitizeDecoded
  // is the documented defense (see purl.js HIGH-2 / CRITICAL-2 notes).
  return sanitizeDecoded(decoded)
}
...
function collapseSlashes(str) {
  ...
  if (segStart < len) {
    ArrayPrototypePush(segments, StringPrototypeSlice(str, segStart, len))
  }
...
function parse(purl) {
  ...
  if (type === 'npm') {
    ...
    normalizedName = StringPrototypeToLowerCase(normalizedName)
    // Replace underscores with dashes. Collect slices and join once instead
    // of repeated string concatenation (O(n²) → O(n)).
    let fromIndex = 0
    let index = 0
    if (StringPrototypeIndexOf(normalizedName, '_') !== -1) {
      const parts = []
      while (
        ...
        ArrayPrototypePush(
          parts,
          StringPrototypeSlice(normalizedName, fromIndex, index),
          '-',
        )
        fromIndex = index + 1
      }
      ArrayPrototypePush(parts, StringPrototypeSlice(normalizedName, fromIndex))
      normalizedName = ArrayPrototypeJoin(parts, '')
    }
    // Replace periods with dashes (per gold standard).
    if (StringPrototypeIndexOf(normalizedName, '.') !== -1) {
      const parts = []
      fromIndex = 0
      while (
