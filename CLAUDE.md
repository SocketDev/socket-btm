...
🚨 **Hot-path invariants** + per-tier deep references in [`docs/claude.md/repo/architecture.md`](docs/claude.md/repo/architecture.md): publish dispatch order, additions `-fno-exceptions` allocation pattern, 1 patch / 1 file unified-diff format, SEA `Module.createRequire`, gypi source-path resolution (enforced by `.claude/hooks/fleet/gypi-source-path-guard/`). Per-tier docs at [`docs/references/btm-*.md`](docs/references/).
