 ...
 *
 * Usage:
 * node scripts/repo/bump-cache-version.mts <package-name>
 * node scripts/repo/bump-cache-version.mts onnxruntime
 * node scripts/repo/bump-cache-version.mts --all.
 *
 * Examples: node scripts/repo/bump-cache-version.mts node-smol # v9 -> v10 node
 * scripts/repo/bump-cache-version.mts --all # Bump all packages.
 */
...
export async function main(): Promise<void> {
  ...
  if (!packageName) {
    logger.fail(
      'Usage: node scripts/repo/bump-cache-version.mts <package-name>',
    )
    logger.log('or: node scripts/repo/bump-cache-version.mts --all')
    logger.log('')
