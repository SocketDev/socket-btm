...
import { getLatestFinalBinary } from '../helpers/paths.mjs'
...
function extractCompressedData(binaryData) {
  const magicMarker = Buffer.from(SMOL_PRESSED_DATA_MAGIC_MARKER, 'utf-8')
  ...
  // Compressed data starts after header (marker + metadata = TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG bytes)
  ...
  describe('Code signing preserves metadata format', () => {
    it('should preserve 3-byte metadata format after signing', async () => {
      ...
      const markerIndex = binaryData.indexOf(
        Buffer.from(SMOL_PRESSED_DATA_MAGIC_MARKER, 'utf-8'),
      )
      ...
      const metadataOffset =
        markerIndex + TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG - 3
    ...
    it('should preserve magic marker after signing', async () => {
      ...
      expect(actualMarker.toString('utf-8')).toBe(
        SMOL_PRESSED_DATA_MAGIC_MARKER,
      )
    })
    ...
    it('should preserve header size after signing', async () => {
      ...
      // Verify TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG is 68 bytes (marker 32 + metadata 36)
      const dataOffset = markerIndex + TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG
