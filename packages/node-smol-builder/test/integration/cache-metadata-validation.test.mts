...
describe.skipIf(skipTests)('cache Metadata Validation', () => {
  ...
  it('should write compression_algorithm="zstd" to cache metadata', async () => {
    ...
    // Validate compression_algorithm field exists and is "zstd"
    expect(metadata.extra).toBeDefined()
    expect(metadata.extra.compression_algorithm).toBe('zstd')
  })
  ...
  it('should consistently write zstd across multiple executions', async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      for (
        let j = 0, { length: cacheKeyDirsLength } = cacheKeyDirs;
        j < cacheKeyDirsLength;
        j += 1
      ) {
        const cacheKeyDir = cacheKeyDirs[j]
    ...
    // Should only have one algorithm: zstd
    expect(algorithms.size).toBe(1)
    expect(algorithms.has('zstd')).toBeTruthy()
  })
})
