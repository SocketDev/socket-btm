...
export async function ensureLzfse({ packageDir }) {
  const lzfseUpstream = path.join(packageDir, '../lief-builder/upstream/lzfse')
  ...
  logger.log(
    'Running: git submodule update --init --recursive packages/lief-builder/upstream/lzfse',
  )
  ...
  try {
    result = await spawn(
      ...
      [
        ...
        '--init',
        '--recursive',
        'packages/lief-builder/upstream/lzfse',
      ],
      {
    ...
    throw new Error(
      'lzfse submodule not initialized and git command failed. ' +
        'Ensure .git directory exists and run: git submodule update --init --recursive packages/lief-builder/upstream/lzfse\n' +
        `Error: ${error.message}`,
      { cause: error },
    )
  }
  ...
  if (result.code !== 0) {
    throw new Error(
      `Failed to initialize lzfse submodule (exit code ${result.code}). ` +
        'Run: git submodule update --init --recursive packages/lief-builder/upstream/lzfse',
    )
  }
