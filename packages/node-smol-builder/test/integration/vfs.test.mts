...
import { SMOL_VFS_BLOB, runBinject } from '../helpers/binject.mjs'
...
`,
        ...
        const execResult = await spawn(seaBinary, [])
        expect(execResult.code).toBe(0)
        expect(execResult.stdout).toContain('ASYNC_READFILE_DATA=async-test-content')
      })
...
`,
        ...
        const vfsDir = path.join(testDir, 'vfs-content')
        await safeMkdir(vfsDir)
        await fs.writeFile(path.join(vfsDir, 'test.txt'), 'promise-test-content')
        ...
        const execResult = await spawn(seaBinary, [])
        expect(execResult.code).toBe(0)
        expect(execResult.stdout).toContain('PROMISE_READFILE_DATA=promise-test-content')
      })
...
`,
        ...
        const execResult = await spawn(seaBinary, [])
        expect(execResult.code).toBe(0)
        expect(execResult.stdout).toContain('REALPATH_NATIVE=/snapshot/test.txt')
...
`,
        ...
        const execResult = await spawn(seaBinary, [])
        expect(execResult.code).toBe(0)
        expect(execResult.stdout).toContain('CAPTURED_DESTRUCTURED=captured-works')
...
`,
        ...
        const execResult = await spawn(seaBinary, [])
        expect(execResult.code).toBe(0)
        expect(execResult.stdout).toMatch(/ENTRY=file\.txt:isFile=true:isDir=false/)
        expect(execResult.stdout).toMatch(/ENTRY=subdir:isFile=false:isDir=true/)
      })
...
`,
      ...
      it('should support async fs.glob() on VFS paths', async () => {
