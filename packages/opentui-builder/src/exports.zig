...
var g_global_ref: ?c.napi_ref = null;

fn ensureGlobalRef(env: napi.napi_env) void {
    if (g_global_ref != null) return;
    var global: napi.napi_value = null;
    _ = c.napi_get_global(env, &global);
    g_global_ref = napi.createRef(env, global);
}

fn getCachedGlobal(env: napi.napi_env) napi.napi_value {
    if (g_global_ref) |ref| {
        return napi.getRefValue(env, ref) orelse {
...
fn readColorArray(env: napi.napi_env, value: napi.napi_value) ?[4]f32 {
    return napi.getColorFromTypedArray(env, value);
}

fn getStringSmall(env: napi.napi_env, value: napi.napi_value, small_buf: []u8, large_buf: []u8) ?[]const u8 {
    const len = napi.getStringLength(env, value) orelse return null;
    if (len < small_buf.len) {
        return napi.getString(env, value, small_buf);
    }
    if (len < large_buf.len) {
        return napi.getString(env, value, large_buf);
    }
    return napi.getString(env, value, large_buf);
}
...
// ── Performance: Float32Array color variants (avoid 4x scalar unbox) ──

pub fn jsBufferDrawTextFA(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [7]napi.napi_value = .{ null, null, null, null, null, null, null };
    if (!requireArgs(env, info, &args, 7, "bufferDrawTextFA(buf, text, x, y, fgF32, bgF32, attrs)")) return null;
    const buf_ptr = napi.unwrapPointer(env, args[0], OptimizedBuffer) orelse return null;
    var small_buf: [256]u8 = undefined;
    var large_buf: [65536]u8 = undefined;
    const text = getStringSmall(env, args[1], &small_buf, &large_buf) orelse return null;
    const x = napi.getU32(env, args[2]) orelse return null;
    const y = napi.getU32(env, args[3]) orelse return null;
    const fg = readColorArray(env, args[4]) orelse return null;
    const bg = readColorArray(env, args[5]) orelse return null;
    const attrs = napi.getU32(env, args[6]) orelse return null;
    bufferDrawText(buf_ptr, text.ptr, text.len, x, y, &fg, &bg, attrs);
    return napi.getUndefined(env);
}

pub fn jsBufferSetCellFA(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [6]napi.napi_value = .{ null, null, null, null, null, null };
    if (!requireArgs(env, info, &args, 6, "bufferSetCellFA(buf, x, y, char, fgF32, bgF32, attrs)")) return null;
    const buf_ptr = napi.unwrapPointer(env, args[0], OptimizedBuffer) orelse return null;
    const x = napi.getU32(env, args[1]) orelse return null;
    const y = napi.getU32(env, args[2]) orelse return null;
    const char = napi.getU32(env, args[3]) orelse return null;
    const fg = readColorArray(env, args[4]) orelse return null;
    const bg = readColorArray(env, args[5]) orelse return null;
    bufferSetCell(buf_ptr, x, y, char, &fg, &bg, 0);
    return napi.getUndefined(env);
}

pub fn jsBufferDrawCharFA(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [7]napi.napi_value = .{ null, null, null, null, null, null, null };
    if (!requireArgs(env, info, &args, 7, "bufferDrawCharFA(buf, char, x, y, fgF32, bgF32, attrs)")) return null;
    const buf_ptr = napi.unwrapPointer(env, args[0], OptimizedBuffer) orelse return null;
    const char = napi.getU32(env, args[1]) orelse return null;
    const x = napi.getU32(env, args[2]) orelse return null;
    const y = napi.getU32(env, args[3]) orelse return null;
    ...
    const bg = readColorArray(env, args[5]) orelse return null;
    const attrs = napi.getU32(env, args[6]) orelse return null;
    bufferDrawChar(buf_ptr, char, x, y, &fg, &bg, attrs);
    return napi.getUndefined(env);
}

pub fn jsBufferFillRectFA(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [6]napi.napi_value = .{ null, null, null, null, null, null };
    if (!requireArgs(env, info, &args, 6, "bufferFillRectFA(buf, x, y, w, h, bgF32)")) return null;
    const buf_ptr = napi.unwrapPointer(env, args[0], OptimizedBuffer) orelse return null;
    const x = napi.getU32(env, args[1]) orelse return null;
    const y = napi.getU32(env, args[2]) orelse return null;
    const w = napi.getU32(env, args[3]) orelse return null;
    const h = napi.getU32(env, args[4]) orelse return null;
    const bg = readColorArray(env, args[5]) orelse return null;
    bufferFillRect(buf_ptr, x, y, w, h, &bg);
    return napi.getUndefined(env);
}

// ── Performance: binary render path variants (avoid string copy) ──

pub fn jsBufferDrawPackedBufferBinary(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [6]napi.napi_value = .{ null, null, null, null, null, null };
    if (!requireArgs(env, info, &args, 6, "bufferDrawPackedBufferBinary(buf, data, posX, posY, termW, termH)")) return null;
    const buf_ptr = napi.unwrapPointer(env, args[0], OptimizedBuffer) orelse return null;
    const ta = napi.getTypedArrayInfo(env, args[1]) orelse {
        napi.throwError(env, "bufferDrawPackedBufferBinary: arg 1 must be Uint8Array");
        return null;
    };
    const posX = napi.getU32(env, args[2]) orelse return null;
    const posY = napi.getU32(env, args[3]) orelse return null;
    const termW = napi.getU32(env, args[4]) orelse return null;
    const termH = napi.getU32(env, args[5]) orelse return null;
    bufferDrawPackedBuffer(buf_ptr, ta.data, ta.len, posX, posY, termW, termH);
    return napi.getUndefined(env);
}

pub fn jsWriteOutBinary(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [2]napi.napi_value = .{ null, null };
    if (!requireArgs(env, info, &args, 2, "writeOutBinary(renderer, data)")) return null;
    const renderer = napi.unwrapPointer(env, args[0], CliRenderer) orelse return null;
    const ta = napi.getTypedArrayInfo(env, args[1]) orelse {
        napi.throwError(env, "writeOutBinary: arg 1 must be Uint8Array");
        return null;
    };
    writeOut(renderer, ta.data, ta.len);
    return napi.getUndefined(env);
}

// ── Performance: short string optimized edit operations ──

pub fn jsEditBufferInsertTextFast(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [2]napi.napi_value = .{ null, null };
    if (!requireArgs(env, info, &args, 2, "editBufferInsertTextFast(eb, text)")) return null;
    const eb = napi.unwrapPointer(env, args[0], EditBuffer) orelse return null;
    var small_buf: [256]u8 = undefined;
    var large_buf: [65536]u8 = undefined;
    const text = getStringSmall(env, args[1], &small_buf, &large_buf) orelse return null;
    editBufferInsertText(eb, text.ptr, text.len);
    return napi.getUndefined(env);
}

pub fn jsTextBufferAppendFast(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [2]napi.napi_value = .{ null, null };
    if (!requireArgs(env, info, &args, 2, "textBufferAppendFast(tb, data)")) return null;
    const tb = napi.unwrapPointer(env, args[0], UnifiedTextBuffer) orelse return null;
    var small_buf: [256]u8 = undefined;
    var large_buf: [65536]u8 = undefined;
    const data = getStringSmall(env, args[1], &small_buf, &large_buf) orelse return null;
    textBufferAppend(tb, data.ptr, data.len);
    return napi.getUndefined(env);
}

// ── Performance: right-sized output scratch buffers ──

pub fn jsTextBufferGetPlainTextSized(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [1]napi.napi_value = .{null};
    if (!requireArgs(env, info, &args, 1, "textBufferGetPlainTextSized(tb)")) return null;
    const ptr = napi.unwrapPointer(env, args[0], UnifiedTextBuffer) orelse return null;
    const byte_size = textBufferGetByteSize(ptr);
    if (byte_size <= 512) {
        var small_buf: [512]u8 = undefined;
        const len = textBufferGetPlainText(ptr, &small_buf, small_buf.len);
        return napi.createString(env, small_buf[0..len]);
    }
    var large_buf: [65536]u8 = undefined;
    const len = textBufferGetPlainText(ptr, &large_buf, large_buf.len);
    return napi.createString(env, large_buf[0..len]);
}

pub fn jsEditBufferGetTextSized(env: napi.napi_env, info: napi.napi_callback_info) callconv(.c) napi.napi_value {
    var args: [1]napi.napi_value = .{null};
    if (!requireArgs(env, info, &args, 1, "editBufferGetTextSized(eb)")) return null;
    const eb = napi.unwrapPointer(env, args[0], EditBuffer) orelse return null;
    const tb = editBufferGetTextBuffer(eb);
    const byte_size = textBufferGetByteSize(tb);
    if (byte_size <= 512) {
        var small_buf: [512]u8 = undefined;
        const len = editBufferGetText(eb, &small_buf, small_buf.len);
        return napi.createString(env, small_buf[0..len]);
    }
    var large_buf: [65536]u8 = undefined;
    const len = editBufferGetText(eb, &large_buf, large_buf.len);
    return napi.createString(env, large_buf[0..len]);
}
...
pub const properties = [_]napi.napi_property_descriptor{
    ...
    napi.method("bufferDrawTextFA", jsBufferDrawTextFA),
    napi.method("bufferSetCellFA", jsBufferSetCellFA),
    napi.method("bufferDrawCharFA", jsBufferDrawCharFA),
    napi.method("bufferFillRectFA", jsBufferFillRectFA),
    // Performance: binary render paths
    napi.method("bufferDrawPackedBufferBinary", jsBufferDrawPackedBufferBinary),
    napi.method("writeOutBinary", jsWriteOutBinary),
    // Performance: short-string optimized
    napi.method("editBufferInsertTextFast", jsEditBufferInsertTextFast),
    napi.method("textBufferAppendFast", jsTextBufferAppendFast),
    // Performance: right-sized output
    napi.method("textBufferGetPlainTextSized", jsTextBufferGetPlainTextSized),
    napi.method("editBufferGetTextSized", jsEditBufferGetTextSized),
    // Unicode
