...
Final binary: `build/<mode>/out/Final/node` (where `<mode>` is `dev` or `prod`).
...
Update checking is configured when building binaries using binject's `--update-config` flag. See the [binject documentation](../binject/README.md#embed-update-config) for configuration details.
...
2. **patch** - 13 Socket security patches applied
...
Socket applies **13 security and size-optimization patches** to Node.js v25.x:
