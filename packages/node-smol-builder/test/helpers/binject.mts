/**
 * @file Helper functions for running binject in tests.
...
const BINJECT_DIR = path.join(PACKAGE_ROOT, '..', 'binject')

// Re-export VFS resource names for convenience
export {
  RESOURCE_SMOL_VFS_BLOB as SMOL_VFS_BLOB,
  RESOURCE_SMOL_VFS_CONFIG as SMOL_VFS_CONFIG,
}
...
export async function runBinject(
  ...
  // Retry once on signal kill (exit code null) - LIEF can occasionally crash
  let result
  for (let attempt = 1; attempt <= 2; attempt++) {
    try {
      // eslint-disable-next-line no-await-in-loop
      result = await spawn(binjectPath, args, {
        cwd: testDir,
        env: spawnEnv,
      })
      break
    } catch (e) {
      if (e.code !== undefined || attempt === 2) {
        const details = [
          'binject failed:',
          `  command: ${binjectPath} ${args.join(' ')}`,
          `  cwd: ${testDir}`,
          e.stderr ? `  stderr: ${e.stderr}` : undefined,
          e.stdout ? `  stdout: ${e.stdout}` : undefined,
