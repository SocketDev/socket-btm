...
description: Regenerates all 16 Node.js patches from pristine upstream source. Triggers when patches fail to apply, after Node.js version update, or when restructuring patches. (project)
user-invocable: true
allowed-tools: Task
---
...
<task>
Your task is to spawn an autonomous agent that regenerates Socket Security patches from pristine upstream source (Node.js v25.5.0 or iocraft v0.9.4), ensuring each patch applies cleanly to unmodified upstream files with proper backups and validation.
</task>

<context>
**What is Patch Regeneration?**
Socket Security maintains patches for two upstream projects:
1. **Node.js** - 16 patches applied during node-smol-builder builds
...
- **Node.js patches**: `packages/node-smol-builder/patches/source-patched/*.patch`
  - Upstream: `packages/node-smol-builder/upstream/node` (v25.5.0)
  - Applies 16 patches to Node.js source files (common.gypi, node.gyp, src/*, lib/*)

- **iocraft patches**: `packages/iocraft-builder/patches/*.patch`
  - Upstream: `packages/iocraft-builder/upstream/iocraft` (v0.9.4)
  - Applies patches to Rust source files (src/*.rs)
...
1. **Feature Patches** - New TextDecoration variants, components
2. **Build Patches** - Cargo.toml, build.rs modifications
...
- Upstream submodule MUST be reset to pristine version before each patch:
  - Node.js: v25.5.0
  - iocraft: v0.9.4
...
- Use `git diff` or `git format-patch` (produces wrong format - must use `diff -u a/ b/`)
...
This skill spawns an autonomous agent to handle the complete patch regeneration workflow. First, determine which patches to regenerate (Node.js or iocraft), then process all patches sequentially.

### Phase 0: Determine Target

<action>
Ask user which patches to regenerate:
</action>

**Options:**
1. **node-smol** - Regenerate all Node.js patches (16 patches)
2. **iocraft** - Regenerate all iocraft patches
3. **both** - Regenerate both Node.js and iocraft patches

<validation>
Based on user selection, set:
- `TARGET=node-smol` or `TARGET=iocraft` or `TARGET=both`
- `UPSTREAM_PATH` accordingly
- `PATCHES_DIR` accordingly
- `UPSTREAM_VERSION` accordingly (v25.5.0 for node-smol, v0.9.4 for iocraft)
</validation>
...
<action>
Check that the upstream submodule(s) exist and are at the correct version:
</action>

**For Node.js (`TARGET=node-smol` or `TARGET=both`):**
```bash
ls -la packages/node-smol-builder/upstream/node
cd packages/node-smol-builder/upstream/node && git describe --tags
```

**For iocraft (`TARGET=iocraft` or `TARGET=both`):**
```bash
ls -la packages/iocraft-builder/upstream/iocraft
cd packages/iocraft-builder/upstream/iocraft && git describe --tags
```

<validation>
**Expected State (Node.js):**
- ✓ Submodule directory exists: `packages/node-smol-builder/upstream/node/`
- ✓ Tagged with v25.5.0

**Expected State (iocraft):**
- ✓ Submodule directory exists: `packages/iocraft-builder/upstream/iocraft/`
- ✓ Tagged with v0.9.4
