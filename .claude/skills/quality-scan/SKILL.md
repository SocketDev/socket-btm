---
name: quality-scan
description: Performs comprehensive code quality analysis using specialized agents. Triggers when user mentions "code quality", "find bugs", "pre-release check", or "scan codebase".
user-invocable: true
...
8. **patch-format** - Patch file format validation (unified diff format, no git format-patch)
...
<constraints>
**Requirements:**
- Read-only analysis (report findings, do not fix)
- Complete all enabled scans before reporting
- Prioritize findings by severity: Critical → High → Medium → Low
- Include file:line references and suggested fixes for all findings

**Behavior:**
- Warn about uncommitted changes but continue scanning
- Run scans in priority order: critical → logic → cache → workflow
