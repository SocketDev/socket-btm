...
import { logTransientErrorHelp } from '../../build-infra/lib/github-error-utils.mjs'
...
async function downloadStub(
  ...
  try {
    ...
    logger.group()
    logger.error(`${platformName} stub: ${err.message}`)

    // Log helpful messages if this is a transient GitHub/network error.
    await logTransientErrorHelp(err)
...
const stubConfigs = [
  { platform: 'darwin', arch: 'arm64', libc: undefined, name: 'darwin-arm64' },
  { platform: 'darwin', arch: 'x64', libc: undefined, name: 'darwin-x64' },
  { platform: 'linux', arch: 'arm64', libc: undefined, name: 'linux-arm64' },
  { platform: 'linux', arch: 'x64', libc: undefined, name: 'linux-x64' },
  { platform: 'linux', arch: 'arm64', libc: 'musl', name: 'linux-arm64-musl' },
  { platform: 'linux', arch: 'x64', libc: 'musl', name: 'linux-x64-musl' },
  { platform: 'win', arch: 'arm64', libc: undefined, name: 'win-arm64' },
  { platform: 'win', arch: 'x64', libc: undefined, name: 'win-x64' },
]
...
const stubFiles = stubResults.map((result, i) => {
  if (result.status === 'fulfilled') {
    return result.value
  }
  logger.warn(
    `Failed to download stub ${stubConfigs[i].name}: ${result.reason?.message || result.reason}`,
  )
  return undefined
})
const [
...
const cArrays = await Promise.all([
  stubDarwinArm64
