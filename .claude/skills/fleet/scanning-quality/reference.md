...
<pattern name="cpp_include_conventions">
**[SOCKET-BTM SPECIFIC] CRITICAL for Node.js additions C++ code:**
Incorrect include paths in socketsecurity/ subdirectories:
- Files in `additions/source-patched/src/socketsecurity/` MUST use full paths
- Relative includes break builds and require extra include path flags
- Example buggy pattern:
  ```cpp
  // BAD - relative include in socketsecurity/http/http_fast_response.cc
  #include "http_header_pool.h"  // Breaks without -I flag

  // GOOD - full path from socketsecurity/ root
  #include "socketsecurity/http/http_header_pool.h"
  ```
- **env.h vs env-inl.h pattern** (causes `-Werror,-Wundefined-inline` errors):
  - If `.cc` file uses `Environment*` methods, MUST include `env-inl.h` (provides inline definitions)
  - Headers that don't use `Environment*` should NOT include `env.h` (reduces dependencies)
  ```cpp
  // BAD - .cc using Environment* with only env.h
  #include "env.h"  // Missing inline definitions!

  // GOOD - include env-inl.h in implementation files
  #include "env-inl.h"  // Provides inline definitions
  ```
- Impact: macOS and Linux musl builds fail with undefined inline function errors
- Check all files in: `additions/source-patched/src/socketsecurity/**/*.{cc,h}`
</pattern>
...
- Check Windows Makefiles (Makefile.win) for:
...
- For Node.js built-in modules: **Cherry-pick fs, default import path/os/url/crypto**
  ...
  - Examples:
    ...
    - `import { spawn } from '@socketsecurity/lib-stable/spawn/spawn'` ✅ (preferred over node:child_process)
...
- Direct child_process usage (except in additions/):
  - `import { execSync } from 'node:child_process'` → use `import { spawn } from '@socketsecurity/lib-stable/spawn/spawn'`
...
- **Pattern**: Hardcoded patch count
- **Actual**: Patch count changes over time (currently 19)
- **Fix**: Avoid hardcoding patch counts — count dynamically from patches directory
...
### Patch Format Scan Agent

**Mission**: Validate patch files use correct unified diff format, not git format-patch output.

**Scan Targets**: All  files in `patches/` directories

**Prompt Template:**

````
Your task is to validate patch file formats across the codebase. Ensure patches use standard unified diff format with `--- a/` and `+++ b/` prefixes, not git format-patch output.

<context>
**Why Patch Format Matters:**
...
3. **Report findings with:**
   ...
   - Specific format violation
   - Fix: Convert to standard unified diff format
...
1. **Git format-patch output** (lines 1-5):
```diff
From 676c29ac Mon Sep 17 00:00:00 2001
From: name <email>
Date: Sun, 22 Mar 2026
Subject: [PATCH] description
```
**Fix**: Remove git headers, keep only patch content with metadata headers

2. **Missing a/ and b/ prefixes:**
```diff
--- src/file.cc
+++ src/file.cc
```
**Fix**: Change to `--- a/src/file.cc` and `+++ b/src/file.cc`
...
index ea08e2e..ba655a3
```
**Fix**: Remove `diff --git` and `index` lines

</pattern>

<output_format>

**If patches are correctly formatted:**
...
- **Fix**: Remove git headers (lines 1-14), use standard unified diff:
  ...
  +++ b/src/file.rs
  \`\`\`
- **Impact**: Patch will fail to apply during build, causing compilation failure

#### packages/node-smol-builder/patches/source-patched/017-fix.patch:12
- **Issue**: Missing a/ and b/ prefixes in diff headers
- **Pattern**: \`--- src/file.cc\` instead of \`--- a/src/file.cc\`
- **Fix**: Add \`a/\` and \`b/\` prefixes to --- and +++ lines
- **Impact**: Patch may fail to apply depending on patch tool version
```
...
**Scan Targets**: All `.patch` files in `patches/` directories
