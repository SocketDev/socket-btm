...
const UWEBSOCKETS_UPSTREAM_DIR = path.join(PACKAGE_ROOT, 'upstream', 'uWebSockets')
...
const EXTERNAL_SOURCES = [
  ...
  // lzfse: Apple compression library (upstream pinned in lief-builder/upstream/lzfse).
  // Used by compression_common.c on Linux/Windows (macOS uses native libcompression).
  {
    from: path.join(LIEF_BUILDER_DIR, 'upstream', 'lzfse', 'src'),
    ...
    ? [
        {
          ...
          to: path.join(ADDITIONS_SOURCE_PATCHED_DIR, 'deps', 'liburing', 'src'),
        },
      ]
    : []),
