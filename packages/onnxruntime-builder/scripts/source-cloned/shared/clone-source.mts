...
export async function cloneOnnxSource(options) {
  ...
  if (existsSync(sharedSourceDir)) {
    ...
    if (!allPatchesApplied) {
      ...
      return {
        ...
        smokeTest: async () => {
          if (
            !existsSync(path.join(sharedSourceDir, 'cmake', 'CMakeLists.txt'))
          ) {
            throw new Error(
              `Cloned ORT source missing CMakeLists.txt: ${sharedSourceDir}`,
            )
          }
        },
      }
  ...
  // Patch 1: Update Eigen SHA1 hash if upstream still references the
  // GitLab eigen source. Historically GitLab regenerated v3.4.0 archives
  // breaking SHA1 verification, so we kept the stable SHA1 in package.json
  // sources.eigen.sha1 and patched deps.txt to match. Upstream onnxruntime
  // has since switched to the github eigen-mirror archive (stable CDN), so
  // when the GitLab URL is no longer in deps.txt, no patching is needed.
  const depsPath = sharedCmakeDepsFile
  let depsContent = await fs.readFile(depsPath, 'utf8')
  ...
  if (usesGitLabEigen) {
    ...
    const verifyContent = await fs.readFile(depsPath, 'utf8')
    ...
    logger.success(
      'Eigen source is not GitLab — skipping SHA1 patch (upstream already uses a stable source)',
    )
  }
  ...
  await fs.writeFile(cmakePath, cmakeContent, 'utf8')
  ...
  if (existsSync(postBuildSourcePath)) {
    let postBuildContent = await fs.readFile(postBuildSourcePath, 'utf8')
    ...
    await fs.writeFile(postBuildSourcePath, postBuildContent, 'utf8')
    logger.success('wasm_post_build.js (source) patched')
  }

  // Upstream's wasm_post_build.js is CommonJS, but the nearest
  // enclosing package.json (our monorepo root) has "type": "module".
  // Node 20+ treats .js as ESM in that case and throws. Drop a scoped
  // package.json in the onnxruntime/wasm directory so Node scopes the
  // script as CommonJS at invocation time.
  const wasmScopePath = path.join(
    sharedSourceDir,
    'onnxruntime',
    'wasm',
    'package.json',
