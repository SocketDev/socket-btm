...
extern "C" int binject_inject_macho_lief(const char* executable,
  ...
  try {
    ...
    if (strcmp(section_name, "__NODE_SEA_BLOB") == 0) {
      ...
      if (!fuse_found) {
        fprintf(stderr, "Warning: NODE_SEA_FUSE not found (binary may not have SEA support)\n");
      }
    }
...
extern "C" int binject_verify_macho_lief(const char* executable,
  ...
  try {
    ...
    if (!has_content) {
      fprintf(stderr, "Warning: Section exists but has no content\n");
      return BINJECT_ERROR;
    }
...
extern "C" int binject_inject_macho_lief_batch(
  ...
  try {
    ...
    if (segment_exists && !overwrite) {
      fprintf(stderr, "Error: NODE_SEA segment already exists. Use --overwrite to replace.\n");
      return BINJECT_ERROR_SECTION_EXISTS;
    }

    // Remove existing segment if overwriting
    if (segment_exists && overwrite) {
      printf("Removing existing NODE_SEA segment...\n");
      // LIEF doesn't have remove_segment, so we'll just overwrite
    }

    // Flip NODE_SEA_FUSE BEFORE adding segment (if SEA data provided)
    if (sea_data && sea_size > 0) {
      ...
      if (!fuse_found) {
        printf("Warning: NODE_SEA_FUSE not found (may not be present in this binary)\n");
      }
    }
