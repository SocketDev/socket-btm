...
export async function buildStripped(config, buildOptions = {}) {
  ...
  if (IS_WINDOWS) {
    ...
    const llvmStripResult = await ensureToolInstalled('llvm-strip', {
      autoInstall: true,
      autoYes: true,
      toolOptions: {
        checkpointName: 'binary-stripped',
        packageRoot: PACKAGE_ROOT,
      },
    })

    if (llvmStripResult.available) {
      ...
      logger.skip('Skipping LLVM stripping - basic strip is sufficient')
    } else {
    ...
    // - Native addons (.node files) link dynamically to Node-API symbols (_napi_*)
    //   which must be preserved for addon loading to work
    //
    // Flag comparison:
    // - strip / --strip-all: Removes ALL symbols including symbol table (breaks pthread + napi)
    // - --strip-unneeded: Removes symbols not needed for relocations (still breaks pthread)
    // - --strip-debug: ONLY removes debug symbols, keeps symbol table (safe for pthread + napi)
    //
    // References:
    ...
    // Safe for Node-API: sstrip removes section headers only, not the
  ...
  const sizeMatch = sizeAfterStrip.match(/^(\d+(?:\.\d+)?)([KMG])/)
  if (sizeMatch) {
