...
// Monorepo root: packages/node-smol-builder/../../
 ...
 *
 * @param {string} phase - 'release' | 'stripped' | 'compressed' | 'final'
 * @param {string} platform - 'darwin' | 'linux' | 'linux-musl' | 'win32'
 * @param {string} arch - 'arm64' | 'x64'
...
export function getBuildSourcePaths(phase, platform, arch) {
  ...
  const phaseDeps = {
    ...
    compressed: ['release', 'stripped', 'compressed'],
    final: ['release', 'stripped', 'compressed', 'final'],
  }
...
export function getBuildPaths(mode, platform = process.platform) {
  ...
  const compressorName = decompressorName.replace('_decompress', '_compress')
