...
// Documentation: docs/additions/lib/internal/socketsecurity/vfs/sqlite_provider.js.md
...
class SmolSqliteProvider {
  ...
  renameSync(oldPath, newPath) {
    ...
    // POSIX EINVAL: rename('/foo', '/foo/bar') on a directory would
    // destroy both source and destination. Our getByPrefix(op) loop
    // below would match the just-renamed entry and orphan subtrees.
    // Matches `rename(2)` behavior: reject when newPath is equal to
    // or a subpath of oldPath.
    if (np === op || StringPrototypeStartsWith(np, op + '/')) {
      throw einval('rename', oldPath)
    }
