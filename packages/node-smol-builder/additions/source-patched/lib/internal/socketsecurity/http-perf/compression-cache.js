...
class CompressionCache {
  ...
  async precompress(key, data) {
    ...
    const brotli = await brotliCompress(dataBuffer, {
      params: {
    ...
    const gzip = await gzipCompress(dataBuffer, { level: 9 });
  ...
  getStats() {
    ...
    return {
      avg_brotli_ratio: total_brotli / total_original,
      avg_gzip_ratio: total_gzip / total_original,
      ...
      hit_rate: isNaN(hit_rate) ? 0 : hit_rate,
...
const globalCompressionCache = new CompressionCache();

module.exports = {
  CompressionCache,
  compressionCache: globalCompressionCache,
};
