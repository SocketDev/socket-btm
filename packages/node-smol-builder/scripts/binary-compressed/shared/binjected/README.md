...
All decompressors use a shared header (`dlx_cache_common.h`) that provides cross-platform caching logic, eliminating code duplication across macOS, Linux, and Windows implementations.

## Caching Strategy

The decompressors follow the exact caching strategy used by [socket-lib's dlxBinary](https://github.com/SocketDev/socket-lib/blob/v4.3.0/src/dlx-binary.ts#L300).
...
- External decompressor (~90 KB overhead)
