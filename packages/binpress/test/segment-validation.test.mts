...
import {
  execCommand,
  parseMachoSegments,
} from './helpers/segment-validation.mts'
...
describe.skipIf(!existsSync(BINPRESS))('segment and section validation', () => {
    ...
    it.skipIf(process.platform !== 'darwin')(
      ...
      async () => {
        ...
        if (segments === undefined) {
          throw new Error('expected Mach-O segments in compressed binary')
        }
    ...
    it.skipIf(process.platform !== 'darwin')(
      ...
      async () => {
        ...
        if (smolSegment === undefined) {
          throw new Error('expected SMOL segment in compressed binary')
        }
  ...
  describe('magic marker validation', () => {
    // Skip marker validation on Linux (ELF) because PT_NOTE segments don't allow
    // reliable marker scanning with Buffer.indexOf(). The marker is still embedded
    // in the PT_NOTE segment, but it's not directly scannable like regular sections.
    // Postject (Node.js SEA) uses the same PT_NOTE approach and doesn't scan for markers.
    it.skipIf(process.platform === 'linux')(
      'should embed magic marker in compressed binary',
      async () => {
    ...
    it.skipIf(process.platform === 'linux')(
      'should place magic marker at correct offset',
      async () => {
    ...
    it.skipIf(process.platform === 'linux')(
      'should have exactly 32-byte magic marker',
      async () => {
        ...
        expect(marker).toHaveLength(32)

        const markerIndex = data.indexOf(marker)
        const foundMarker = data.subarray(markerIndex, markerIndex + 32)

        expect(foundMarker.toString('utf8')).toBe(
