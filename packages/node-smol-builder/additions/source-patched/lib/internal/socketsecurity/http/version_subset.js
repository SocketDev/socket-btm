...
const semver = {
  ...
  satisfies(version, range) {
    ...
    // Handle >= range.
    if (StringPrototypeStartsWith(range, '>=')) {
      const base = semver.parse(StringPrototypeTrim(StringPrototypeSlice(range, 2)));
      if (!base) return false;
    ...
    // Handle > range.
    if (StringPrototypeStartsWith(range, '>')) {
      const base = semver.parse(StringPrototypeTrim(StringPrototypeSlice(range, 1)));
      if (!base) return false;
...
function subsetPackument(packument, versionRange) {
  ...
  for (const [version, data] of ObjectEntries(packument.versions)) {
