...
//
// WHAT THIS FILE DOES
// Implements the full repack workflow: codesign binaries (macOS), compress
// them using the built-in ZSTD compressor, build SMOL section metadata,
// and use LIEF to replace the compressed data segment inside the stub
// executable. Works across Mach-O, ELF, and PE formats.
//
// WHY IT EXISTS
// When a build injects new resources into an already-compressed binary, the
// old compressed payload is stale. This file handles the decompress-modify-
// recompress cycle, including platform-specific signing on macOS.
// ============================================================================
...
int binject_compress_binary(const char *input_path, const char *output_path, const char *quality) {
    ...
    /* Read input file. Use fstat for size to avoid extra fseek/ftell round-trips. */
    ...
    if (fstat(fileno(fp), &st) != 0) {
        ...
        fprintf(stderr, "Error: Cannot fstat file: %s (errno: %d - %s)\n",
    ...
    if (result != COMPRESS_OK) {
        fprintf(stderr, "Error: Compression failed (code: %d = %s)\n",
                result, compress_error_string(result));
        fprintf(stderr, "  Input size: %lld bytes\n", (long long)file_size);
        return -1;
    }
    ...
    /* Write compressed data to output file using cross-platform helper. */
    if (write_file_atomically(output_path, compressed_data, compressed_size, 0755) == -1) {
        free(compressed_data);
...
/**
 * Read platform metadata from original stub binary.
 * This ensures cross-platform builds preserve the target platform's metadata.
 */
static int read_stub_platform_metadata(const char *stub_path, uint8_t *platform, uint8_t *arch, uint8_t *libc) {
    int fd = open(stub_path, O_RDONLY);
    if (fd < 0) {
        return -1;
    }
    ...
    if (result == 0) {
        *platform = metadata.platform_metadata[0];
        *arch = metadata.platform_metadata[1];
        *libc = metadata.platform_metadata[2];
        return 0;
    }
    return -1;
}
 ...
 */
int binject_repack_stub(const char *stub_path, const char *compressed_data_path, const char *output_path, size_t uncompressed_size, const uint8_t *smol_config_binary, const char *extracted_path) {
    ...
    if (smol_config_binary) {
        printf("  Smol config: embedded (1192 bytes, SMFG v2)\n");
    }
    ...
    /* Read platform metadata from original stub to preserve target platform.
     * This is critical for cross-platform builds (e.g., building darwin-x64 on darwin-arm64).
     * Fall back to compile-time detection if stub doesn't have metadata yet. */
    uint8_t platform_byte;
    uint8_t arch_byte;
    uint8_t libc_byte;

    if (read_stub_platform_metadata(stub_path, &platform_byte, &arch_byte, &libc_byte) != 0) {
        /* Stub doesn't have SMOL metadata yet, use compile-time detection as fallback. */
        printf("  Platform metadata: detecting from build environment (stub has no SMOL section)\n");
        smol_detect_platform_metadata(&platform_byte, &arch_byte, &libc_byte);
    } else {
        printf("  Platform metadata: preserved from original stub\n");
    }
    ...
    if (smol_build_section_data(compressed_data, compressed_size, uncompressed_size,
                                 platform_byte, arch_byte, libc_byte, smol_config_binary, &section) != 0) {
        free(compressed_data);
    ...
    switch (format) {
        case BINARY_FORMAT_MACHO:
            result = binject_macho_repack_smol(stub_path, section.data, section.size, output_path, extracted_path);
            break;
 ...
 */
int binject_repack_workflow(const char *stub_path, const char *extracted_path, const char *output_path, const uint8_t *smol_config_binary) {
    ...
    if (binject_repack_stub(stub_path, temp_compressed, output_path, uncompressed_size, smol_config_binary, extracted_path) != 0) {
