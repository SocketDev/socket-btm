 ...
 * - Linux: liblzma-dev (for LZMA compression)
 * - Linux musl: musl-tools, gcc-aarch64-linux-gnu (for cross-compilation)
...
async function setup() {
  ...
  // Linux: Install liblzma-dev
  if (platform === 'linux') {
    logger.log('Installing Linux build dependencies...')
    updatePackageCache()

    const tools = ['liblzma-dev']
