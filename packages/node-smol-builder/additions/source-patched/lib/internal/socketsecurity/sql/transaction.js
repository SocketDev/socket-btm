...
class Transaction {
  ...
  async savepoint(nameOrFn, fn) {
    ...
    try {
      ...
      // Only rollback if the savepoint was actually created. A failed
      // create() hasn't established anything to roll back to; issuing
      // ROLLBACK TO SAVEPOINT on a nonexistent name produces a second
      // error and leaves the outer transaction in aborted state.
      if (created) {
        try {
          await sp.rollback()
        } catch {
          // Swallow rollback errors so the original create/callback
...
class Savepoint {
  ...
  [kConnection]
  #name
  // `#released` means the savepoint has been explicitly released
  // (RELEASE SAVEPOINT). `#active` means tagged-template queries are
  // still valid against it. After a rollback, PG leaves the savepoint
  // name allocated and it can be released or rolled back to again —
  // conflating both states into a single `#released` flag broke that
  // pattern. See R35 scan.
  #released = false
  #active = true
  ...
  async release() {
    if (!this.#released) {
      ...
      // Set flags AFTER query succeeds to preserve state on error.
      this.#released = true
      this.#active = false
    }
  }
  ...
  /**
   * Rollback to the savepoint. Leaves the savepoint live per
   * PostgreSQL manual §13.3.1: "After ROLLBACK TO, the savepoint is
   * still active and can be released or rolled back to again."
   *
   * @returns {Promise<void>}
   */
  async rollback() {
    ...
    // After rollback the savepoint name is still live; only a subsequent
    // release() invalidates it. Don't set #released here.
  }
}
