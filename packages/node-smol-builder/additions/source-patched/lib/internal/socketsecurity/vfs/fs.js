...
const { StreamReadable: Readable } = require('internal/socketsecurity/safe-references')
...
function readFileSync(filepath, options) {
  ...
  const result = readFileFromVFS(normalized, typeof options === 'string' ? { encoding: options } : options);
...
function openSync(filepath, flags = 'r', mode) {
  ...
  if (flagStr !== 'r' && flagStr !== 'r+' && flagStr !== 'rs' && flagStr !== 'rs+') {
    const err = new VFSError(
...
function readMultiple(filepaths, options) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    try {
      ...
      results[i] = { __proto__: null, path: filepath, content: undefined, error: err };
    }
  }
  return results;
}
    ...
    stream = new ReadableClass({
      ...
      read(size) {
        ...
        try {
          ...
          position += bytesRead;
          this.push(bytesRead < toRead ? BufferPrototypeSlice(buffer, 0, bytesRead) : buffer);
        } catch (err) {
          this.destroy(err);
        }
      },
