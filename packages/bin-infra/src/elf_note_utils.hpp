...
inline int smol_reuse_multi_ptnote(
    ...
    // Validate endianness - we only support little-endian (x86-64, ARM64)
    // Big-endian (PowerPC, s390x) would require byte-swapping all header fields
    // (phoff, phentsize, phnum, PT_NOTE offsets/sizes) which is not implemented.
    // This is acceptable as our target platforms are all little-endian.
