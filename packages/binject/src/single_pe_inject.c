...
static int read_file(const char *path, uint8_t **data, size_t *size) {
    ...
    /* Validate file size fits in size_t and is within bounds */
    if (file_size > (long)MAX_PE_SIZE) {
        fprintf(stderr, "Error: PE file too large (%ld bytes, max %d)\n", file_size, MAX_PE_SIZE);
        fclose(fp);
        return BINJECT_ERROR;
    }

    /* Safe to cast since we checked file_size is positive and within MAX_PE_SIZE */
...
static int parse_pe_headers(uint8_t *pe_data, size_t pe_size, pe_headers_t *headers) {
    ...
    if (headers->dos->e_lfanew > pe_size - 4 - sizeof(IMAGE_FILE_HEADER) - sizeof(IMAGE_OPTIONAL_HEADER64)) {
    ...
    // Parse file header
    headers->file_hdr = (IMAGE_FILE_HEADER *)(pe_data + headers->dos->e_lfanew + 4);
    ...
    size_t opt_hdr_offset = headers->dos->e_lfanew + 4 + sizeof(IMAGE_FILE_HEADER);
...
int binject_single_pe(const char *executable, const char *output, const char *section_name,
    ...
    result = write_file(tmpfile, new_pe, new_pe_size);
