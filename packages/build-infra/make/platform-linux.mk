...
ifeq ($(BUILD_MODE),prod)
    ...
    # Note: LTO (-flto) could be enabled here, but tests currently run on the GitHub Actions host
    # while builds happen in Docker. If test binaries need to link against build artifacts,
    # GCC versions must match. Consider running tests in Docker to enable LTO.
