// ============================================================================
// pe_compress_lief.cpp — PE (Windows) compression via LIEF (cross-platform)
// ============================================================================
//
// WHAT THIS FILE DOES
// Compresses a binary and embeds it as a SMOL section inside a Windows PE
// stub executable using the LIEF C++ library. This enables building Windows
// self-extracting binaries from macOS or Linux build machines.
//
// WHY IT EXISTS
// You cannot easily manipulate PE executables on non-Windows systems with
// native APIs. LIEF provides cross-platform PE read/write, so this C++
// file uses LIEF to parse a PE stub, add the compressed data section, and
// write the final .exe — all without needing Windows.
// ============================================================================
...
int pe_compress_lief(const char* input_path,
  ...
  if (!input_path || !output_path) {
    ...
    // Sign PE binary with Authenticode (Windows only, no-op on other platforms).
    // Non-fatal: signing requires a valid certificate.
    pe_codesign(final_output_path);
