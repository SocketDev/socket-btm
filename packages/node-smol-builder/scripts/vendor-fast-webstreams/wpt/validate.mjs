 ...
 */
function fetchWPTStreams(url, ref, force = false) {
  ...
  try {
    ...
    execSync(`git remote add origin ${url}`, { cwd: tmpDir, stdio: 'pipe' })
    ...
    execSync('echo "streams" >> .git/info/sparse-checkout', {
      cwd: tmpDir,
      stdio: 'pipe',
    })
    ...
    execSync('git checkout FETCH_HEAD', { cwd: tmpDir, stdio: 'pipe' })
    ...
    execSync(`cp -r "${srcStreams}" "${WPT_DIR}"`, { stdio: 'pipe' })
...
async function main() {
  ...
  fetchWPTStreams(url, ref, opts.fetch)
