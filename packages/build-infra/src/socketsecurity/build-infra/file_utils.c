 ...
 *
 * Optimized to attempt mkdir first and only stat() on EEXIST,
 * reducing syscalls for the common case where directory doesn't exist.
 *
...
int mkdir_recursive(const char *dirpath) {
    ...
    // Try mkdir first - avoids stat() syscall in common case.
#ifdef _WIN32
    if (_mkdir(dirpath) == 0) {
#else
    ...
    if (errno == EEXIST) {
        // Path exists - verify it's a directory (single stat, not two).
        struct stat st;
    ...
    if (errno == ENOENT) {
        ...
        // Retry mkdir after creating parents.
        DEBUG_LOG("Retrying mkdir after creating parents: %s\n", dirpath);
#ifdef _WIN32
        if (_mkdir(dirpath) == 0) {
#else
        if (mkdir(dirpath, 0755) == 0) {
        ...
        // Still failed - check if it was created by another process.
        if (errno == EEXIST) {
            struct stat st;
            if (stat(dirpath, &st) == 0 && S_ISDIR(st.st_mode)) {
                return 0;
