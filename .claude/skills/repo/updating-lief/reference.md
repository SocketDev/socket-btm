...
- `binject`: vN → v(N+1)
- `binpress`: vN → v(N+1)
- `node-smol`: vN → v(N+1) (node-smol embeds binpress/binject which link LIEF)

\`\`\`bash
echo "Bumping cache versions..."

# Read current cache versions and bump: lief, binject, binpress, node-smol
...
- ✅ Cache versions bumped: lief, binject, binpress, node-smol
