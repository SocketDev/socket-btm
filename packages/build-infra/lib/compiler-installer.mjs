 ...
 * @returns {Promise<{installed: boolean, version: string|null, meetsRequirements: boolean}>}
 */
...
async function installGccApt(version) {
  ...
  try {
    ...
    printSubstep(`Setting GCC ${version} as default...`)
    ...
    if (gccAltResult.code !== 0 || gxxAltResult.code !== 0) {
      printWarning('Failed to set GCC as default, but installation succeeded')
    }
...
export async function ensureGccVersion({
  ...
  if (!quiet) {
    printStep('Checking GCC version...')
  }
  ...
  if (currentCheck.installed && !quiet) {
    ...
    printSubstep(requirement.reason)
  }
  ...
  if (platform !== 'linux') {
    if (!quiet) {
      printWarning('Automatic GCC installation is only supported on Linux')
    }
    return {
      available: false,
      version: currentCheck.version,
  ...
  if (!managers.includes('apt')) {
    if (!quiet) {
      printWarning('Automatic GCC installation requires apt package manager')
    }
    return {
      available: false,
      version: currentCheck.version,
  ...
  if (verifyCheck.meetsRequirements) {
    if (!quiet) {
      printSuccess(`GCC ${verifyCheck.version} installed successfully`)
    }
    return {
      available: true,
