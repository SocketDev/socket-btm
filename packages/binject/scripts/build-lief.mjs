...
const liefVendorDir = path.join(packageRoot, 'vendor', 'lief')
const liefSubmodule = path.join(liefVendorDir, 'submodule')
const liefBuildDir = path.join(liefVendorDir, 'build')

const WIN32 = process.platform === 'win32'
const DARWIN = process.platform === 'darwin'

// LIEF version (tracked in submodule).
...
async function main() {
  try {
    ...
    // Check if LIEF submodule exists.
    if (!existsSync(liefSubmodule)) {
      throw new Error(
        `LIEF submodule not found at ${liefSubmodule}. Run 'git submodule update --init --recursive' first.`,
      )
    }
    logger.info('LIEF submodule found')
