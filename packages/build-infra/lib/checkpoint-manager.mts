  ...
  if (data.artifactPath) {
    ...
    try {
      ...
      if (isDir) {
        try {
          ...
          if (contents.length === 0) {
            throw new Error(
              `Cannot archive empty directory: ${artifactPath}. ` +
          ...
          if (e.code === 'EACCES' || e.code === 'EPERM') {
            throw new Error(
              `Cannot read directory: ${artifactPath} (${e.code}). ` +
      ...
      try {
        ...
        const isFileChangedWarning =
          tarError.code === 1 && stderr.includes('file changed as we read it')
    ...
    if (!checkpointAlreadyExists) {
      ...
      try {
        ...
        // absolute prefix (the final artifactPath), so the absolute path
        ...
        artifactHash = await computeSourceHash(
