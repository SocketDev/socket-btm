// ============================================================================
// smol_config.h — SMFG binary config format for self-update settings
// ============================================================================
//
// WHAT THIS FILE DOES
// Defines the struct and serialization API for the SMFG ("SMol conFiG")
// binary format — a fixed-size 1192-byte blob that stores self-update
// configuration (update URL, check interval, prompt settings, etc.).
//
// WHY IT EXISTS
// The compressed binary needs to know how to self-update (where to check,
// how often, what to show the user). This config is serialized into a
// compact binary format and embedded directly in the executable so it
// can be read without parsing JSON at runtime.
// ============================================================================
 ...
 * SMFG format: 1192-byte config data with magic "SMFG" (0x534D4647).
 */
...
#define MAX_NODE_VERSION_LEN 15
...
typedef struct {
    ...
    // Node.js version (for SEA blob cross-compilation compatibility)
    const char *node_version;      // Node.js version (e.g., "25.5.0", max 15 chars)
...
/**
 * Free smol_update_config_t string fields.
 *
 * @param config - Config struct to free (must not be NULL).
 */
 ...
 *
 * Converts smol_update_config_t struct to 1192-byte config data.
 * SMFG format:
 *   Header (8 bytes): magic, version, flags
 *   Numeric (16 bytes): interval, notifyInterval
 *   Strings (1168 bytes): binname, command, url, tag, skipEnv, fakeArgvEnv, nodeVersion
 *
 * @param config - Smol update config struct (must not be NULL).
 * @return 1192-byte config data, or NULL on error. Caller must free().
 */
