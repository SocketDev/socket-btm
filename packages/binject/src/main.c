...
/**
 * Generate update config binary from JSON file using validation module.
 * Returns malloc'd buffer (1112 bytes) that caller must free, or NULL on error.
 */
static uint8_t* generate_update_config_binary(const char *config_path) {
    // Validate config_path
    if (!config_path || strlen(config_path) == 0) {
        fprintf(stderr, "Error: Update config path is empty\n");
        return NULL;
    }

    // Check for path traversal attempts
    if (strstr(config_path, "..") != NULL) {
        fprintf(stderr, "Error: Path traversal detected in update config path\n");
        return NULL;
    }
    ...
    if (stat(config_path, &st) != 0) {
        fprintf(stderr, "Error: Update config file not found: %s\n", config_path);
        return NULL;
    }

    if (!S_ISREG(st.st_mode)) {
        fprintf(stderr, "Error: Update config path is not a regular file: %s\n", config_path);
        return NULL;
    }

    printf("Validating update config: %s\n", config_path);

    // Build command to run Node.js validator
    // Use process.stdout.write() to output binary data
    char command[8192];
    snprintf(command, sizeof(command),
        "node --no-warnings --input-type=module -e "
        "\"import {parseConfigFile} from './scripts/update-config-binary.mjs';"
        "const buf = parseConfigFile('%s');"
        "process.stdout.write(buf);\"",
        config_path);

    // Execute command and capture binary output
    ...
    if (!buffer) {
        pclose(fp);
        fprintf(stderr, "Error: Failed to allocate memory for update config\n");
        return NULL;
    }
    ...
    if (bytes_read != 1112) {
        fprintf(stderr, "Error: Invalid update config binary size (expected 1112, got %zu)\n", bytes_read);
        free(buffer);
        return NULL;
    }

    // Verify magic number (0x55504446 = "UPDF")
    uint32_t magic = buffer[0] | (buffer[1] << 8) | (buffer[2] << 16) | (buffer[3] << 24);
    if (magic != 0x55504446) {
        fprintf(stderr, "Error: Invalid update config magic number: 0x%08x\n", magic);
        free(buffer);
        return NULL;
    }

    printf("✓ Update config validated and serialized (%zu bytes)\n", bytes_read);
    return buffer;
}
...
static void print_usage(const char *program) {
    ...
    printf("Usage:\n");
    printf("  %s inject -e <executable> -o <output> [--sea <path>] [--vfs <path>|--vfs-on-disk <path>|--vfs-in-memory <path>|--vfs-compat] [--update-config <path>] [--skip-repack]\n", program);
    ...
    printf("  --update-config <path>        Embed update config from JSON file (for compressed node-smol binaries)\n");
    printf("                                Example: --update-config update-config.json\n");
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(command, "inject") == 0) {
        ...
        // Generate update config binary if path provided
        uint8_t *update_config_binary = NULL;
        ...
        int result = binject_batch(executable, output, sea_resource, vfs_resource, vfs_in_memory, skip_repack, update_config_binary);
