...
Update bin-stub-builder by first updating curl (dependency), then rebuilding stubs.

- **Package**: `packages/bin-stub-builder/`
...
3. **Rebuild stubs** (skip in CI): `pnpm run clean && pnpm run build` in `packages/bin-stub-builder`
4. **Test** (skip in CI): `pnpm test` in `packages/bin-stub-builder`
5. **Bump caches**: Increment `stubs`, `binpress`, `node-smol` in `.github/cache-versions.json`. Always bump if curl was updated.
6. **Commit** (if changes exist): Stage `packages/bin-stub-builder` and cache-versions.json
