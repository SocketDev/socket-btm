 ...
 */
static int extract_and_execute(int self_fd, const char *exe_path, char *envp[]) {
    int exit_code = 1;
    ...
    snprintf(output_path, sizeof(output_path), "%s/.socket/_dlx/%s/node-smol-%s-%s",
             getenv("HOME") ? getenv("HOME") : "/tmp",
             cache_key,
             "linux", // platform
             "x64"); // TODO: detect architecture
...
int main(int argc, char *argv[], char *envp[]) {
    ...
    // Extract and execute
    exit_code = extract_and_execute(self_fd, exe_path, envp);
