...
export async function generateWasmSyncWrapper(options) {
  ...
  // Two-pass AST transformation:
  // Pass 1: Replace import.meta.url with __filename
  // Pass 2: Handle everything else including the now-replaced __filename patterns

  // === PASS 1: Replace import.meta.url ===
  let ast = parse(mjsContent, {
    plugins: [],
    sourceType: 'module',
  })
  let s = new MagicString(mjsContent)
    ...
    MetaProperty(path) {
      if (
        ...
        if (
          ...
          // import.meta.url -> replace with pathToFileURL(__filename).href
          // .href converts the URL object to a string, matching import.meta.url behavior
          s.overwrite(
            path.parent.start,
            path.parent.end,
  ...
  traverse(ast, {
    ...
    ExportDefaultDeclaration(path) {
      if (path.parent.type === 'Program') {
        ...
        if (path.node.declaration?.type === 'FunctionDeclaration') {
          ...
          safeRemove(path.node.start, path.node.declaration.start)
        } else {
          // For other default exports (expressions, etc), remove entirely
          safeRemove(path.node.start, path.node.end)
        }
    ...
    FunctionDeclaration(p) {
      if (p.node.id?.name === 'findWasmBinary') {
        safeRemove(p.node.start, p.node.end)
        return
      }
      ...
      p.traverse({
        ...
        Identifier(idPath) {
          ...
          if (idPath.node.name === 'wasmBinary') {
            hasLoadingMechanism = true
          }
        },
      })
      ...
      if (p.node.async) {
        const asyncPos = mjsContent.indexOf('async', p.node.start)
        if (asyncPos >= 0 && asyncPos < p.node.body.start) {
    ...
    FunctionExpression(path) {
      if (path.node.async) {
        const pos = mjsContent.indexOf('async', path.node.start)
        if (pos >= 0 && pos < path.node.start + 20) {
    ...
    AwaitExpression(path) {
      const pos = mjsContent.indexOf('await', path.node.start)
      if (pos >= 0 && pos < path.node.start + 10) {
        safeRemove(pos, pos + 6)
      }
    },
    ...
    OptionalCallExpression(path) {
      if (
        ...
        path.node.callee?.name === 'readyPromiseResolve' &&
        ...
        safeOverwrite(path.node.start, path.node.end, `(moduleRtn = ${arg})`)
        return
      }
    },
    CallExpression(path) {
      ...
      if (
        ...
        safeRemove(path.node.callee.property.start - 1, path.node.end)
        return
      }
      ...
      if (path.node.callee?.name === 'findWasmBinary') {
        safeOverwrite(path.node.start, path.node.end, "''")
        return
      }
      ...
      if (
        path.node.callee?.type === 'MemberExpression' &&
        path.node.callee?.property?.name === 'fileURLToPath' &&
        path.node.callee?.object?.type === 'CallExpression' &&
        path.node.callee?.object?.callee?.name === 'require' &&
        (path.node.callee?.object?.arguments?.[0]?.value === 'url' ||
          path.node.callee?.object?.arguments?.[0]?.value === 'node:url')
      ) {
        const arg = path.node.arguments?.[0]
        // Check if the argument is __filename or import.meta.url
        const isFilename =
        ...
        if (isFilename || isImportMetaUrl) {
          ...
          safeOverwrite(path.node.start, path.node.end, '__filename')
        } else if (isNewURLWithFilename || isNewURLWithImportMeta) {
          // Replace require("url").fileURLToPath(new URL("./", __filename or import.meta.url)) with __dirname
          safeOverwrite(path.node.start, path.node.end, '__dirname')
        }
        return
      }
      ...
      if (path.node.callee?.name === 'fileURLToPath') {
        const arg = path.node.arguments?.[0]
        const isFilename =
          arg?.type === 'Identifier' && arg.name === '__filename'
        // Check for new URL("./", __filename) or new URL("./", import.meta.url) pattern
        ...
        if (isFilename) {
          ...
          // Replace fileURLToPath(new URL("./", __filename or import.meta.url)) with __dirname
          safeOverwrite(path.node.start, path.node.end, '__dirname')
        }
        return
      }
      ...
      if (
        ...
        if (
          path.node.arguments?.[0]?.value === 'module' &&
          path.parent.type === 'VariableDeclarator' &&
          path.parent.id?.type === 'ObjectPattern'
        ) {
          // This is the createRequire import - mark it for removal
          // Find the entire variable declaration
          let varDecl = path
          while (varDecl && varDecl.node.type !== 'VariableDeclaration') {
            varDecl = varDecl.parentPath
          }
          if (varDecl && varDecl.parent.type === 'Program') {
            topLevelStatementsToRemove.push(varDecl.node)
          }
        } else {
          // Convert import("module") to require("module")
          safeOverwrite(path.node.callee.start, path.node.callee.end, 'require')
        }
        return
      }
      ...
      if (
        ...
        if (
          ...
          path.parent.type === 'MemberExpression' &&
          path.parent.property?.name === 'fileURLToPath'
        ) {
    ...
    VariableDeclaration(path) {
      ...
      if (
        ...
        path.node.declarations[0]?.id?.type === 'ObjectPattern'
      ) {
        const properties = path.node.declarations[0].id.properties
        ...
        if (hasCreateRequire) {
          ...
          // Note: import() is parsed as CallExpression with callee.type === 'Import'
          if (
    ...
    VariableDeclarator(path) {
      const isRequireDeclarator = path.node.id?.name === 'require'
      const parentIsVarDecl = path.parent.type === 'VariableDeclaration'

      if (isRequireDeclarator && parentIsVarDecl) {
        // Mark this declarator for special handling
        requireDeclaratorsToRemove.push({
          path,
          varDecl: path.parentPath,
        })
      }

      // Simplify ENVIRONMENT_IS_NODE check - we're Node.js only (.cjs)
      // Pattern: var ENVIRONMENT_IS_NODE = globalThis.process?.versions?.node && globalThis.process?.type != 'renderer';
      if (
        path.node.id?.name === 'ENVIRONMENT_IS_NODE' &&
        path.node.init?.type === 'LogicalExpression'
      ) {
        // Replace the entire initialization with: true
        safeOverwrite(path.node.init.start, path.node.init.end, 'true')
      }
      ...
      if (
        ...
        path.node.init.callee?.name === 'Promise' &&
        ...
        const arrowFn = path.node.init.arguments[0]
        if (arrowFn.params?.length === 2) {
          // Replace with null - we'll assign the actual module later when we see fa(D)
          safeOverwrite(path.node.init.start, path.node.init.end, 'null')
        }
      }
    },
    ...
    AssignmentExpression(path) {
      ...
      if (
        ...
        path.node.left?.name === 'moduleRtn' &&
        path.node.right?.type === 'Identifier' &&
        path.node.right?.name === 'ia' &&
        path.parent?.type === 'ExpressionStatement'
      ) {
        // Remove the entire expression statement
        safeRemove(path.parent.start, path.parent.end + 1)
      }
    },
    ...
    ReturnStatement(path) {
      if (
        ...
        path.node.argument?.name === 'moduleRtn'
      ) {
        ...
        // Walk up to find the function declaration/expression
        let funcPath = path
        while (
          funcPath &&
          funcPath.node.type !== 'FunctionDeclaration' &&
          funcPath.node.type !== 'FunctionExpression'
        ) {
          funcPath = funcPath.parentPath
        }

        // Check if the function is named 'Module' (Yoga pattern)
        if (funcPath?.node.id?.name === 'Module') {
          moduleVar = 'Module'
        }
      ...
      if (
        ...
        let funcPath = path
        while (funcPath && funcPath.node.type !== 'FunctionDeclaration') {
          funcPath = funcPath.parentPath
        }
  ...
  for (const { path, varDecl } of requireDeclaratorsToRemove) {
    const declarators = varDecl.node.declarations
    const index = declarators.indexOf(path.node)

    if (declarators.length === 1) {
      // Only one declarator - remove entire statement
      safeRemove(varDecl.node.start, varDecl.node.end)
    } else if (index === 0) {
      ...
      const textBetween = s.original.slice(path.node.end, nextDeclarator.start)
      const commaIndex = textBetween.indexOf(',')
      if (commaIndex >= 0) {
        safeRemove(path.node.start, path.node.end + commaIndex + 1)
      } else {
        safeRemove(path.node.start, nextDeclarator.start)
      }
    } else {
      // Not first declarator - remove from previous comma to this declarator
      const prevDeclarator = declarators[index - 1]
      const textBetween = s.original.slice(prevDeclarator.end, path.node.start)
      const commaIndex = textBetween.lastIndexOf(',')
      if (commaIndex >= 0) {
        safeRemove(prevDeclarator.end + commaIndex, path.node.end)
      } else {
        safeRemove(prevDeclarator.end, path.node.end)
      }
    }
  }
    ...
    traverse(ast3, {
      FunctionDeclaration(p) {
        ...
        if (
          ...
          // Extract param names
          const paramNames = p.node.params.map(param =>
          ...
          // Strategy 1: Use Babel to traverse the function body and find WebAssembly.instantiate calls
          ...
          p.traverse({
            CallExpression(callPath) {
              ...
              if (
                ...
                const importsArg = callPath.node.arguments[1]
          ...
          if (Object.keys(paramCounts).length > 0) {
            importsParam = Object.entries(paramCounts).sort(
              ([, a], [, b]) => b - a,
            )[0][0]
          }
          ...
          if (!foundInstantiate) {
            const bodyStatements = p.node.body.body
...
// Users must await this: const yoga = await require('./yoga-sync.js');
