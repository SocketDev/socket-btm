...
/**
 * Parse an asset platform-arch string (e.g., `linux-x64-musl`) back into the
 * Node.js-canonical `{platform, arch, libc}` tuple that the checkpoint layer
 * expects. Inverse of `getAssetPlatformArch`.
 *
 * Used by build orchestrators (e.g. build-pipeline.mts) to thread explicit
 ...
 */
export function parsePlatformArch(platformArch: string): {
  arch: string
  libc: string | undefined
  platform: string
} {
  ...
  if (
    parts.length < 2 ||
    parts.length > 3 ||
    assetPlatform === undefined ||
    assetArch === undefined
  ) {
    throw new Error(
      `Invalid platform-arch '${platformArch}': expected '<platform>-<arch>' or '<platform>-<arch>-<libc>'`,
    )
  }
  const platform = ASSET_PLATFORM_TO_NODE[assetPlatform]
  if (!platform) {
    throw new Error(
      `Invalid platform-arch '${platformArch}': unknown platform '${assetPlatform}' ` +
        `(expected one of: ${Object.keys(ASSET_PLATFORM_TO_NODE).join(', ')})`,
    )
  }
  const arch = ASSET_ARCH_TO_NODE[assetArch]
  if (!arch) {
    throw new Error(
      `Invalid platform-arch '${platformArch}': unknown arch '${assetArch}' ` +
        `(expected one of: ${Object.keys(ASSET_ARCH_TO_NODE).join(', ')})`,
    )
  }
  if (libc && libc !== 'musl' && libc !== 'glibc') {
    throw new Error(
      `Invalid platform-arch '${platformArch}': unknown libc '${libc}' ` +
        `(expected 'musl' or 'glibc')`,
    )
  }
  if (libc && platform !== 'linux') {
    throw new Error(
      `Invalid platform-arch '${platformArch}': libc suffix only valid on Linux`,
    )
  }
  return { arch, libc, platform }
}
 ...
 */
export async function tarSupportsNoAbsoluteNames(): Promise<boolean> {
  try {
 ...
 */
export async function tarSupportsOverwrite(): Promise<boolean> {
