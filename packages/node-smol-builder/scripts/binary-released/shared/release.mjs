...
import { getBuildPaths, getDefaultPlatformArch } from '../../paths.mjs'
...
async function generateVersion() {
  ...
  const { stdout } = await spawn('git', ['rev-parse', '--short=7', 'HEAD'])
...
const PLATFORMS = [
  ...
  { arch: 'x64', ext: 'tar.gz', platform: 'darwin' },
  { arch: 'x64', ext: 'tar.gz', libc: 'glibc', platform: 'linux' },
  { arch: 'arm64', ext: 'tar.gz', libc: 'glibc', platform: 'linux' },
  { arch: 'x64', ext: 'tar.gz', libc: 'musl', platform: 'linux' },
  { arch: 'arm64', ext: 'tar.gz', libc: 'musl', platform: 'linux' },
  { arch: 'x64', ext: 'zip', platform: 'win32' },
  { arch: 'arm64', ext: 'zip', platform: 'win32' },
]
...
async function findBinary(platform, arch, libc) {
  ...
  const { outputFinalNodeDir } = getBuildPaths(BUILD_MODE, platform, platformArch)
  const finalBinary = path.join(outputFinalNodeDir, binaryName)
  if (
  ...
  const archivePlatform = getArchivePlatform(platform, arch, libc)
...
async function createReleaseArchive(
  ...
  if (!config) {
    ...
    throw new Error(`Unknown platform: ${platform}-${arch}${libcLabel}`)
  }
  ...
  if (!binaryPath) {
    logger.warn(`  Binary not found, skipping ${platform}-${arch}${libcLabel}`)
    const libcFlag = libc ? ` LIBC=${libc}` : ''
    logger.warn(
      `  Build with: PLATFORM=${platform} ARCH=${arch}${libcFlag} pnpm build`,
    )
    return undefined
  }
