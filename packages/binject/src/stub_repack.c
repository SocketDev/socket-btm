...
#include "stub_smol_repack_lief.h"
...
int binject_repack_stub(const char *stub_path, const char *compressed_data_path, const char *output_path, size_t uncompressed_size, const uint8_t *update_config_binary) {
    ...
    if (update_config_binary) {
        printf("  Update config: embedded (1112 bytes)\n");
    }
    ...
    if (smol_build_section_data(compressed_data, compressed_size, uncompressed_size,
                                 platform_byte, arch_byte, libc_byte, update_config_binary, &section) != 0) {
        free(compressed_data);
...
int binject_repack_workflow(const char *stub_path, const char *extracted_path, const char *output_path, const uint8_t *update_config_binary) {
    ...
    if (binject_repack_stub(stub_path, temp_compressed, output_path, uncompressed_size, update_config_binary) != 0) {
