...
//
//   std::unordered_map<uint32_t, std::shared_ptr<PostgresPool>> pools_
//     -- A hash map from pool ID to pool object.  JS passes numeric IDs
//        (not raw pointers) for safety. `shared_ptr` so async workers
//        (libuv thread pool) can hold a live reference for the duration
//        of an in-flight query, even if JS calls destroyPool(id)
//        concurrently. The pool is destroyed only when the last holder
//        drops it.
//
...
class PostgresBinding {
 public:
  ...
  static std::unordered_map<uint32_t, std::shared_ptr<PostgresPool>>& GetPools();
 ...
 private:
  ...
  static thread_local std::unordered_map<uint32_t, std::shared_ptr<PostgresPool>> pools_;
