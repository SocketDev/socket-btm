 ...
 *      scripts/repo/check-vendored-symbol-coverage.mts node
 *      scripts/repo/check-vendored-symbol-coverage.mts --explain node
 *      scripts/repo/check-vendored-symbol-coverage.mts --json Exit codes: 0 —
 *      all binding symbols are covered by listed sources. 1 — at least one
 *      symbol has no covering source. 2 — usage / args error.
 */
...
const VENDOR_PREFIXES: readonly VendorPrefix[] = [
  ...
  {
    name: 'md4c',
    callRegex: /\b(MD_[A-Z][A-Za-z0-9_]*|md_parse)\s*\(/g,
  },
]
 ...
 * Main coverage check.
 */
// oxlint-disable-next-line socket/sort-source-methods -- helpers grouped by topic; alphabetical interleave reads worse than topical clusters.
...
function parseArgs(): ParsedArgs {
  ...
  for (let i = 0, { length } = argv; i < length; i += 1) {
    ...
    if (a === '--explain') {
      ...
      logger.log(
        'Usage: node scripts/repo/check-vendored-symbol-coverage.mts [--explain] [--json]',
      )
      process.exit(0)
    } else {
