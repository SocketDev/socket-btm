...
function createMinimalPE(
  ...
  optHeader.writeUInt16LE(0x20b, 0) // PE64 magic
  ...
  if (!noResourceDir) {
    optHeader.writeUInt32LE(0x1000, resourceDirOffset) // Resource RVA
    optHeader.writeUInt32LE(0x2000, resourceDirOffset + 4) // Resource Size
  }
  ...
  for (let i = 0; i < numSections; i++) {
    ...
    if (i === 0) {
      ...
      sectionHeaders.writeUInt32LE(0x2000, offset + 8) // VirtualSize
      sectionHeaders.writeUInt32LE(0x1000, offset + 12) // VirtualAddress (matches resource RVA)
      sectionHeaders.writeUInt32LE(0x2000, offset + 16) // SizeOfRawData
      sectionHeaders.writeUInt32LE(0x400, offset + 20) // PointerToRawData
    } else {
      // Other sections
  ...
  resourceDir.writeUInt32LE(0x80000080, entryOffset + 4) // Offset to subdirectory (high bit set)
  ...
  resourceDir.writeUInt32LE(0x800000c0, 0x80 + 20) // Offset to level 3
  ...
  resourceDir.writeUInt32LE(0x1100, 0xe0) // Data RVA
  ...
  versionData.writeUInt32LE(0xfeef04bd, 0) // Signature
  versionData.writeUInt32LE(0x00010000, 4) // StructVersion
  versionData.writeUInt32LE(0x00160018, 8) // FileVersionMS (24.22)
  versionData.writeUInt32LE(0x00000003, 12) // FileVersionLS (0.3)

  // Build the complete PE file
  // Layout: DOS header + PE header + Optional header + Sections + Resource data
  const rsrcOffset = 0x400 // Where .rsrc section data starts
  const versionDataOffset = rsrcOffset + 0x100 // Offset 0x1100 - 0x1000 = 0x100 within .rsrc

  const totalSize = rsrcOffset + 0x2000 // Include room for resource data
...
describe('PE VS_VERSION_INFO scan guards', () => {
  ...
  describe('version data size guard', () => {
    ...
    it('should reject version data too large (> 65536 bytes)', async () => {
      ...
      const pe = createMinimalPE({ versionDataSize: 70000 })
    ...
    it('should accept version data at upper limit (65536 bytes)', async () => {
      const pe = createMinimalPE({ versionDataSize: 65536 })
