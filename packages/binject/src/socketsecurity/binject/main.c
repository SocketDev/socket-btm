 ...
 *   Path: /usr/bin/node
 *   Note: Same path as system apt, different repository for newer versions
...
#else
    // Linux only: apt/apt-get, NodeSource: /usr/bin/node
    ...
    if (strcmp(command, "inject") == 0) {
        ...
        if (sea_resource && is_json_file(sea_resource)) {
            ...
            if (config) {
                ...
                char *extracted_node_version = NULL;  // Track for cleanup
                if (parse_smol_update_config(config->smol, &smol_update_config) == 0) {
                    ...
                    if (!smol_update_config.node_version || strlen(smol_update_config.node_version) == 0) {
                        extracted_node_version = smol_extract_node_version_fast(executable);
                ...
                // Free extracted version after serialization (serialization copies the string)
                if (extracted_node_version) {
                    free(extracted_node_version);
                }
