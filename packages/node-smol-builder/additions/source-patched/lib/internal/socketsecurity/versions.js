...
function parseMaven(version) {
  ...
  // Use explicit length check instead of `nums[i] || 0` — `|| 0` collapses
  // a real zero ("0", "0.1") with a missing segment (no digits parsed).
  // Maven inputs like "alpha" or "0-RC1" both need these to default to
  // 0, but a future compare that distinguishes "0.0.0" from "missing"
  // would see the same value. Explicit zero is the right default now;
  // the comparison semantics are clearer when a missing part is spelled out.
  const major = nums.length > 0 ? nums[0] : 0
  const minor = nums.length > 1 ? nums[1] : 0
  const patch = nums.length > 2 ? nums[2] : 0
...
// Also records which components were explicitly present in the input so
// caret/tilde satisfies() can treat missing components as "any value" per
// npm semver: `~1` := >=1.0.0 <2.0.0 (minor any), `^1` := >=1.0.0 <2.0.0.
// Attaches `_partialMinor`/`_partialPatch` flags when the input was
// partial ("1" or "1.2").
function tryParseOrCoerce(str, ecosystem) {
  ...
  const parsed = tryParse(coerced, ecosystem)
  if (!parsed) {
    return undefined
  }
  // Mark which parts were absent (or wildcard-like) in the input so
  // caret/tilde satisfies() can treat `~1` as `>=1.0.0 <2.0.0` instead
  // of the too-strict `~1.0.0` := `>=1.0.0 <1.1.0`. parseNpm (and the
  // other ecosystem parsers) return frozen records cached across calls,
  // so we can't mutate `parsed`; instead wrap it in a fresh frozen
  // record carrying the partial flags alongside every field the parser
  // produced (shape varies by ecosystem — gem has `_gem`, etc).
  const partialMinor =
    !match[2] || RegExpPrototypeTest(RANGE_XCH_REGEX, match[2])
  const partialPatch =
    !match[3] || RegExpPrototypeTest(RANGE_XCH_REGEX, match[3])
  return ObjectFreeze({
    __proto__: null,
    ...parsed,
    _partialMinor: partialMinor,
    _partialPatch: partialPatch,
  })
}
...
function parseComparator(comp, ecosystem) {
  ...
  //
  // Special case for `=`: npm treats `= 1` / `= 1.2` as the partial-range
  // form (`>=1.0.0 <2.0.0`), NOT as exact-match `=1.0.0`. When the operator
  // is `=` and the operand is a partial version, route to the wildcard
  // branch below by dropping the `=` prefix. For fully-qualified `= 1.2.3`
  // we still want exact-match, so only redirect when tryParse (strict)
  // rejects the operand.
  const opMatch = RegExpPrototypeExec(RANGE_OPERATOR_REGEX, comp)
  if (opMatch) {
    if (opMatch[1] === '=' && !tryParse(opMatch[2], ecosystem)) {
      // Fall through to wildcard handling with the `=` stripped. The
      // wildcard branch at the bottom matches partial forms `1`, `1.x`,
      // `1.2`, `1.2.x`, etc.
      comp = opMatch[2]
    } else {
...
function satisfiesComparator(version, comp, checkPrerelease = true) {
  ...
  switch (comp.op) {
    ...
    case '^': {
      ...
      // Partial forms expand more leniently per npm semver:
      //   ^1     := >=1.0.0 <2.0.0 (treat missing minor/patch as "any")
      //   ^0.2   := >=0.2.0 <0.3.0 (missing patch is "any")
      //   ^0.0   := >=0.0.0 <0.1.0 (missing patch is "any")
      ...
      } else if (cv.minor > 0 || cv._partialPatch) {
    ...
    case '~': {
      // ~1.2.3 := >=1.2.3 <1.3.0
      // Partial forms:
      //   ~1    := >=1.0.0 <2.0.0 (missing minor/patch ⇒ match any minor)
      //   ~1.2  := >=1.2.0 <1.3.0 (full tilde semantics)
      const cv = comp.version
      if (compare(v, cv) < 0) {
        return false
      }
