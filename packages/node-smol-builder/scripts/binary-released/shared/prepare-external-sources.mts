...
export async function generateVendoredGypi(): Promise<void> {
  ...
  for (let i = 0, { length } = VENDORED_GYPI_BUNDLES; i < length; i += 1) {
    ...
    // Paths in the gypi must be relative to the patched node source
    // root (where node.gyp lives), which is the directory ABOVE
    // additions/source-patched/. ADDITIONS_SOURCE_PATCHED_DIR is the
    // pre-sync location; the actual sources land under
    // <source>/<rel-paths> at build time. Emit forward-slash paths;
    // gyp normalizes per-host.
    const relSources = sources.map(s =>
      path.relative(ADDITIONS_SOURCE_PATCHED_DIR, s).split(path.sep).join('/'),
    )
    const lines: string[] = []
