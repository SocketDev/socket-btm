...
# Build LIEF first via lief-builder.
echo "1. Building LIEF..."
BINJECT_DIR="$(dirname "$0")/.."
cd "$BINJECT_DIR/../lief-builder"
pnpm run build

# Check if LIEF was built.
LIEF_LIB="$BINJECT_DIR/../lief-builder/build/dev/lief-install/lib/libLIEF.a"
if [[ ! -f "$LIEF_LIB" ]]; then
    echo "❌ LIEF library not found at $LIEF_LIB"
    exit 1
fi
