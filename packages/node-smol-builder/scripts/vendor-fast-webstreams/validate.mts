...
async function getDefaultBinary() {
...
async function main() {
  const binaryPath = process.argv[2] || (await getDefaultBinary())
  ...
  try {
    const result = await spawn(binaryPath, ['-e', TEST_CODE], {
      stdio: 'inherit',
      env: { ...process.env },
    })
    process.exitCode = result.code ?? 0
  } catch (e) {
    logger.fail('Failed to run validation:', errorMessage(e))
    process.exitCode = 1
  }
}
