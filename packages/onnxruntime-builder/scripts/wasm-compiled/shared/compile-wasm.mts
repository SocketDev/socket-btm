...
// Lazy — a module-scope getDefaultSpinner() call captures a TTY/stdout
// handle at import time, which is slow to start and not V8-snapshot-safe.
let cachedSpinner
function getSpinner() {
  return cachedSpinner || (cachedSpinner = getDefaultSpinner())
}
...
export async function compileWasm(options) {
  ...
  // Add --allow_running_as_root only when necessary AND supported.
