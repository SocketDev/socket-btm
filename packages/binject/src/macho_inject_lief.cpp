 ...
 * (see src/MachO/Binary.cpp:1431-1468) that can fail with newly created segments,
 * so we use SegmentCommand::add_section() which only updates metadata.
 *
 * The workaround:
...
static bool sign_binary_adhoc(const char* binary_path) {
    ...
    if (pid == 0) {
        ...
        fprintf(stderr, "⚠ Failed to execute codesign\n");
        exit(1);
    }
 ...
 *    See: vendor/lief/submodule/src/MachO/Binary.cpp:1431-1468
 *
 * 3. Content Storage: The key insight is that LIEF stores section content via
 *    the .content() method (not the constructor), as shown in the ELF example:
 *    vendor/lief/submodule/examples/cpp/elf_add_section.cpp:33-36
 *
...
extern "C" int binject_inject_macho_lief(const char* executable,
  ...
  try {
    ...
    // Step 7: Write binary using Binary::write() method
    // This properly handles the section content we set via .content() method
    printf("Writing modified binary with LIEF...\n");
 ...
 * @return BINJECT_OK on success, error code otherwise.
 */
extern "C" int binject_list_macho_lief(const char* executable) {
  if (!executable) {
    fprintf(stderr, "Error: Invalid arguments\n");
    return BINJECT_ERROR_INVALID_ARGS;
  }
 ...
 * @return BINJECT_OK on success, error code otherwise.
 */
extern "C" int binject_extract_macho_lief(const char* executable,
                                           const char* section_name,
 ...
 * @return BINJECT_OK on success, error code otherwise.
 */
extern "C" int binject_verify_macho_lief(const char* executable,
                                          const char* section_name) {
 ...
 */
extern "C" int binject_inject_macho_lief_batch(
  ...
  try {
    ...
    // Flip NODE_SEA_FUSE BEFORE removing/adding segment (if SEA data provided)
    // IMPORTANT: Must check fuse BEFORE removing segment to avoid accessing invalid section references
    // Check if fuse is already flipped by looking for both :0 and :1 versions
    if (sea_data && sea_size > 0) {
      ...
      // First, check if fuse is already flipped
      for (const LIEF::MachO::Section& section : binary->sections()) {
        LIEF::span<const uint8_t> content_span = section.content();
        std::vector<uint8_t> content(content_span.begin(), content_span.end());

        for (size_t i = 0; i + fuse_length <= content.size(); i++) {
          if (memcmp(content.data() + i, fuse_flipped, fuse_length) == 0) {
            already_flipped = true;
            break;
          }
        }
        if (already_flipped) break;
      }

      if (already_flipped) {
        printf("NODE_SEA_FUSE already flipped (skipping)\n");
      } else {
        // Fuse not flipped yet, search for unflipped version and flip it
        printf("Flipping NODE_SEA_FUSE...\n");
