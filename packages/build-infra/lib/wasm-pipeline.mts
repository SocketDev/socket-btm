...
import { formatDuration, getFileSize } from './build-helpers.mts'
import { ensureToolInstalled } from './tool-installer.mts'
import { generateSyncCjs } from '../wasm-synced/generate-sync-cjs.mts'
import { generateSyncEsm } from '../wasm-synced/generate-sync-esm.mts'
...
export async function optimizeWasm(options) {
  ...
  // Validate input WASM file
  const inputBuffer = await fs.readFile(inputFile)
  // WebAssembly.Module validates magic number and throws if invalid
  // eslint-disable-next-line no-new
  new WebAssembly.Module(inputBuffer)
  ...
  // Validate output WASM file
  const outputBuffer = await fs.readFile(outputFile)
  // WebAssembly.Module validates magic number and throws if invalid
  // eslint-disable-next-line no-new
  new WebAssembly.Module(outputBuffer)
 ...
 * @param {string} options.inputFile - Input WASM file path
 * @param {string} options.mtsFile - Output .mts file path
...
export async function generateSyncWrappers(options) {
  ...
  await fs.writeFile(mjsFile, esmCode, 'utf8')
  ...
  await fs.writeFile(cjsFile, cjsCode, 'utf8')
 ...
 * @param {string} options.mtsFile - Source .mts file
 ...
 * @param {string} options.outputMjsName - Output .mts filename
