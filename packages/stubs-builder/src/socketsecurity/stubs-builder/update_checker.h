...
static void extract_version_from_tag(const char *tag, const char *pattern,
    ...
    if (!pattern || pattern[0] == '\0') {
        snprintf(version, version_size, "%s", tag);
        return;
    }
...
static const char* json_find_string(const char *json, const char *key,
    ...
    char search_key[128];
    /* Use snprintf return value to avoid strlen() call. */
    int key_len = snprintf(search_key, sizeof(search_key), "\"%s\"", key);
    if (key_len < 0 || (size_t)key_len >= sizeof(search_key)) return NULL;

    const char *pos = strstr(json, search_key);
    if (!pos) return NULL;

    pos += key_len;  /* Use precomputed length instead of strlen(). */
...
static int parse_github_releases(const char *json, const char *pattern,
        ...
        if (json_find_string(obj_start, "tag_name", tag, sizeof(tag))) {
            ...
            if (!pattern || pattern[0] == '\0' || glob_match(pattern, tag)) {
                ...
                if (compare_timestamps(published_at, best_published_at) > 0) {
                    snprintf(best_tag, sizeof(best_tag), "%s", tag);
                    snprintf(best_published_at, sizeof(best_published_at), "%s", published_at);
                }
            }
        }
    }
    ...
    if (best_tag[0] != '\0') {
        snprintf(latest_tag, tag_size, "%s", best_tag);
        return 0;
    }
...
static int execute_curl(const char *url, char *response, size_t response_size) {
    ...
    if (!temp) { curl_easy_cleanup(curl); return -1; }
    headers = temp;

    temp = curl_slist_append(headers, "X-GitHub-Api-Version: 2022-11-28");
    if (!temp) { curl_slist_free_all(headers); curl_easy_cleanup(curl); return -1; }
    headers = temp;

    temp = curl_slist_append(headers, "User-Agent: socket-stub-updater/1.0");
    if (!temp) { curl_slist_free_all(headers); curl_easy_cleanup(curl); return -1; }
    headers = temp;
    ...
    if (token && token[0] != '\0') {
        ...
        temp = curl_slist_append(headers, auth_header);
        if (!temp) { curl_slist_free_all(headers); curl_easy_cleanup(curl); return -1; }
        headers = temp;
    }
