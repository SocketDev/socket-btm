...
class Transaction {
  ...
  constructor(adapter, connection, options) {
    ...
    const handler = {
      ...
      get(target, prop, receiver) {
        ...
        if (ObjectHasOwn(self, prop) || ObjectHasOwn(Transaction.prototype, prop)) {
   ...
   */
  async rollback() {
    if (this[kState] === STATE_COMMITTED || this[kState] === STATE_ROLLED_BACK) {
...
class Savepoint {
  ...
  constructor(adapter, connection, name) {
    ...
    const handler = {
      ...
      get(target, prop, receiver) {
        // Use ObjectHasOwn to prevent prototype pollution attacks.
        if (ObjectHasOwn(self, prop) || ObjectHasOwn(Savepoint.prototype, prop)) {
          const value = self[prop];
          if (typeof value === 'function') {
            return FunctionPrototypeBind(value, self);
          }
