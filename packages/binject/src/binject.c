...
int binject_batch(const char *executable, const char *output,
                         ...
                         int vfs_in_memory, int skip_repack, const uint8_t *update_config_binary) {
    ...
    if (vfs_resource && strlen(vfs_resource) > 0) {
        ...
        if (is_directory(vfs_resource)) {
            ...
            int read_rc = binject_read_resource(vfs_resource, &vfs_data, &vfs_size);
            if (read_rc != BINJECT_OK) {
                free(sea_data);
                return read_rc;
            }
            ...
            size_t tar_size = 0;
            int read_rc = binject_read_resource(vfs_resource, &tar_data, &tar_size);
            if (read_rc != BINJECT_OK) {
                free(sea_data);
                return read_rc;
            }
            ...
            size_t file_size = 0;
            int read_rc = binject_read_resource(vfs_resource, &file_data, &file_size);
            if (read_rc != BINJECT_OK) {
                free(sea_data);
                return read_rc;
            }
    ...
    if (is_compressed) {
        if (skip_repack) {
            ...
            rc = binject_repack_workflow(executable, target_binary, output, update_config_binary);
            if (rc != BINJECT_OK) {
