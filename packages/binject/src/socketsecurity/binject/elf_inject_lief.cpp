...
extern "C" int binject_elf_lief_batch(
  ...
  if (!executable || !output) {
    ...
    if (sea_data && sea_size > 0) {
        ...
        notes.emplace_back(ELF_NOTE_NODE_SEA_BLOB, sea_vec);
    }
    ...
    if ((vfs_data && vfs_size > 0) || vfs_compat_mode) {
        ...
        notes.emplace_back(ELF_NOTE_SMOL_VFS_BLOB, vfs_vec);
    }
    ...
    if (vfs_config_data) {
        ...
        notes.emplace_back(ELF_NOTE_SMOL_VFS_CONFIG, vfs_config_vec);
    }
