...
const tmp = mkdtempSync(path.join(os.tmpdir(), 'detect-feat-'))
...
describe('detectBundleFeatures', () => {
  it('drops every auto feature when the bundle uses none', async () => {
    ...
    expect(m.features['ai']!.drop).toBe(true)
    ...
    expect(m.configureFlags).toContain('--without-smol-ai')
  })

  it('keeps node:smol-ai when the bundle imports its Prompt API', async () => {
    const p = bundle(`
      const { LanguageModel } = require('node:smol-ai')
      LanguageModel.create()
    `)
    const m = await detectBundleFeatures({ bundlePath: p })
    expect(m.features['ai']!.use).toBe('hard')
    expect(m.features['ai']!.drop).toBe(false)
    expect(m.configureFlags).not.toContain('--without-smol-ai')
  })
