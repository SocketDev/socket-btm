// ============================================================================
// smol_config.c — SMFG config serialization implementation
// ============================================================================
//
// WHAT THIS FILE DOES
// Takes a smol_update_config_t struct (already parsed from JSON elsewhere)
// and packs it into a fixed 1192-byte binary buffer in SMFG format. The
// buffer has a magic header, numeric fields, and length-prefixed strings.
//
// WHY IT EXISTS
// The self-update config must be embedded as raw bytes inside the
// executable. This serializer converts the friendly struct into the exact
// byte layout that the runtime decompressor expects to read back.
// ============================================================================
...
void smol_config_init(smol_update_config_t *config) {
    ...
    config->binname = strdup("");
    config->command = strdup("self-update");
    config->url = strdup("");
    config->tag = strdup("");
    config->skip_env = strdup("");
    config->fake_argv_env = strdup("SMOL_FAKE_ARGV");
    config->prompt = false;
    config->prompt_default = 'n';
    config->interval = 86400000;        // 24 hours in ms
    config->notify_interval = 86400000; // 24 hours in ms
    config->node_version = strdup("");  // Empty by default (will be filled by build system)
}

void smol_config_free(smol_update_config_t *config) {
    if (!config) return;

    free((void*)config->binname);
    free((void*)config->command);
    free((void*)config->url);
    free((void*)config->tag);
    free((void*)config->skip_env);
    free((void*)config->fake_argv_env);
    free((void*)config->node_version);

    config->binname = NULL;
    config->command = NULL;
    config->url = NULL;
    config->tag = NULL;
    config->skip_env = NULL;
    config->fake_argv_env = NULL;
    config->node_version = NULL;
}
...
uint8_t* serialize_smol_config(const smol_update_config_t *config) {
    ...
    // Header (8 bytes).
    // Use memcpy for 32-bit and 16-bit values (assumes little-endian host).
    uint32_t magic = SMOL_CONFIG_MAGIC;
    uint16_t version = SMOL_CONFIG_VERSION;
    ...
    // Numeric values (16 bytes, little-endian).
    // Use memcpy instead of byte-by-byte loop (assumes little-endian host).
    memcpy(buffer + offset, &config->interval, sizeof(config->interval));
    offset += sizeof(config->interval);
    memcpy(buffer + offset, &config->notify_interval, sizeof(config->notify_interval));
    offset += sizeof(config->notify_interval);
