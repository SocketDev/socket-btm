// ============================================================================
// remove_signature_lib.c — Strip code signatures from Mach-O binaries
// ============================================================================
//
// WHAT THIS FILE DOES
// Memory-maps a Mach-O (macOS) binary, walks its load commands to find the
// LC_CODE_SIGNATURE entry, and zeroes it out. This removes Apple's code
// signature so the binary can be modified and re-signed later.
//
// WHY IT EXISTS
// macOS code signatures become invalid the moment you change a single byte
// in the binary. Before injecting SEA/VFS data, we must strip the old
// signature. After injection, a fresh ad-hoc signature is applied.
// On non-Apple platforms this file compiles to a stub that returns an error.
// ============================================================================
...
int remove_macho_signature(const char *path) {
    ...
    for (uint32_t i = 0; i < header->ncmds; i++) {
        ...
        // Mach-O load commands MUST be 8-byte aligned (LC_REQ_DYLD ABI).
        // A misaligned cmdsize would advance lc to a misaligned address,
        // producing UB on strict-alignment hosts (SIGBUS on ARM64 pre-ARMv8.1).
        if ((lc->cmdsize % 8) != 0) {
            fprintf(stderr, "Error: Misaligned load command size: %u (must be 8-byte aligned)\n", lc->cmdsize);
            munmap(map, st.st_size);
            close(fd);
            return -1;
        }
        ...
        if (lc->cmd == LC_CODE_SIGNATURE) {
            ...
            // Invalidate the load command while preserving cmdsize.
            lc->cmd = 0;
            found = 1;
            break;
        }
    ...
    // Note: Explicit fsync after msync removed to avoid blocking indefinitely.
    // msync(MS_SYNC) has already flushed the modified pages to kernel buffers,
    // and the kernel will sync to disk asynchronously. For build tooling,
    // the rare power-failure corruption risk is acceptable.
