...
function getBinaryPath(packageName, binaryName) {
  ...
  const withPlatform = path.join(
    PROJECT_ROOT, 'packages', packageName, 'build', BUILD_MODE,
    platformArch, 'out', 'Final', binaryName + ext,
  )
  if (existsSync(withPlatform)) return withPlatform
  return path.join(
    PROJECT_ROOT, 'packages', packageName, 'build', BUILD_MODE,
    'out', 'Final', binaryName + ext,
  )
}
