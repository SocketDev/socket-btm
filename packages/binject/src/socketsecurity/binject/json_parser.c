// ============================================================================
// json_parser.c — sea-config.json parser implementation
// ============================================================================
//
// WHAT THIS FILE DOES
// Reads a sea-config.json file from disk, parses it with cJSON, and extracts
// the "output", "main", "smol.update", and "smol.vfs" fields into C structs.
// Includes security checks (path traversal, symlink detection, size limits).
//
// WHY IT EXISTS
// binject needs structured access to the SEA config. Rather than doing
// fragile string searching, this uses a proper JSON parser (cJSON) with
// validation, so malformed configs produce clear error messages.
// ============================================================================
...
// Forward declaration.
static vfs_build_config_t* parse_vfs_config_internal(cJSON *vfs);
...
static char* read_file_contents(const char *path, size_t *out_size) {
    ...
    // Use >= to also prevent size + 1 overflow when size == SIZE_MAX
    ...
    buffer[file_size] = '\0';  // Use validated size_t
...
sea_config_t* parse_sea_config(const char *config_path) {
    ...
    // Block ".." anywhere in path (covers ../, \.., and embedded ..)
    if (strstr(config_path, "..") != NULL) {
        fprintf(stderr, "Error: Path traversal detected in config path\n");
        return NULL;
    }
    // Note: Absolute paths ARE allowed for the config file path itself.
    // The config file is passed via CLI (e.g., --sea /path/to/config.json).
    // Path restrictions only apply to paths INSIDE the config (main, output).

    // Verify file exists and is regular file (not symlink).
    ...
    struct stat st;
    if (POSIX_LSTAT(config_path, &st) != 0) {
        fprintf(stderr, "Error: Config file not found: %s\n", config_path);
        return NULL;
    }

    // Block symlinks to prevent information disclosure attacks.
    // POSIX_S_ISLNK returns false on Windows (no-op since symlinks need admin).
    if (POSIX_S_ISLNK(st.st_mode)) {
        fprintf(stderr, "Error: Config path is a symbolic link (not allowed): %s\n", config_path);
        return NULL;
    }
 ...
 */
vfs_build_config_t* parse_vfs_config(cJSON *smol) {
    if (!smol || !cJSON_IsObject(smol)) {
        return NULL;
    }
    ...
    if (cJSON_IsBool(vfs)) {
        ...
        vfs_build_config_t *config = parse_vfs_config_internal(vfs);
        cJSON_Delete(vfs);
        return config;
    }
...
static vfs_build_config_t* parse_vfs_config_internal(cJSON *vfs) {
    ...
    vfs_build_config_t *config = calloc(1, sizeof(vfs_build_config_t));
    if (!config) {
 ...
 * Free VFS config.
 */
void free_vfs_config(vfs_build_config_t *config) {
    if (!config) return;
...
int parse_smol_update_config(cJSON *smol, smol_update_config_t *config) {
    ...
    // Extract string fields (strdup to decouple from cJSON tree lifetime).
    // Free the old (default) value before replacing with strdup'd JSON value.
    // Helper: replace a strdup'd field, returning -1 on allocation failure.
#define REPLACE_STR_FIELD(field, json_key)                                     \
    do {                                                                        \
        cJSON *_item = cJSON_GetObjectItemCaseSensitive(update, (json_key));    \
        if (_item && cJSON_IsString(_item) && _item->valuestring) {            \
            char *_copy = strdup(_item->valuestring);                          \
            if (!_copy) {                                                      \
                fprintf(stderr, "Error: strdup failed for '%s'\n", (json_key));\
                return -1;                                                     \
            }                                                                  \
            free((void*)config->field);                                        \
            config->field = _copy;                                             \
        }                                                                      \
    } while (0)

    REPLACE_STR_FIELD(binname, "binname");
    REPLACE_STR_FIELD(command, "command");
    REPLACE_STR_FIELD(url, "url");
    REPLACE_STR_FIELD(tag, "tag");
    REPLACE_STR_FIELD(skip_env, "skipEnv");
    REPLACE_STR_FIELD(fake_argv_env, "fakeArgvEnv");
    REPLACE_STR_FIELD(node_version, "nodeVersion");

#undef REPLACE_STR_FIELD
