...
static void Initialize(Local<Object> target,
  ...
  SetMethod(context, target, "writeJsonResponse", WriteJsonResponse);
  SetMethod(context, target, "writeBinaryResponse", WriteBinaryResponse);
  SetMethod(context, target, "writeNotModifiedResponse", WriteNotModifiedResponse);
  SetMethod(context, target, "acquireRequest", AcquireRequest);
  SetMethod(context, target, "releaseRequest", ReleaseRequest);
  SetMethod(context, target, "acquireResponse", AcquireResponse);
  SetMethod(context, target, "releaseResponse", ReleaseResponse);
  SetMethod(context, target, "getObjectPoolStats", GetObjectPoolStats);

  // New advanced optimization methods.
  SetMethod(context, target, "isTcpOptAvailable", IsTcpOptAvailable);
  SetMethod(context, target, "isIoUringAvailable", IsIoUringAvailable);
  SetMethod(context, target, "isMimallocAvailable", IsMimallocAvailable);
}
...
void RegisterExternalReferences(ExternalReferenceRegistry* registry) {
  ...
  registry->Register(WriteNotModifiedResponse);
...
NODE_BINDING_CONTEXT_AWARE_INTERNAL(smol_http_perf,
                                     node::socketsecurity::http_perf::Initialize)
NODE_BINDING_EXTERNAL_REFERENCE(smol_http_perf,
                                 node::socketsecurity::http_perf::RegisterExternalReferences)
