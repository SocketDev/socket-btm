 ...
 * - Zero-block detection (end of archive)
 * - Octal parsing
 */

const { ERR_INVALID_ARG_TYPE } = require('internal/errors').codes

// Native binding for SIMD-accelerated TAR operations
const binding = internalBinding('smol_vfs')
const {
  ArrayPrototypePush,
...
function calculateChecksum(buffer, offset) {
  ...
  return binding.tarCalculateChecksum(buffer, offset)
}
...
function parseHeader(buffer, offset, options = {}) {
  ...
  if (binding.tarIsZeroBlock(buffer, offset)) {
    // End of archive
    return
  }
