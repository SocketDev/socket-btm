// ============================================================================
// macho_inject_lief_wrapper.c — Mach-O (macOS) injection wrappers
// ============================================================================
//
// WHAT THIS FILE DOES
// Wraps the LIEF C++ functions for Mach-O binary injection behind a clean
// C API. Handles inject, list, extract, verify, and repack operations on
// macOS Mach-O executables.
//
// WHY IT EXISTS
// Mach-O is Apple's executable format. LIEF is written in C++ but binject's
// public API is pure C. This file bridges the two worlds — it validates
// arguments, prints status messages, and calls the C++ LIEF functions.
// ============================================================================
...
int binject_macho_repack_smol(const char *stub_path, const uint8_t *section_data,
    ...
    return binject_macho_repack_smol_lief(stub_path, section_data, section_size, output_path, extracted_source_path);
}
