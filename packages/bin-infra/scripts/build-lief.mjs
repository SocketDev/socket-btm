...
async function downloadPrebuiltLIEF() {
  try {
    ...
    // Find latest lief release using Octokit.
    const latestTag = await getLatestRelease('lief')
    ...
    const assetUrl = await getReleaseAssetUrl(latestTag, assetName)

    if (!assetUrl) {
      throw new Error(`Asset ${assetName} not found in release ${latestTag}`)
    }

    // Create build directory.
    await fs.mkdir(liefBuildDir, { recursive: true })

    // Download archive using httpDownload.
    logger.info(`Downloading ${assetName}...`)
    const downloadPath = path.join(liefBuildDir, assetName)

    await httpDownload(assetUrl, downloadPath, {
      logger,
    })
