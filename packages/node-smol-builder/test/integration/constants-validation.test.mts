...
describe('header Size Constant Validation', () => {
  it('should have correct MAGIC_MARKER length', () => {
    ...
    expect(MAGIC_MARKER).toHaveLength(32)
  ...
  it('should have correct HEADER_SIZES.INTEGRITY_HASH', () => {
    // SHA-512 of compressed data = 64 bytes. Must match INTEGRITY_HASH_LEN
    // in packages/bin-infra/src/socketsecurity/bin-infra/compression_constants.h.
    expect(HEADER_SIZES.INTEGRITY_HASH).toBe(64)
  })

  it('should calculate METADATA_HEADER_SIZE correctly', () => {
    // compressed_size (8) + uncompressed_size (8) + cache_key (16) + platform_metadata (3) + integrity_hash (64) + smol_config_flag (1) = 100 bytes
    const expectedSize =
  ...
  it('should calculate TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG correctly', () => {
    // marker (32) + metadata (100) = 132 bytes
    const expectedTotalSize = HEADER_SIZES.MAGIC_MARKER + METADATA_HEADER_SIZE

    expect(expectedTotalSize).toBe(132)
    expect(TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG).toBe(132)
    expect(TOTAL_HEADER_SIZE_WITHOUT_SMOL_CONFIG).toBe(expectedTotalSize)
  })
